...
4. Team artifacts (`.omg/state/team/*/`, tmux sessions matching `omg-team-*`) are best-effort cleared as part of the legacy fallback.
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omg/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected by checking for config files in `.omg/state/team/`:

```bash
# Check for active teams
ls .omg/state/team/*/config.json 2>/dev/null
```
...
```
For each team found in .omg/state/team/:
  1. Read config.json to get team_name and workers list
  2. For each worker:
     a. Write shutdown inbox to .omg/state/team/{name}/workers/{worker}/inbox.md
...
After graceful pass:
  ...
  2. Destroy the tmux session: tmux kill-session -t omg-team-{name}
```
...
1. Check `.omg/state/team/*/config.json` for active teams
...
5. Destroy tmux session: `tmux kill-session -t omg-team-{name}`
6. Strip GEMINI.md overlay
7. Remove state: `rm -rf .omg/state/team/{name}/`
...
if [[ -f .omg/state/autopilot-state.json ]]; then
  # Clean up ralph if active
  if [[ -f .omg/state/ralph-state.json ]]; then
    RALPH_STATE=$(cat .omg/state/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .omg/state/ultrawork-state.json ]]; then
      rm -f .omg/state/ultrawork-state.json
      echo "Cleaned up: ultrawork (linked to ralph)"
    fi

    # Clean ralph
    rm -f .omg/state/ralph-state.json
    rm -f .omg/state/ralph-verification.json
    echo "Cleaned up: ralph"
  fi

  # Clean up ultraqa if active
  if [[ -f .omg/state/ultraqa-state.json ]]; then
    rm -f .omg/state/ultraqa-state.json
    echo "Cleaned up: ultraqa"
  fi

  # Mark autopilot inactive but preserve state
  CURRENT_STATE=$(cat .omg/state/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .omg/state/autopilot-state.json
...
if [[ -f .omg/state/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .omg/state/ultrawork-state.json ]]; then
    UW_STATE=$(cat .omg/state/ultrawork-state.json)
  ...
  # Clean ralph state
  rm -f .omg/state/ralph-state.json
  rm -f .omg/state/ralph-plan-state.json
  rm -f .omg/state/ralph-verification.json
...
```bash
if [[ -f .omg/state/ultrawork-state.json ]]; then
...
echo "  - Ralph (.omg/state/ralph-state.json)"
echo "  - Ultrawork (.omg/state/ultrawork-state.json)"
echo "  - UltraQA (.omg/state/ultraqa-state.json)"
echo ""
...
5. Team artifacts (`.omg/state/team/*/`, tmux sessions matching `omg-team-*`) remain best-effort cleanup items invoked during the legacy/global pass.
...
```bash
rm -rf .omg/state/team/                  # All team state

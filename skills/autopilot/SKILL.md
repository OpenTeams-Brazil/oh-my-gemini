...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.omg/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.omg/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.omg/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.omg/plans/autopilot-spec.md`
...
Use `omg_state` MCP tools for autopilot lifecycle state.
...
Optional settings in `~/.gemini/config.toml`:
...
```toml
[omg.autopilot.pipeline]
maxRalphIterations = 10    # Ralph verification iteration ceiling
workerCount = 2            # Number of Gemini CLI team workers

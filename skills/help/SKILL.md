...
If you only need lightweight directory guidance scaffolding for `GEMINI.md` files, use:

```bash
omg agents-init .
```

That command is intentionally narrower than full setup: it only bootstraps `GEMINI.md` files for the target directory and its immediate child directories.
...
1. Reads token tracking from `~/.omg/state/token-tracking.jsonl`
2. Reads session history from `.omg/state/session-history.json`
...
TOKEN_FILE="$HOME/.omg/state/token-tracking.jsonl"
...
- "Set defaultExecutionMode in /omg-setup for consistent behavior"
...
- **README**: https://github.com/Yeachan-Heo/oh-my-gemini
- **Issues**: https://github.com/Yeachan-Heo/oh-my-gemini/issues

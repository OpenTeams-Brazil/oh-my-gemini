...
- Child prompts stay under GEMINI.md authority.
...
For Gemini native child agents, model routing defaults to inheritance/current repo defaults unless the caller has a concrete reason to override it.
</model_routing>
...
|-------------|-------|--------|
| "ralph", "don't stop", "must complete", "keep going" | `$ralph` | Read `./.gemini/skills/ralph/SKILL.md`, execute persistence loop |
| "autopilot", "build me", "I want a" | `$autopilot` | Read `./.gemini/skills/autopilot/SKILL.md`, execute autonomous pipeline |
| "ultrawork", "ulw", "parallel" | `$ultrawork` | Read `./.gemini/skills/ultrawork/SKILL.md`, execute parallel agents |
| "ultraqa" | `$ultraqa` | Read `./.gemini/skills/ultraqa/SKILL.md`, run QA cycling workflow |
| "analyze", "investigate" | `$analyze` | Read `./.gemini/skills/analyze/SKILL.md`, run deep analysis |
...
| "team", "swarm", "coordinated team", "coordinated swarm" | `$team` | Read `./.gemini/skills/team/SKILL.md`, start team orchestration (swarm compatibility alias) |
| "ecomode", "eco", "budget" | `$ecomode` | Read `./.gemini/skills/ecomode/SKILL.md`, enable token-efficient mode |
| "cancel", "stop", "abort" | `$cancel` | Read `./.gemini/skills/cancel/SKILL.md`, cancel active modes |
| "tdd", "test first" | `$tdd` | Read `./.gemini/skills/tdd/SKILL.md`, start test-driven workflow |
| "fix build", "type errors" | `$build-fix` | Read `./.gemini/skills/build-fix/SKILL.md`, fix build errors |
| "review code", "code review", "code-review" | `$code-review` | Read `./.gemini/skills/code-review/SKILL.md`, run code review |
| "security review" | `$security-review` | Read `./.gemini/skills/security-review/SKILL.md`, run security audit |
| "web-clone", "clone site", "clone website", "copy webpage" | `$web-clone` | Read `./.gemini/skills/web-clone/SKILL.md`, start website cloning pipeline |
...
When to use what:
- Use `omg explore --prompt ...` for simple read-only lookups.
- Use `omg sparkshell` for noisy read-only shell commands, bounded verification runs, repo-wide listing/search, or tmux-pane summaries; `omg sparkshell --tmux-pane ...` is explicit opt-in.
- Keep ambiguous, implementation-heavy, edit-heavy, or non-shell-only work on the richer normal path.
- `omg explore` is a shell-only, allowlisted, read-only path; do not rely on it for edits, tests, diagnostics, MCP/web access, or complex shell composition.
- If `omg explore` or `omg sparkshell` is incomplete or ambiguous, retry narrower and gracefully fall back to the normal path.
...
- Persist verdict JSON in `.omg/state/{scope}/ralph-progress.json`.
...
Execute `omg setup` to install all components. Execute `omg doctor` to verify installation.

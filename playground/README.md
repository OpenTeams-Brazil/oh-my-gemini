...
This folder collects **small, reproducible research-style demos** used to showcase `omg autoresearch` on harder optimization problems.
...
- evaluator-driven keep/discard loops that are easy to inspect under `.omg/logs/autoresearch/`
...
```bash
omg autoresearch missions/noisy-bayesopt-highdim
```

Then inspect:

```bash
RUN_ID=$(find .omg/logs/autoresearch -maxdepth 1 -mindepth 1 -type d -printf '%f\n' | sort | tail -n 1)
cat .omg/logs/autoresearch/$RUN_ID/manifest.json
cat .omg/logs/autoresearch/$RUN_ID/candidate.json
cat .omg/logs/autoresearch/$RUN_ID/iteration-ledger.json
```
...
- runtime autoresearch logs under `.omg/logs/`

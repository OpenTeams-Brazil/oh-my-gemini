...
fn temp_output_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    env::temp_dir().join(format!("omg-explore-{}-{}.md", std::process::id(), nanos))
}
...
fn build_direct_wrapper(self_exe: &Path, command: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "#!/bin/sh\nprintf '%s\\n' {} >&2\nexit 127\n",
        shell_quote(&format!(
            "omg explore allowlisted host command `{command}` is unavailable on this host"
        )),
    ))
}
...
fn validate_direct_command(command_name: &str, args: &[String]) -> Result<(), String> {
    if !ALLOWED_DIRECT_COMMANDS.contains(&command_name) {
        return Err(format!(
            "command `{command_name}` is not on the omg explore allowlist"
        ));
    }
    ...
    match command_name {
        "rg" => {
            ...
            {
                return Err("ripgrep `--pre` is not allowed in omg explore".to_string());
            }
            if args.iter().any(|arg| arg == "-") {
                return Err("ripgrep stdin (`-`) is not allowed in omg explore".to_string());
            }
        }
        "grep" => {
            if args.iter().any(|arg| arg == "-") {
                return Err("grep stdin (`-`) is not allowed in omg explore".to_string());
            }
            if non_option_operands(args).len() < 2 {
                return Err(
                    "grep requires a pattern and at least one file/path in omg explore".to_string(),
                );
            }
        }
        "find" => {
            if args.iter().any(|arg| {
                ...
                return Err(
                    "find actions that execute, delete, or write files are not allowed in omg explore"
                        .to_string(),
                );
            }
        }
        "cat" => {
            ...
            if operands.is_empty() {
                return Err("cat requires at least one file/path in omg explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("cat stdin (`-`) is not allowed in omg explore".to_string());
            }
        }
        "head" | "wc" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err(format!(
                    "{command_name} requires at least one file/path in omg explore"
                ));
            }
            if operands.contains(&"-") {
                return Err(format!(
                    "{command_name} stdin (`-`) is not allowed in omg explore"
                ));
            }
        }
        "tail" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err("tail requires at least one file/path in omg explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("tail stdin (`-`) is not allowed in omg explore".to_string());
            }
            if args.iter().any(|arg| {
                matches!(arg.as_str(), "-f" | "-F" | "--retry") || arg.starts_with("--follow")
            }) {
                return Err("tail follow/retry modes are not allowed in omg explore".to_string());
            }
        }
        _ => {}
    }
...
fn validate_repo_paths(command_name: &str, args: &[String]) -> Result<(), String> {
    ...
    for operand in candidate_paths {
        ...
        if !normalized.starts_with(&repo_root) {
            return Err(format!(
                "path `{operand}` escapes the omg explore repository root {}",
                repo_root.display()
            ));
        }
        if let Some(canonical_candidate) = canonicalize_existing_prefix(&normalized) {
            if let Some(canonical_repo_root) = &canonical_repo_root {
                if !canonical_candidate.starts_with(canonical_repo_root) {
                    return Err(format!(
                        "path `{operand}` resolves outside the omg explore repository root {}",
...
"#,
    ...
    fn discover_codex_support_dirs_includes_home_omg_and_codex_when_present() {
        ...
        create_dir_all(home_dir.join(".omg")).expect("create .omg");
        create_dir_all(home_dir.join(".gemini")).expect("create .gemini");
        ...
        assert_eq!(dirs, vec![home_dir.join(".omg"), home_dir.join(".gemini")]);
    }

/**
 * Base mode lifecycle management for oh-my-gemini
...
export async function startMode(
  ...
  const stateBase: ModeState = {
    ...
    ...(mode === 'ralph' && scope.sessionId ? { owner_omg_session_id: scope.sessionId } : {}),
  };
...
export async function updateModeState(
  ...
  if (mode === 'ralph' && scope.sessionId && typeof updatedBase.owner_omg_session_id !== 'string') {
    updatedBase.owner_omg_session_id = scope.sessionId;
  }

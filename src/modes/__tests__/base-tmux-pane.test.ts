...
describe('modes/base tmux pane capture', () => {
  it('captures tmux_pane_id in mode state on startMode()', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-pane-'));
    try {
      await startMode('ralph', 'test', 1, wd);
      const raw = JSON.parse(await readFile(join(wd, '.omg', 'state', 'ralph-state.json'), 'utf-8'));
  ...
  it('blocks exclusive mode startup when another exclusive state file is malformed', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-malformed-'));
    try {
      ...
      await assert.rejects(
        ...
        /repair or clear that workflow state yourself via `omg state clear --mode ralph` or the `omg_state\.\*` MCP tools/i,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }

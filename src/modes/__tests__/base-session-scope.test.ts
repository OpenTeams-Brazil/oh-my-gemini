...
describe('modes/base session-scoped persistence', () => {
  it('writes mode state into the current session scope when session.json exists', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-session-scope-'));
    try {
      await mkdir(join(wd, '.omg', 'state'), { recursive: true });
      await writeFile(join(wd, '.omg', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-base-write' }));

      await startMode('ralplan', 'write in session scope', 5, wd);

      const scopedPath = join(wd, '.omg', 'state', 'sessions', 'sess-base-write', 'ralplan-state.json');
      assert.equal(existsSync(scopedPath), true);
      assert.equal(existsSync(join(wd, '.omg', 'state', 'ralplan-state.json')), false);
  ...
  it('persists owner_omg_session_id for Ralph when session scope is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-session-ralph-owner-'));
    try {
      ...
      assert.equal(scoped.owner_omg_session_id, sessionId);
  ...
  it('prefers session-scoped reads over root fallback and writes updates back to the session scope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-session-read-'));
    try {
      const stateDir = join(wd, '.omg', 'state');
  ...
  it('allows an explicit Ralph start to overwrite an inactive current-session Ralph file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-mode-session-ralph-restart-'));
    try {
      const stateDir = join(wd, '.omg', 'state');

...
describe('CI Rust gates', () => {
  ...
  it('uses the current crates/omg-sparkshell manifest for Rust coverage', () => {
    const workflow = readCiWorkflow();

    assert.match(workflow, /crates\/omg-sparkshell\/Cargo\.toml/);
    assert.doesNotMatch(workflow, /native\/omg-sparkshell\/Cargo\.toml/);
  });
  ...
  it('uses the current sparkshell crate manifest in the Rust coverage lane', () => {
    ...
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omg-sparkshell\/Cargo\.toml --summary-only/);
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omg-sparkshell\/Cargo\.toml --lcov --output-path coverage\/rust\/omg-sparkshell\.lcov/);
    assert.doesNotMatch(workflow, /native\/omg-sparkshell\/Cargo\.toml/);
  });
});

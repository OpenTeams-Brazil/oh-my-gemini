 ...
 * Storage: .omg/state/{mode}-state.json
 */
				...
				const {
					...
					await withStateWriteLock(path, async () => {
						...
						if (
							mode === "ralph" &&
							effectiveSessionId &&
						typeof mergedRaw.owner_omg_session_id !== "string"
					) {
						mergedRaw.owner_omg_session_id = effectiveSessionId;
					}

...
const SERVER_DISABLE_ENV: Record<McpServerName, string> = {
  state: 'OMG_STATE_SERVER_DISABLE_AUTO_START',
  memory: 'OMG_MEMORY_SERVER_DISABLE_AUTO_START',
  code_intel: 'OMG_CODE_INTEL_SERVER_DISABLE_AUTO_START',
  trace: 'OMG_TRACE_SERVER_DISABLE_AUTO_START',
  wiki: 'OMG_WIKI_SERVER_DISABLE_AUTO_START',
};

const GLOBAL_DISABLE_ENV = 'OMG_MCP_SERVER_DISABLE_AUTO_START';
const LIFECYCLE_DEBUG_ENV = 'OMG_MCP_TRANSPORT_DEBUG';
...
export function autoStartStdioMcpServer(
  ...
  const logLifecycle = (message: string, error?: unknown) => {
    ...
    process.stderr.write(`[omg-${serverName}-server] ${message}${detail}\n`);
  };
  ...
  const shutdown = async (reason: string) => {
    ...
    try {
      ...
      console.error(`[omg-${serverName}-server] shutdown failed`, error);
    }

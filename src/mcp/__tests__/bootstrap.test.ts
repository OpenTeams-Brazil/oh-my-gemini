...
describe('mcp bootstrap auto-start guard', () => {
  ...
  it('disables all servers when global disable flag is set', () => {
    const env = { OMG_MCP_SERVER_DISABLE_AUTO_START: '1' };
...
describe('mcp duplicate sibling detection', () => {
  it('extracts same-entrypoint markers from command lines', () => {
    assert.equal(
      extractMcpEntrypointMarker('node /tmp/oh-my-gemini/dist/mcp/state-server.js'),
      'state-server.js',
    );
    assert.equal(
      extractMcpEntrypointMarker('node C:\\\\tmp\\\\oh-my-gemini\\\\dist\\\\mcp\\\\trace-server.ts'),
      'trace-server.ts',
    );

...
describe('state paths', () => {
  ...
  it('builds global state paths', () => {
    ...
    assert.equal(base, '/repo/.omg/state');
    assert.equal(getStateDir('/repo'), '/repo/.omg/state');
    assert.equal(getStatePath('team', '/repo'), '/repo/.omg/state/team-state.json');
  });

  it('builds session state paths', () => {
    assert.equal(getStateDir('/repo', 'sess1'), '/repo/.omg/state/sessions/sess1');
    assert.equal(
      getStatePath('ralph', '/repo', 'sess1'),
      '/repo/.omg/state/sessions/sess1/ralph-state.json'
    );
    assert.equal(
      getStateFilePath('hud-state.json', '/repo', 'sess1'),
      '/repo/.omg/state/sessions/sess1/hud-state.json'
    );
  });

...
const OPENCLAW_LIFECYCLE_BRIDGE = [
	{ omgEvent: "session-start", openclawEvent: "session-start" },
	{ omgEvent: "session-end", openclawEvent: "session-end" },
	{ omgEvent: "session-idle", openclawEvent: "session-idle" },
	{ omgEvent: "ask-user-question", openclawEvent: "ask-user-question" },
	{ omgEvent: "session-stop", openclawEvent: "stop" },
] as const;
	...
	return {
		...
		nextSteps:
			blockedHooks.length > 0
				...
				: [
						"Run omg adapt openclaw init --write to materialize adapter-owned OpenClaw artifacts.",
...
export function buildOpenClawStatusReport(
	...
	return {
		...
		phase: "foundation",
		summary: initialized
			? `OpenClaw adapter artifacts exist under .omg/adapters/openclaw/... and local runtime evidence is ${openclaw.observedState}.`
		...
		adapter: {
			...
			detail: initialized
				? "Adapter-owned OpenClaw artifacts are present under .omg/adapters/openclaw/..."
...
export function buildOpenClawDoctorReport(
	...
	if (!existsSync(paths.configPath) || !existsSync(paths.envelopePath)) {
		issues.push({
			...
			message:
				"No OpenClaw adapter artifacts exist under .omg/adapters/openclaw.",
		});
	}
...
export function initOpenClawFoundation(
	...
	return {
		...
		write,
		summary: write
			? "OpenClaw adapter metadata was written under .omg/adapters/openclaw/..."

...
export function buildAdaptProbeReport(
	...
	return {
		...
		nextSteps: [
			`Run omg adapt ${target} init --write to materialize OMX-owned adapter artifacts.`,
...
export function buildAdaptStatusReport(
	...
	return {
		...
		adapter: {
			...
			detail: initialized
				? "Adapter foundation artifacts exist under .omg/adapters/<target>/..."
...
export function buildAdaptDoctorReport(
	...
	if (status.adapter.state === "not-initialized") {
		issues.push({
			...
			message: `No adapter foundation artifacts exist for ${target} under ${join(".omg", "adapters", target)}.`,
		});
	}
	...
	return {
		...
		issues,
		nextSteps: [
			`Run omg adapt ${target} init --write.`,
			"Keep follow-on integration work out of .omg/state/... and target runtime internals unless a reviewed contract exists.",
			descriptor.followupHint,
		],
	};
}

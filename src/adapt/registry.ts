...
const FOUNDATION_CAPABILITIES: AdaptCapabilityReport[] = [
	capability(
		...
		"omg-owned",
		"ready",
		"Adapter artifacts stay under .omg/adapters/<target>/... rather than .omg/state or target internals.",
	),
	capability(
...
const TARGET_DESCRIPTORS: Record<AdaptTarget, AdaptTargetDescriptor> = {
	...
	hermes: {
		...
		followupHint:
			"Hermes adapter reads external ACP, gateway, and session-store evidence while keeping all writes under .omg/adapters/hermes/.",
		capabilities: [
			...FOUNDATION_CAPABILITIES,
			capability(

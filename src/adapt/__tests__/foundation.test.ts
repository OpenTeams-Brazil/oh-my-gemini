...
async function writeOpenClawOmxConfig(config: unknown): Promise<void> {
	const configDir = join(tempDir, ".gemini");
...
describe("adapt foundation", () => {
	it("resolves OMX-owned adapter paths under .omg/adapters/<target>", () => {
		const paths = resolveAdaptPaths(tempDir, "openclaw");
		assert.equal(
			paths.adapterRoot,
			join(tempDir, ".omg", "adapters", "openclaw"),
		);
		assert.equal(
			paths.configPath,
			join(tempDir, ".omg", "adapters", "openclaw", "adapter.json"),
		);
		assert.equal(
			paths.envelopePath,
			join(tempDir, ".omg", "adapters", "openclaw", "envelope.json"),
		);
		assert.equal(
			paths.probeReportPath,
			join(tempDir, ".omg", "adapters", "openclaw", "reports", "probe.json"),
		);
		assert.equal(
			paths.statusReportPath,
			join(tempDir, ".omg", "adapters", "openclaw", "reports", "status.json"),
		);
	});
	...
	it("keeps OpenClaw init preview read-only until --write is used", () => {
		...
		assert.equal(existsSync(join(tempDir, ".omg", "state")), false);
	});

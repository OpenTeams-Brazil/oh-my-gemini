/**
 * Native agent config generators for Gemini CLI.
 * Writes standalone TOML files under ~/.gemini/agents/ or ./.gemini/agents/.
 */
...
import { geminiAgentsDir } from "../utils/paths.js";
...
function readConfigTomlContent(
  ...
  const configPath = join(geminiHomeOverride ?? process.env.GEMINI_HOME ?? "", "config.toml");
  if (geminiHomeOverride && existsSync(configPath)) {
...
function resolveFrontierModel(options: AgentModelResolutionOptions): string {
  const configTomlContent = readConfigTomlContent(
    options.geminiHomeOverride,
    options.configTomlContent,
  );
  return getRootModelName(configTomlContent)
    ?? getMainDefaultModel(options.geminiHomeOverride);
}
...
function resolveStandardModel(options: AgentModelResolutionOptions): string {
  ...
  return getStandardDefaultModel(options.geminiHomeOverride);
}
...
function resolveAgentModel(
  ...
  switch (agent.modelClass) {
    case "frontier":
      return resolveFrontierModel(options);
    case "fast":
      return getSparkDefaultModel(options.geminiHomeOverride);
    case "standard":
    default:
...
export function generateStandaloneAgentToml(
  ...
  const lines = [
    `# oh-my-gemini agent: ${config.name}`,
...
/**
 * Install prompt-backed native agent config .toml files to ~/.gemini/agents/
...
export async function installNativeAgentConfigs(
  ...
  const {
    ...
    dryRun = false,
    verbose = false,
    agentsDir = geminiAgentsDir(),
  } = options;
  const geminiHomeOverride = join(agentsDir, "..");
  ...
  for (const [name, agent] of Object.entries(AGENT_DEFINITIONS)) {
    ...
    const toml = generateAgentToml(agent, promptContent, { geminiHomeOverride });

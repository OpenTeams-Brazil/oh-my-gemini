...
describe("agents/native-config", () => {
  it("generates TOML with stripped frontmatter and escaped triple quotes", () => {
    ...
    assert.match(toml, /# oh-my-gemini agent: executor/);
  ...
  it("installs only agents with prompt files and skips existing files without force", async () => {
    const root = await mkdtemp(join(tmpdir(), "omg-native-config-"));
  ...
  it("keeps standard agents off a custom gpt-5.2 root model", async () => {
    const root = await mkdtemp(join(tmpdir(), "omg-native-config-root-model-"));
    const geminiHome = join(root, ".gemini");
    const promptsDir = join(root, "prompts");
    const outDir = join(geminiHome, "agents");
    ...
    try {
      ...
      await writeFile(join(geminiHome, "config.toml"), 'model = "gpt-5.2"\n');
  ...
  it("keeps executor on the frontier lane so an explicit gpt-5.2 root model still applies there", async () => {
    const root = await mkdtemp(join(tmpdir(), "omg-native-config-executor-model-"));
    const geminiHome = join(root, ".gemini");
    const promptsDir = join(root, "prompts");
    const outDir = join(geminiHome, "agents");
    const previousGeminiHome = process.env.GEMINI_HOME;

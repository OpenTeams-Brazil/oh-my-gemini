 ...
 *
 * SECURITY: Command gateway requires OMG_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > OMG_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.OMG_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires OMG_OPENCLAW_COMMAND=1 opt-in (separate gate from OMG_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or OMG_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.OMG_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set OMG_OPENCLAW_COMMAND=1 to enable)",
    };
  }

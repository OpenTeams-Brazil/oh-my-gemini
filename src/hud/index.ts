 ...
 *
 * Usage:
 *   omg hud              Show current HUD state
 *   omg hud --watch      Poll every 1s with terminal clear
 *   omg hud --json       Output raw state as JSON
 *   omg hud --preset=X   Use preset: minimal, focused, full
 *   omg hud --tmux       Open HUD in a tmux split pane (auto-detects orientation)
 */
 ...
 * passed as a literal argument to tmux (no shell expansion).  `omgBin` is
...
export function buildTmuxSplitArgs(
  ...
  const sessionPrefix = safeSessionId ? `OMG_SESSION_ID=${shellEscape(safeSessionId)} ` : '';
  const cmd = `${sessionPrefix}node ${shellEscape(omgBin)} hud --watch${presetArg}`;
...
async function launchTmuxPane(cwd: string, flags: HudFlags): Promise<void> {
  ...
  if (!process.env.TMUX) {
    console.error('Not inside a tmux session. Start tmux first, then run: omg hud --tmux');
    process.exit(1);
  }

  const omgBin = resolveOmxCliEntryPath();
  if (!omgBin) {
    console.error('Failed to resolve OMX launcher path for tmux HUD startup.');
    process.exit(1);
  }
  const args = buildTmuxSplitArgs(cwd, omgBin, flags.preset, process.env.OMG_SESSION_ID);

  try {
    // Split bottom pane, 4 lines tall, running omg hud --watch.
    // execFileSync bypasses the shell – cwd and omgBin cannot inject commands.

...
describe('runHudAuthorityTick', () => {
  it('writes a live HUD authority owner lease before ticking', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-hud-authority-'));
    try {
      ...
      const lease = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'notify-fallback-authority-owner.json'), 'utf-8'));

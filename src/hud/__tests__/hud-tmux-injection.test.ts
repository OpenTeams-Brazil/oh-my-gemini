...
describe('VULNERABILITY – old string-interpolation approach', () => {
   ...
   *   const cmd  = `node ${omgBin} hud --watch${presetArg}`;
   *   execSync(`tmux split-window -v -l 4 -c "${cwd}" '${cmd}'`);
   */
  function buildOldCommand(cwd: string, omgBin: string, preset?: string): string {
  ...
  it('cwd containing $() is injectable via double-quote context', () => {
    ...
    const shellCmd = buildOldCommand(maliciousCwd, '/usr/bin/omg.js');
  ...
  it("omgBin containing single quote breaks out of the '-quoted command", () => {
    const maliciousOmx = "/tmp/it';touch /tmp/pwned;echo '/omg.js";
    ...
    assert.ok(
      shellCmd.includes("'node /tmp/it';touch /tmp/pwned;echo '/omg.js"),
...
describe('buildTmuxSplitArgs – shell injection hardening', () => {
  it('produces correct argv for normal inputs', () => {
    const args = buildTmuxSplitArgs('/home/user/project', '/usr/local/bin/omg.js');
    assert.deepEqual(args, [
      'split-window', '-v', '-l', String(HUD_TMUX_HEIGHT_LINES),
      // split height should come from shared HUD constants
      '-c', '/home/user/project',
      "node '/usr/local/bin/omg.js' hud --watch",
    ]);
  });

  it('cwd is an isolated array element – never shell-interpreted', () => {
    const maliciousCwd = '/tmp/foo"$(touch /tmp/pwned)"bar';
    const args = buildTmuxSplitArgs(maliciousCwd, '/usr/bin/omg.js');
  ...
  it("omgBin with single quote is properly escaped in command string", () => {
    const maliciousOmx = "/tmp/it's/omg.js";
    ...
    assert.equal(cmd, "node '/tmp/it'\\''s/omg.js' hud --watch");
  });

  it('omgBin with $() is neutralised by single-quote wrapping', () => {
    const maliciousOmx = '/tmp/$(id)/omg.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];

    // Inside single quotes, $() is literal.
    assert.equal(cmd, "node '/tmp/$(id)/omg.js' hud --watch");
  });

  it('omgBin with backticks is neutralised by single-quote wrapping', () => {
    const maliciousOmx = '/tmp/`whoami`/omg.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];

    assert.equal(cmd, "node '/tmp/`whoami`/omg.js' hud --watch");
  });

  it("omgBin with ';command' breakout attempt is neutralised", () => {
    const maliciousOmx = "/tmp/x';touch /tmp/pwned;echo '/omg.js";
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];
    ...
    //
    // Raw expected value: node '/tmp/x'\'';touch /tmp/pwned;echo '\''/omg.js' hud --watch
    assert.equal(
      cmd,
      "node '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/omg.js' hud --watch",
    );
  ...
  it('preset is appended safely', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omg.js', 'minimal');
    const cmd = args[6];
    assert.ok(cmd.endsWith('--preset=minimal'));
  });

  it('absent preset produces no --preset flag', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omg.js');
    const cmd = args[6];
  ...
  it('invalid preset is dropped (defense in depth)', () => {
    ...
    const cmd = args[6];
    assert.equal(cmd, "node '/usr/bin/omg.js' hud --watch");
    assert.ok(!cmd.includes('--preset='));
  });

  it('prepends OMG_SESSION_ID when provided', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omg.js', 'focused', 'sess-managed');
    const cmd = args[6];
    assert.equal(cmd, "OMG_SESSION_ID='sess-managed' node '/usr/bin/omg.js' hud --watch --preset=focused");
  });
});

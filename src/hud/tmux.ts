...
export function buildHudWatchCommand(omgBin: string, preset?: string, sessionId?: string): string {
  const safePreset = preset === 'minimal' || preset === 'focused' || preset === 'full'
    ? ` --preset=${preset}`
    : '';
  const safeSessionId = typeof sessionId === 'string' ? sessionId.trim() : '';
  const sessionPrefix = safeSessionId ? `OMG_SESSION_ID=${shellEscapeSingle(safeSessionId)} ` : '';
  return `${sessionPrefix}node ${shellEscapeSingle(omgBin)} hud --watch${safePreset}`;
}

 ...
 *
 * Reads .omg/state/ files to build HUD render context.
 */
...
export async function readMetrics(cwd: string): Promise<HudMetrics | null> {
  return readJsonFile<HudMetrics>(join(cwd, '.omg', 'metrics.json'));
}
...
export async function readHudConfig(cwd: string): Promise<ResolvedHudConfig> {
  const config = await readJsonFile<HudConfig>(join(cwd, '.omg', 'hud-config.json'));
 ...
 *
 * See: https://github.com/Yeachan-Heo/oh-my-gemini/issues/1100
 */

...
function buildTmuxSessionName(cwd: any, sessionId: any): string {
  ...
  const dirToken = parentDir.endsWith('.omg-worktrees')
    ? sanitizeTmuxToken(`${parentDir.slice(0, -'.omg-worktrees'.length)}-${dirName}`)
  ...
  const sessionToken = sanitizeTmuxToken(safeString(sessionId).replace(/^omg-/, ''));

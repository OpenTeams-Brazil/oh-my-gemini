 ...
 * owned path.
 * Disable entirely with OMG_RUNTIME_BRIDGE=0.
 */
function runtimeExec(command, stateDir) {
  if (process.env.OMG_RUNTIME_BRIDGE === '0') return;
  try {
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'OMG_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'OMG_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
async function injectDispatchRequest(request, config, cwd, stateDir) {
  ...
  for (let round = 0; round < INJECT_VERIFY_ROUNDS; round++) {
    ...
    try {
      ...
      // This avoids false confirmations when Gemini leaves the unsent draft just
...
export async function drainPendingTeamDispatch({
  ...
  if (safeString(process.env.OMG_TEAM_WORKER)) {

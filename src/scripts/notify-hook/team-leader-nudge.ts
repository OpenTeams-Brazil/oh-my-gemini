...
function buildStatusCheckReminder(teamName) {
  return `Next: check messages; keep orchestrating; if done, gracefully shut down: omg team shutdown ${teamName}.`;
}

function buildMailboxCheckReminder(teamName) {
  return `Next: read messages; keep orchestrating; if done, gracefully shut down: omg team shutdown ${teamName}.`;
}

function buildWorkerStartEvidenceReminder(teamName, workerName) {
  return `Next: check ${workerName} msg/output, confirm task in omg team status ${teamName}, then reassign/nudge.`;
}
...
function buildLeaderActionGuidance(teamName, {
  ...
  if (leaderActionState === 'done_waiting_on_leader') {
    return `Next: decide whether to reconcile/merge results or gracefully shut down: omg team shutdown ${teamName}.`;
  }
  if (leaderActionState === 'stuck_waiting_on_leader') {
    return `Next: omg team status ${teamName}; read worker messages; unblock/reassign or shutdown.`;
  }
...
function normalizeVisibleLeaderStateText(text) {
  return safeString(text)
    .toLowerCase()
    .replace(/\[omg_tmux_inject\]/g, ' ')
    .replace(/\[omg_intent:[^\]]+\]/g, ' ')
...
export async function emitTeamNudgeEvent(cwd, teamName, reason, orchestrationIntent, nowIso) {
  const eventsDir = join(cwd, '.omg', 'state', 'team', teamName, 'events');
...
export async function maybeNudgeTeamLeader({
  ...
  for (const teamName of candidateTeamNames) {
    ...
    let workers = [];
    try {
      const manifestPath = join(omgDir, 'state', 'team', teamName, 'manifest.v2.json');
      const configPath = join(omgDir, 'state', 'team', teamName, 'config.json');
    ...
    let mailbox = null;
    try {
      const mailboxPath = join(omgDir, 'state', 'team', teamName, 'mailbox', 'leader-fixed.json');
    ...
    const paneGuard = await evaluatePaneInjectionReadiness(tmuxTarget, {
      ...
      // Leader nudges should still queue into a live Gemini pane even while the

// @ts-nocheck
/**
 * Auto-nudge: detect Gemini "asking for permission" stall patterns and
...
export async function loadAutoNudgeConfig() {
  const geminiHomePath = process.env.GEMINI_HOME || join(homedir(), '.gemini');
  const configPath = join(geminiHomePath, '.omg-config.json');
...
async function localTmuxInjectionDisabled(cwd) {
  ...
  const raw = await readJsonIfExists(join(normalizedCwd, '.omg', 'tmux-hook.json'), null);

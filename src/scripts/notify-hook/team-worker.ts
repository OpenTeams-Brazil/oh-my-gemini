...
export async function resolveTeamStateDirForWorker(cwd, parsedTeamWorker) {
  ...
  if (leaderCwd) {
    candidateStateDirs.push(join(resolvePath(leaderCwd), '.omg', 'state'));
  }
  candidateStateDirs.push(join(cwd, '.omg', 'state'));
...
async function checkLeaderPaneReadyForWorkerStateReminder(paneTarget) {
  return evaluatePaneInjectionReadiness(paneTarget, {
    ...
    // queue into a live Gemini pane even while the leader is busy or not
...
export async function maybeNotifyLeaderAllWorkersIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const nextAction = `Run \`omg team status ${teamName}\` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down.`;

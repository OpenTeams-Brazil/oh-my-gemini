...
const binaryName = process.platform === 'win32' ? 'omg-explore-harness.exe' : 'omg-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'omg-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'omg-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',

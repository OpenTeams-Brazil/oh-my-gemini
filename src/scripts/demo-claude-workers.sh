...
cleanup() {
  if ((TEAM_STARTED == 1)); then
    ...
    omg team shutdown "$TEAM_NAME" >/dev/null 2>&1 || true
    echo "[cleanup] cleaning state for team: $TEAM_NAME"
    omg team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  CREATE_JSON="$(omg team api create-task --input "$CREATE_INPUT" --json)"
...
omg team api list-tasks --input "$LIST_INPUT" --json | jq -r '.data.tasks[] | "  Task \(.id): \(.subject) [\(.status)]"'
echo ""
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_ID="$(omg team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_ID" ]]; then
    ...
    CLAIM_JSON="$(omg team api claim-task --input "$CLAIM_INPUT" --json)"
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_INFO="$(omg team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_INFO" ]]; then
    ...
    omg team api transition-task-status --input "$TRANSITION_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  omg team api send-message --input "$SEND_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  MAILBOX_JSON="$(omg team api mailbox-list --input "$MAILBOX_INPUT" --json)"
...
BROADCAST_RESULT="$(omg team api broadcast --input "$BROADCAST_INPUT" --json)"
...
SUMMARY_JSON="$(omg team api get-summary --input "$SUMMARY_INPUT" --json)"
...
omg team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null
TEAM_STARTED=0
echo ""

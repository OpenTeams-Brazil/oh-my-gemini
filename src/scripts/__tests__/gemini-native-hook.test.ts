...
async function writeReleaseReadinessStateMarker(
  ...
  await writeJson(
    join(cwd, ".omg", "state", "sessions", sessionId, "release-readiness-state.json"),
    {
      active: true,
...
describe("codex native hook dispatch", () => {
  ...
  it("maps Gemini events onto OMX logical surfaces", () => {
    assert.equal(mapGeminiHookEventToOmxEvent("SessionStart"), "session-start");
    assert.equal(mapGeminiHookEventToOmxEvent("UserPromptSubmit"), "keyword-detector");
    assert.equal(mapGeminiHookEventToOmxEvent("PreToolUse"), "pre-tool-use");
    assert.equal(mapGeminiHookEventToOmxEvent("PostToolUse"), "post-tool-use");
    assert.equal(mapGeminiHookEventToOmxEvent("Stop"), "stop");
  });

  it("writes SessionStart state against the long-lived session owner pid and stays quiet for clean sessions", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-session-start-"));
    try {
      ...
      const sessionState = JSON.parse(
        await readFile(join(cwd, ".omg", "state", "session.json"), "utf-8"),
  ...
  it("preserves canonical OMX session scope when native SessionStart arrives with a different id", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-session-reconcile-"));
    try {
      ...
      const promptResult = await dispatchGeminiNativeHook(
        {
      ...
      assert.equal(promptResult.omgEventName, "keyword-detector");
  ...
  it("appends .omg/ to repo-root .gitignore during SessionStart when missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-session-gitignore-"));
    try {
  ...
  it("includes persisted project-memory summary in SessionStart context", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-session-memory-"));
    try {
      await writeJson(join(cwd, ".omg", "project-memory.json"), {
  ...
  it("records keyword activation from UserPromptSubmit payloads", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-"));
    try {
      ...
      assert.equal(result.omgEventName, "keyword-detector");
      assert.equal(result.skillState?.skill, "ralplan");
      assert.ok(result.outputJson, "UserPromptSubmit should emit developer context");
      assert.match(JSON.stringify(result.outputJson), /skill: ralplan activated and initial state initialized at \.omg\/state\/sessions\/sess-1\/ralplan-state\.json; write subsequent updates via omg_state MCP\./);

      const statePath = join(cwd, ".omg", "state", "skill-active-state.json");
      ...
      assert.equal(existsSync(join(cwd, ".omg", "state", "sessions", "sess-1", "ralplan-state.json")), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("clarifies that prompt-side $ralph activation does not invoke the PRD-gated CLI path", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-ralph-routing-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      const result = await dispatchGeminiNativeHook(
        {
      ...
      assert.match(message, /skill: ralph activated and initial state initialized at \.omg\/state\/sessions\/sess-ralph-msg\/ralph-state\.json; write subsequent updates via omg_state MCP\./);
      assert.match(message, /Prompt-side `\$ralph` activation seeds Ralph workflow state only; it does not invoke `omg ralph`\./);
      assert.match(message, /Use `omg ralph --prd \.\.\.` only when you explicitly want the PRD-gated CLI startup path\./);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("ignores generic wrapper fields so metadata cannot trigger workflow routing or Stop blocking", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-wrapper-metadata-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      const promptResult = await dispatchGeminiNativeHook(
        {
      ...
      assert.equal(existsSync(join(cwd, ".omg", "state", "skill-active-state.json")), false);

      const stopResult = await dispatchGeminiNativeHook(
        {
      ...
      assert.equal(stopResult.omgEventName, "stop");
  ...
  it("does not expose submitted prompt text to keyword-detector hook plugins", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-prompt-sanitized-"));
    try {
      await mkdir(join(cwd, ".omg", "hooks"), { recursive: true });
      await writeFile(
        join(cwd, ".omg", "hooks", "capture-keyword-context.mjs"),
        `import { mkdir, writeFile } from "node:fs/promises";
import { dirname, join } from "node:path";

export async function onHookEvent(event) {
  if (event.event !== "keyword-detector") return;
  const outPath = join(process.cwd(), ".omg", "captured-keyword-context.json");
...
`,
      ...
      const captured = JSON.parse(
        await readFile(join(cwd, ".omg", "captured-keyword-context.json"), "utf-8"),
  ...
  it("does not emit UserPromptSubmit routing context for unknown $tokens", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-unknown-token-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      const result = await dispatchGeminiNativeHook(
        {
  ...
  it("nudges $team prompt-submit routing toward omg team runtime usage", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-team-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      const result = await dispatchGeminiNativeHook(
        {
      ...
      assert.match(
        JSON.stringify(result.outputJson),
        /skill: team activated and initial state initialized at \.omg\/state\/team-state\.json; write subsequent updates via omg_state MCP\./,
      );
      assert.match(JSON.stringify(result.outputJson), /Use the durable OMX team runtime via `omg team \.\.\.`/);
      assert.match(JSON.stringify(result.outputJson), /If you need runtime syntax, run `omg team --help` yourself\./);

      const state = JSON.parse(
        await readFile(join(cwd, ".omg", "state", "team-state.json"), "utf-8"),
  ...
  it("returns actionable denial guidance for unsupported workflow overlaps on prompt submit", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-transition-deny-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      await dispatchGeminiNativeHook(
        {
      ...
      const denied = await dispatchGeminiNativeHook(
        {
          hook_event_name: "UserPromptSubmit",
          cwd,
          session_id: "sess-deny-1",
      ...
      assert.match(JSON.stringify(denied.outputJson), /`omg state clear --mode <mode>`/);
      assert.match(JSON.stringify(denied.outputJson), /`omg_state\.\*` MCP tools/);
      assert.equal(
        existsSync(join(cwd, ".omg", "state", "sessions", "sess-deny-1", "autopilot-state.json")),
        false,
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
  ...
  it("surfaces transition success output for allowlisted prompt-submit handoffs", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-transition-success-"));
    try {
      const sessionDir = join(cwd, ".omg", "state", "sessions", "sess-handoff-1");
  ...
  it("keeps the planning skill active when planning and execution workflows are invoked together", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-planning-precedence-"));
    try {
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      ...
      assert.match(message, /skill: ralplan activated and initial state initialized at \.omg\/state\/sessions\/sess-multi-1\/ralplan-state\.json; write subsequent updates via omg_state MCP\./);
      assert.doesNotMatch(message, /Use the durable OMX team runtime via `omg team \.\.\.`/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("runs prompt-submit HUD reconciliation as a best-effort tmux-only side effect", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-hud-reconcile-"));
    ...
    try {
      ...
      await mkdir(join(cwd, ".omg", "state"), { recursive: true });
      await writeFile(
        join(cwd, ".omg", "hud-config.json"),
        JSON.stringify({ preset: "focused", git: { display: "branch" } }, null, 2),
      );

      const binDir = await mkdtemp(join(tmpdir(), "omg-native-hook-hud-reconcile-bin-"));
...
`,
  ...
  it("returns a destructive-command caution on PreToolUse for rm -rf dist", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-danger-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
      ...
      assert.equal(result.omgEventName, "pre-tool-use");
  ...
  it("stays silent on PreToolUse for neutral pwd", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-neutral-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
      ...
      assert.deepEqual(result.outputJson, {
        ...
        hookSpecificOutput: {
          ...
          additionalContext: [
            ...
            "- Add the required co-author trailer: `Co-authored-by: OmX <omg@oh-my-gemini.dev>`.",
          ].join("\n"),
        },
        systemMessage: [
          "git commit is blocked until the inline commit message follows the Lore protocol and includes `Co-authored-by: OmX <omg@oh-my-gemini.dev>`.",
          "- Add a blank line after the subject before the narrative body.",
          "- Add a narrative body paragraph explaining the decision context.",
          "- Add at least one Lore trailer such as `Constraint:`, `Confidence:`, or `Tested:`.",
          "- Add the required co-author trailer: `Co-authored-by: OmX <omg@oh-my-gemini.dev>`.",
  ...
  it("stays silent on PreToolUse for `git help commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-help-commit-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for `git config alias.ci commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-config-alias-commit-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for `git tag commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-tag-commit-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env-prefixed git commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-env-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when git options appear before the real commit subcommand", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-option-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-env-wrapper-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env flag wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-env-flag-wrapper-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env value-taking wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-env-value-wrapper-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse path-qualified Windows git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-windows-path-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse quoted backslash Windows git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-exe-commit-windows-backslash-path-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse path-qualified git commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-path-invalid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when the message comes from an external source", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-file-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when Lore trailers exist but the OmX co-author trailer is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-missing-omg-coauthor-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
          cwd,
          tool_name: "Bash",
          tool_use_id: "tool-git-commit-missing-omg-coauthor",
          tool_input: {
            command: [
              'git commit',
  ...
  it("stays silent on PreToolUse for Lore-compliant git commit with OmX co-author trailer", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-pretool-git-commit-valid-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("returns PostToolUse remediation guidance for command-not-found output", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-failure-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
      ...
      assert.equal(result.omgEventName, "post-tool-use");
      assert.deepEqual(result.outputJson, {
        decision: "block",
  ...
  it("returns PostToolUse MCP transport fallback guidance for clear MCP transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-mcp-transport-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PostToolUse",
          cwd,
          tool_name: "mcp__omg_state__state_write",
  ...
  it("does not classify non-transport MCP failures as transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-mcp-nontransport-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("marks active team state failed on MCP transport death without deleting team state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-team-mcp-transport-"));
    ...
    try {
      ...
      assert.equal(existsSync(join(cwd, ".omg", "state", "team", "transport-team")), true);
    } finally {
  ...
  it("treats stderr-only informative non-zero output as reviewable instead of a generic failure", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-informative-stderr-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("treats non-zero gh pr checks style output as informative instead of a generic failure", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-informative-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("returns MCP transport-death guidance and preserves failed team state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-mcp-dead-"));
    try {
      await initTeamState(
      ...
      assert.match(
        ...
        /Retry via CLI parity with `omg state state_write --input '\{\}' --json`\./,
      );
      assert.match(
        String(hookSpecificOutput?.additionalContext || ""),
        /omg team api read-stall-state/,
      );

      const phase = JSON.parse(
        await readFile(join(cwd, ".omg", "state", "team", "mcp-transport-dead-team", "phase.json"), "utf-8"),
      ...
      const attention = JSON.parse(
        await readFile(join(cwd, ".omg", "state", "team", "mcp-transport-dead-team", "leader-attention.json"), "utf-8"),
  ...
  it("stays silent on neutral successful PostToolUse output", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-posttool-neutral-"));
    try {
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("returns Stop continuation output while Autopilot is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-autopilot-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      ...
      assert.equal(result.omgEventName, "stop");
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
  ...
  it("returns Stop continuation output while Ultrawork is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-ultrawork-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("returns Stop continuation output while UltraQA is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-ultraqa-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("returns Stop continuation output while team phase is non-terminal", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("blocks Stop for a team worker with a non-terminal assigned task via native worker context", async () => {
    ...
    try {
      ...
      await writeJson(join(cwd, ".omg", "state", "team", "worker-stop-team", "tasks", "task-1.json"), {
        id: "1",
        subject: "hook task",
      ...
      const result = await dispatchGeminiNativeHook(
        {
          hook_event_name: "Stop",
          cwd: join(cwd, ".omg", "team", "worker-stop-team", "worktrees", "worker-1"),
  ...
  it("does not block Stop for a team worker when assigned task is terminal", async () => {
    ...
    try {
      ...
      await writeJson(join(cwd, ".omg", "state", "team", "worker-stop-team-terminal", "tasks", "task-1.json"), {
        id: "1",
        subject: "hook task",
        description: "finish hook task",
        status: "completed",
  ...
  it("returns Stop continuation output from canonical team state when coarse mode state is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-canonical-"));
    try {
      await initTeamState(
        "canonical-team",
  ...
  it("emits one concise final decision summary and auto-finalize guidance when release-readiness already has a stable final recommendation and no active worker tasks", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-release-readiness-finalize-"));
    try {
      await initTeamState(
        "release-ready-team",
  ...
  it("does not auto-finalize non-release team stops that happen to contain a stable recommendation summary", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-non-release-readiness-control-"));
    try {
      await initTeamState(
        "general-review-team",
  ...
  it("re-fires canonical-team Stop output for a later fresh Stop reply when coarse mode state is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-canonical-refire-"));
    try {
      await initTeamState(
  ...
  it("does not block Stop from canonical team state alone when the canonical phase is terminal", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-terminal-"));
    try {
      await initTeamState(
        "terminal-team",
  ...
  it("returns Stop continuation output from canonical team state when manifest session ownership is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-legacy-"));
    try {
      await initTeamState(
        "legacy-team",
  ...
  it("blocks Stop from session-scoped team mode when session.json points to another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-session-mismatch-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("returns Stop continuation output for active ralplan skill with matching active mode state and without active subagents", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-skill-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block on stale ralplan skill-active state when the matching mode state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-stale-skill-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block on active ralplan skill when subagents are still active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-skill-subagent-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block on stale root ralplan skill when the explicit session-scoped canonical skill state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-stale-root-skill-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not block Stop solely because deep-interview is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-deep-interview-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("ignores root skill-active fallback from a different thread when evaluating Stop", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-foreign-thread-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "skill-active-state.json"), {
  ...
  it("returns Stop continuation output while Ralph is active without an explicit session pin", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
      await writeFile(
  ...
  it("blocks Stop from session-scoped Ralph state when session.json points to another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-ralph-session-mismatch-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block Stop from stale session-scoped Ralph state that belongs to another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-stale-session-ralph-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block Stop from another session-scoped Ralph state when an explicit session_id has no active Ralph state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-explicit-session-ralph-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not block Stop from root Ralph fallback when the current session has no scoped Ralph state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-root-fallback-ralph-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
  ...
  it("does not block Stop when the current session Ralph state is cancelled even if stale root fallback remains", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-cancelled-session-ralph-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
      await writeJson(join(stateDir, "session.json"), { session_id: "sess-current", cwd });
  ...
  it("does not block Stop from root Ralph fallback when an explicit session_id is present and session.json points to another worktree", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-root-fallback-cwd-mismatch-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not re-block Ralph when Stop already continued once", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-ralph-once-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
      await writeFile(
  ...
  it("returns Stop continuation output for native auto-nudge stall prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("suppresses duplicate native auto-nudge replays for the same Stop reply", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-once-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("suppresses duplicate native auto-nudge replays across native/canonical session-id drift", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-session-drift-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
      ...
      assert.deepEqual(Object.keys(persisted.sessions ?? {}), ["omg-canonical"]);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("re-fires native auto-nudge for a later fresh Stop reply even when stop_hook_active is true", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-refire-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not auto-continue native Stop while deep-interview is waiting on an intent-first question", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-deep-interview-question-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("suppresses native auto-nudge re-fire while session-scoped deep-interview state is still active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-deep-interview-state-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("suppresses native auto-nudge when root deep-interview mode state is active without an explicit session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-deep-interview-mode-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not suppress native auto-nudge from stale root deep-interview mode state when the explicit session-scoped mode state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-stale-root-mode-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not suppress native auto-nudge from stale root deep-interview skill state when the explicit session-scoped canonical skill state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-stale-root-skill-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not suppress native auto-nudge from stale root deep-interview input lock when the explicit session-scoped canonical skill state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-stale-root-lock-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
  ...
  it("does not suppress native auto-nudge from active root deep-interview state when the current scoped mode state is explicitly inactive", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-inactive-scoped-mode-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
  ...
  it("does not auto-continue native Stop for a plain Gemini session started outside OMX runtime", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-auto-nudge-plain-session-"));
    try {
      await dispatchGeminiNativeHook(
        {
          hook_event_name: "SessionStart",
  ...
  it("re-fires team Stop output for a later fresh Stop reply while the team is still active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-refire-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "team-state.json"), {
  ...
  it("suppresses duplicate team Stop replays across native/canonical session-id drift", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-team-session-drift-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      ...
      const duplicate = await dispatchGeminiNativeHook(
        {
          hook_event_name: "Stop",
          cwd,
          session_id: "omg-canonical",
      ...
      assert.equal(duplicate.omgEventName, "stop");
      assert.equal(duplicate.outputJson, null);

      const fresh = await dispatchGeminiNativeHook(
        {
          hook_event_name: "Stop",
          cwd,
          session_id: "omg-canonical",
      ...
      assert.equal(fresh.omgEventName, "stop");
  ...
  it("does not block Stop from another session's stale root team state when no scoped team state exists", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-stale-root-team-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
      await writeJson(join(stateDir, "session.json"), { session_id: "sess-current" });
  ...
  it("does not block Stop from orphaned team mode state after cleanup removed canonical team artifacts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-orphaned-team-state-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
      await writeJson(join(stateDir, "session.json"), { session_id: "sess-current" });
  ...
  it("prefers the current session team state over a stale root team fallback during Stop", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-current-session-team-preferred-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
      await writeJson(join(stateDir, "session.json"), { session_id: "sess-current" });
  ...
  it("does not fall back to active root team state when the current scoped team state is inactive", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-native-hook-stop-inactive-scoped-team-"));
    try {
      const stateDir = join(cwd, ".omg", "state");
      await mkdir(join(stateDir, "sessions", "sess-current"), { recursive: true });
      await writeJson(join(stateDir, "session.json"), { session_id: "sess-current" });

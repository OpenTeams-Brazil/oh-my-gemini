...
test('resolveReplyListenerLiveEnv stays disabled until explicitly opted in', () => {
  const result = resolveReplyListenerLiveEnv({
    OMG_DISCORD_NOTIFIER_BOT_TOKEN: 'discord-token',
    OMG_DISCORD_NOTIFIER_CHANNEL: 'channel-1',
    OMG_TELEGRAM_BOT_TOKEN: 'telegram-token',
    OMG_TELEGRAM_CHAT_ID: 'chat-1',
  });

...
function readCommand(payload: GeminiHookPayload): string {
...
function isMcpLikeToolName(toolName: string): boolean {
  return /^(mcp__|omg_(?:state|memory|trace|code_intel)\b|state_|project_memory_|notepad_|trace_)/i.test(toolName);
}
...
export function detectMcpTransportFailure(
  ...
  const mcpContextDetected = isMcpLikeToolName(normalized.toolName)
    ...
    || /\bomg-(?:state|memory|trace|code-intel)-server\b/i.test(combined);
...
function resolveOmxParityTarget(toolName: string): { command: OmxParityCommand; tool: string } | null {
  const match = toolName.match(/^mcp__omg_(state|memory|trace|code_intel)__([a-z0-9_]+)$/i);
...
function buildOmxParityFallbackCommand(payload: GeminiHookPayload, toolName: string): string | null {
  const target = resolveOmxParityTarget(toolName);
  if (!target) return null;
  const input = safeObject(payload.tool_input) ?? {};
  return `omg ${target.command} ${target.tool} --input ${shellSingleQuote(JSON.stringify(input))} --json`;
}

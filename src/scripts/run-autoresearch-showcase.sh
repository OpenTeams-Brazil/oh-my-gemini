...
Available showcases:
  omg-self       -> missions/in-action-cat-shellout-demo
  ...
  all            -> omg-self ml-tabular bayesopt latent sorting

Examples:
  scripts/run-autoresearch-showcase.sh --list
  scripts/run-autoresearch-showcase.sh bayesopt
  scripts/run-autoresearch-showcase.sh omg-self ml-tabular
USAGE
}

mission_for() {
  case "$1" in
    omg-self)   printf '%s' 'missions/in-action-cat-shellout-demo' ;;
...
args=("$@")
if [[ " ${args[*]} " == *" all "* ]]; then
  args=(omg-self ml-tabular bayesopt latent sorting)
fi

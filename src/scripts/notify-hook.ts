...
/**
 * oh-my-gemini Notification Hook
 * Gemini CLI fires this after each agent turn via the `notify` config.
...
async function main() {
  ...
  // Reconcile Ralph ownership for same-Gemini-session continuation before
  ...
  if (!isTeamWorker) {
    const metricsPath = join(omgDir, 'metrics.json');
    try {
      let metrics = {
        total_turns: 0,
        session_turns: 0,
  ...
  // 4. Write HUD state summary for `omg hud` (lead session only)
  if (!isTeamWorker) {
    try {
  ...
  try {
    ...
    const event = buildNativeHookEvent('turn-complete', {
      ...
      omg_session_id: sessionIdForHooks || null,
  ...
  // 9. Auto-nudge: detect Gemini stall patterns and automatically send a continuation prompt.

...
import { omgNotepadPath, omgProjectMemoryPath } from "../utils/paths.js";
...
type GeminiHookPayload = Record<string, unknown>;
...
export interface NativeHookDispatchResult {
  hookEventName: GeminiHookEventName | null;
  omgEventName: string | null;
...
function readHookEventName(payload: GeminiHookPayload): GeminiHookEventName | null {
...
export function mapGeminiHookEventToOmxEvent(
  hookEventName: GeminiHookEventName | null,
): string | null {
...
function readPromptText(payload: GeminiHookPayload): string {
  const candidates = [
    payload.prompt,
    payload.user_prompt,
    payload.userPrompt,
...
function resolveSessionOwnerPid(payload: GeminiHookPayload): number {
  const explicitPid = [
    payload.session_pid,
    payload.sessionPid,
    payload.gemini_pid,
...
async function buildSessionStartContext(
  ...
  const projectMemory = await readJsonIfExists(omgProjectMemoryPath(cwd));
  if (projectMemory) {
  ...
  if (existsSync(omgNotepadPath(cwd))) {
    try {
      const notepad = await readFile(omgNotepadPath(cwd), "utf-8");
...
function buildAdditionalContextMessage(prompt: string, skillState?: SkillActiveState | null): string | null {
  ...
  const ralphPromptActivationNote = skillState?.initialized_mode === "ralph"
    ? "Prompt-side `$ralph` activation seeds Ralph workflow state only; it does not invoke `omg ralph`. Use `omg ralph --prd ...` only when you explicitly want the PRD-gated CLI startup path."
    : null;
  ...
  if (skillState?.transition_error) {
    return [
      ...
      'Follow GEMINI.md routing and preserve workflow transition and planning-safety rules.',
    ].join(' ');
  }
  ...
  if (skillState?.transition_message) {
    return [
      ...
      skillState.initialized_mode && skillState.initialized_state_path
        ? `skill: ${skillState.initialized_mode} activated and initial state initialized at ${skillState.initialized_state_path}; write subsequent updates via omg_state MCP.`
        : null,
      teamDetected
        ? "Use the durable OMX team runtime via `omg team ...` for coordinated execution; do not replace it with in-process fanout."
        : null,
      teamDetected ? "If you need runtime syntax, run `omg team --help` yourself." : null,
      'Follow GEMINI.md routing and preserve workflow transition and planning-safety rules.',
    ].filter(Boolean).join(' ');
  }
  ...
  if (teamDetected) {
    ...
    return [
      ...
      "Use the durable OMX team runtime via `omg team ...` for coordinated execution; do not replace it with in-process fanout.",
      "If you need runtime syntax, run `omg team --help` yourself.",
      "Follow GEMINI.md routing and preserve workflow transition and planning-safety rules.",
    ].filter(Boolean).join(" ");
  }
  ...
  return `${detectedKeywordMessage} Follow GEMINI.md routing and preserve workflow transition and planning-safety rules.`;
}
...
async function readTeamModeStateForStop(
  ...
  const rootState = await readJsonIfExists(join(cwd, ".omg", "state", "team-state.json"));
...
function hasReleaseReadinessMode(payload: GeminiHookPayload): boolean {
...
function readPayloadSessionId(payload: GeminiHookPayload): string {
  return safeString(payload.session_id ?? payload.sessionId).trim();
}

function readPayloadThreadId(payload: GeminiHookPayload): string {
  return safeString(payload.thread_id ?? payload.threadId).trim();
}

function readPayloadTurnId(payload: GeminiHookPayload): string {
...
async function readStopAutoNudgePhase(
  ...
  if (normalizedSessionId) {
    ...
    const rootModeState = await readJsonIfExists(join(cwd, ".omg", "state", "deep-interview-state.json"));
    if (
...
export async function dispatchGeminiNativeHook(
  ...
  const omgEventName = mapGeminiHookEventToOmxEvent(hookEventName);
  ...
  if (omgEventName) {
    ...
    if (nativeSessionId) {
      ...
      baseContext.gemini_session_id = nativeSessionId;
    }
    if (canonicalSessionId) {
      baseContext.omg_session_id = canonicalSessionId;
    }
...
export async function runGeminiNativeHookCli(): Promise<void> {
  ...
  const result = await dispatchGeminiNativeHook(payload);

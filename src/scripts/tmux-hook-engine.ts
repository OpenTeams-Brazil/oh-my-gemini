 ...
 */
export function resolveGeminiPane(): string {
...
export function paneLooksReady(captured: any): boolean {
  ...
  const hasGeminiPromptLine = lines.some((line) => /^\s*›\s*/u.test(line));
  const hasClaudePromptLine = lines.some((line) => /^\s*❯\s*/u.test(line));
  if (hasGeminiPromptLine || hasClaudePromptLine) return true;

  const hasGeminiWelcomePrompt = lines.some((line) => /\bhow can i help(?: you)?\b/i.test(line));
  if (hasGeminiWelcomePrompt) return true;

  return lines.some((line) => /^\s*(?:[›>❯]\s*)?[A-Z][A-Z0-9]+-\d+\s+only(?:\s*(?:…|\.{3}))?\s*$/iu.test(line));
}

export function paneShowsGeminiViewport(captured: any): boolean {
  const lines = normalizePaneLines(captured);
  if (lines.length === 0) return false;
  if (paneIsBootstrapping(lines)) return false;

  const hasGeminiBanner = lines.some((line) => /\bOpenAI Gemini\b/i.test(line));
  if (!hasGeminiBanner) return false;
...
export function buildSendKeysArgv({ paneTarget, prompt, dryRun, submitKeyPresses = 2 }: any): any {
  ...
  return {
    ...
    // Gemini generally prefers two presses; Claude typically needs one.
    submitArgv,
  };
}

...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.omg', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.omg', 'state', 'ralph-progress.json');
      ...
      await writeFile(join(cwd, '.omg', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.omg', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.omg', 'prd.json');
      const legacyProgressPath = join(cwd, '.omg', 'progress.txt');
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.omg\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.omg/progress.txt');
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-visual-feedback-'));
    try {

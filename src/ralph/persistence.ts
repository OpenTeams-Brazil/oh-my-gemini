...
const LEGACY_PRD_PATH = '.omg/prd.json';
const LEGACY_PROGRESS_PATH = '.omg/progress.txt';
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.omg', 'plans', 'ralph-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.omg/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',

...
/**
 * Read the global OMX config from ~/.omg/config.json.
 * Returns null if the file does not exist or cannot be parsed.
 *
 * @param configDir - Optional override for the home directory. When provided,
 *   the config is read from `<configDir>/.omg/config.json` instead of
 *   `~/.omg/config.json`. Useful for testing without relying on `os.homedir()`.
 */
export function readOmxConfig(configDir?: string): OmxGlobalConfig | null {
  const configPath = join(configDir ?? homedir(), '.omg', 'config.json');

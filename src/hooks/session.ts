/**
 * Session Lifecycle Manager for oh-my-gemini
 *
...
import { omgStateDir, omgLogsDir, sameFilePath } from '../utils/paths.js';
...
function sessionPath(cwd: string): string {
  return join(omgStateDir(cwd), SESSION_FILE);
}

function historyPath(cwd: string): string {
  return join(omgLogsDir(cwd), HISTORY_FILE);
}
...
export async function resetSessionMetrics(cwd: string, sessionId?: string): Promise<void> {
  ...
  await writeFile(join(omgDir, 'metrics.json'), JSON.stringify({
    total_turns: 0,
    session_turns: 0,
    last_activity: now,
...
/**
 * Reconcile a native/Gemini SessionStart with the canonical OMX launch session.
...
export async function appendToLog(cwd: string, entry: Record<string, unknown>): Promise<void> {
  ...
  const logFile = join(logsDir, `omg-${date}.jsonl`);

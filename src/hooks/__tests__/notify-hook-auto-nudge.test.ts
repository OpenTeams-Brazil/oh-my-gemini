...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-auto-nudge-'));
  try {
    await run(cwd);
  } finally {
...
describe('notify-hook auto-nudge', () => {
  ...
  it('sends nudge when stall pattern detected in last-assistant-message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // Gemini CLI needs C-m sent twice with a delay for reliable submission
  ...
  it('respects `.omg/tmux-hook.json` enabled:false and skips auto-nudge injection', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
      ...
      await writeJson(join(omgDir, 'tmux-hook.json'), {
        enabled: false,
  ...
  it('does not auto-nudge plain tmux Gemini sessions that only inherit OMX session env', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
  ...
  it('does not auto-nudge plain tmux Gemini sessions that are not OMX-managed', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
...
`;
  ...
  it('deduplicates semantic proceed-style variants on the same turn', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, geminiHome, {
        'turn-id': sharedTurnId,
        'last-assistant-message': 'Keep going and finish the cleanup from here.',
      });
      assert.equal(first.status, 0, `first hook failed: ${first.stderr || first.stdout}`);

      const second = runNotifyHook(cwd, fakeBinDir, geminiHome, {
        'turn-id': sharedTurnId,
  ...
  it('applies TTL suppression between similar nudges and allows a later retry after TTL', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const third = runNotifyHook(cwd, fakeBinDir, geminiHome, {
  ...
  it('ignores non-turn-complete payloads so the same stalled reply cannot re-nudge without a new Gemini boundary', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
  ...
  it('uses custom patterns from config', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result1 = runNotifyHook(cwd, fakeBinDir, geminiHome, {
        'last-assistant-message': 'Keep going and finish the focused cleanup.',
      });
      ...
      const result2 = runNotifyHook(cwd, fakeBinDir, geminiHome, {
  ...
  it('defaults to enabled when no config file exists', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // No .omg-config.json at all — should use defaults (enabled=true, stallMs=5000)
      await writeJson(join(geminiHome, '.omg-config.json'), {
        autoNudge: { enabled: true, delaySec: 0, stallMs: 0 },
      });

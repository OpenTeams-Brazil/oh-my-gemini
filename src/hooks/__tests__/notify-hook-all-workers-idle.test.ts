...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-notify-all-idle-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHookAsWorker(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMG_TEAM_STATE_ROOT: join(cwd, '.omg', 'state'),
      OMG_TEAM_LEADER_CWD: '',
      OMG_MODEL_INSTRUCTIONS_FILE: '',
      OMG_TEAM_WORKER_IDLE_NOTIFY: 'false',
      OMG_TEAM_ALL_IDLE_COOLDOWN_MS: '500', // short cooldown for tests
      TMUX: '',
      TMUX_PANE: '',
      ...extraEnv,
    },
...
describe('notify-hook all-workers-idle notification', () => {
  ...
  it('writes deferred visibility once per cooldown window when leader pane is missing', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { OMG_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { OMG_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
...
`;
  ...
  it('respects cooldown: does not send repeated notifications', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        OMG_TEAM_ALL_IDLE_COOLDOWN_MS: '600000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
    ...
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
        ...
        env: {
          ...process.env,
          PATH: `${fakeBinDir}:${process.env.PATH || ''}`,
          OMG_TEAM_WORKER: '', // empty = not a worker
          OMG_TEAM_STATE_ROOT: '',
          OMG_TEAM_LEADER_CWD: '',
          OMG_MODEL_INSTRUCTIONS_FILE: '',
          TMUX: '',
  ...
  it('handles single worker team correctly with singular message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Run `omg team status solo-team` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should tell the agent to execute the runtime status check directly');
      assert.doesNotMatch(tmuxLog, /Next: run omg team status solo-team, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should not fall back to human-advisory wording');

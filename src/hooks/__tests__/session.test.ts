...
describe('session lifecycle manager', () => {
  it('resets session metrics files with zeroed counters', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-metrics-'));
    try {
      await resetSessionMetrics(cwd);

      const metricsPath = join(cwd, '.omg', 'metrics.json');
      const hudPath = join(cwd, '.omg', 'state', 'hud-state.json');
  ...
  it('writes hud session metrics into the active session scope when session id is provided', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-metrics-scoped-'));
    try {
      await resetSessionMetrics(cwd, 'sess-scoped');

      const metricsPath = join(cwd, '.omg', 'metrics.json');
      const hudPath = join(cwd, '.omg', 'state', 'sessions', 'sess-scoped', 'hud-state.json');
      assert.equal(existsSync(metricsPath), true);
      assert.equal(existsSync(hudPath), true);
  ...
  it('treats symlinked cwd aliases as authoritative for the same session state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-cwd-alias-'));
  ...
  it('writes session start/end lifecycle artifacts and archives session history', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-lifecycle-'));
    ...
    try {
      ...
      const historyPath = join(cwd, '.omg', 'logs', 'session-history.jsonl');
  ...
  it('preserves canonical session id while reconciling native SessionStart metadata', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-native-reconcile-'));
    try {
  ...
  it('falls back to a fresh canonical session when reconciling without authoritative launch state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-native-fallback-'));
    try {
      const statePath = join(cwd, '.omg', 'state', 'session.json');
      await resetSessionMetrics(cwd);
  ...
  it('treats invalid session JSON as absent state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-invalid-'));
    try {
      const statePath = join(cwd, '.omg', 'state', 'session.json');
      await resetSessionMetrics(cwd);
  ...
  it('ignores session.json when its recorded cwd points at another worktree', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-mismatched-cwd-'));
    try {
      const statePath = join(cwd, '.omg', 'state', 'session.json');
      await resetSessionMetrics(cwd);
      await writeFile(statePath, JSON.stringify({
  ...
  it('ignores session.json when its PID identity is stale', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-stale-pointer-'));
    try {
      const statePath = join(cwd, '.omg', 'state', 'session.json');
      await resetSessionMetrics(cwd);
      await writeFile(statePath, JSON.stringify({
        ...
        pid: 4242,
        pid_start_ticks: 11,
        pid_cmdline: 'node omg',
      }), 'utf-8');

      const state = await readUsableSessionState(cwd, {
        platform: 'linux',
        isPidAlive: () => true,
        readLinuxIdentity: () => ({ startTicks: 22, cmdline: 'node omg' }),
      });
      assert.equal(state, null);
    } finally {
      await rm(cwd, { recursive: true, force: true });
...
describe('isSessionStale', () => {
  it('returns false for a live Linux process when identity matches', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 111, cmdline: 'node omg' }),
    });
  ...
  it('returns true for PID reuse on Linux when start ticks mismatch', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 222, cmdline: 'node omg' }),
    });

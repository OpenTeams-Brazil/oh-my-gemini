...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-notify-worker-idle-'));
  try {
    await run(cwd);
  } finally {
...
`;
  ...
  it('existing all-workers-idle hook still fires alongside per-worker', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Run `omg team status both-hooks` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle nudge should tell the agent to execute the runtime status check directly');
      assert.doesNotMatch(tmuxLog, /Next: run omg team status both-hooks, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle nudge should not fall back to human-advisory wording');
    });
  });
});

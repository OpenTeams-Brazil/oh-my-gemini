...
async function listDispatchProcessingLeases(cwd: string, teamName: string): Promise<string[]> {
  const dispatchDir = join(cwd, '.omg', 'state', 'team', teamName, 'dispatch');
...
describe('notify-hook team dispatch consumer', () => {
  ...
  it('leader-fixed dispatch remains pending with leader_pane_missing_deferred when pane missing', async () => {
    ...
    try {
      ...
      const eventsPath = join(cwd, '.omg', 'state', 'team', 'alpha', 'events', 'events.ndjson');
      ...
      const dispatchLogPath = join(cwd, '.omg', 'logs', `team-dispatch-${new Date().toISOString().slice(0, 10)}.jsonl`);
  ...
  it('invokes omg-runtime exec via shared bridge fallback', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-hook-team-dispatch-'));
  ...
  it('leader-fixed dispatch uses pane target only when leader_pane_id exists', async () => {
    ...
    try {
      ...
      await enqueueDispatchRequest('alpha', {
        ...
        to_worker: 'leader-fixed',
        message_id: msg.message_id,
        trigger_message: 'Read .omg/state/team/alpha/mailbox/leader-fixed.json; worker-1 sent a new message. Review it and decide the next concrete step.',
      }, cwd);
...
if [[ "$cmd" == "display-message" ]]; then
  ...
  if [[ "$fmt" == "#{pane_start_command}" && "$target" == "%91" ]]; then
    echo "node dist/cli/omg.js hud --watch"
    exit 0
  fi
...
`,
  ...
  it('releases the global dispatch lock before slow tmux injection so mailbox sends do not wedge mid-run', async () => {
    ...
    try {
      ...
      const requests = JSON.parse(
        await readFile(join(cwd, '.omg', 'state', 'team', 'alpha', 'dispatch', 'requests.json'), 'utf-8'),
  ...
  it('keeps retry_pending derived-only and does not persist transient tags', async () => {
    ...
    try {
      ...
      const rawRequests = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'alpha', 'dispatch', 'requests.json'), 'utf8'));
  ...
  it('resolves session-only dispatch targets without managed leader session context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-dispatch-session-target-'));
    ...
    try {
      ...
      assert.ok(cfg);
      if (!cfg) throw new Error('missing team config');
      cfg.tmux_session = 'omg-team-session-target';
      ...
      assert.match(tmuxLog, /list-panes -t omg-team-session-target/);

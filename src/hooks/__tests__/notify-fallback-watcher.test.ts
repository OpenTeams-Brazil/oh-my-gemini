...
describe('notify-fallback watcher', () => {
  it('one-shot mode forwards only recent task_complete events', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-once-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-home-'));
    ...
    try {
      ...
      const turnLog = join(wd, '.omg', 'logs', `turns-${new Date().toISOString().split('T')[0]}.jsonl`);
      ...
      const fallbackLog = join(wd, '.omg', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('rotates notify-fallback logs when the size cap is exceeded', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-once-rotate-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-home-'));
  ...
  it('streaming mode buffers partial JSON lines until the newline arrives', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-stream-partial-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-home-'));
    const sid = randomUUID();
    const sessionDir = todaySessionDir(tempHome);
...
`
      ...
      const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
      const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
      const watcherStatePath = join(wd, '.omg', 'state', 'notify-fallback-state.json');
      const turnLog = join(wd, '.omg', 'logs', `turns-${new Date().toISOString().split('T')[0]}.jsonl`);
      const child = spawn(
        process.execPath,
  ...
  it('streaming mode tails from EOF and does not replay backlog', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-stream-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-home-'));
    const sid = randomUUID();
    const sessionDir = todaySessionDir(tempHome);
  ...
  it('records explicit leader-only dispatch drain state and log visibility in one-shot mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-dispatch-state-'));
    try {
      ...
      const logPath = join(wd, '.omg', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('suppresses idle no-op lifecycle and control-plane logs during authority-only one-shot ticks', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-authority-noop-'));
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state'), { recursive: true });
  ...
  it('suppresses authority-only control-plane ticks when only skill-active-state carries the deep-interview input lock', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-authority-skill-lock-'));
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state'), { recursive: true });
      await writeFile(join(wd, '.omg', 'state', 'skill-active-state.json'), JSON.stringify({
        version: 1,
        active: true,
  ...
  it('backs off authority-only nudge ticks when the primary watcher is healthy', async () => {
    ...
    try {
      ...
      await mkdir(join(wd, '.omg', 'state', 'sessions', 'sess-managed-fallback'), { recursive: true });
      await writeFile(join(wd, '.omg', 'state', 'sessions', 'sess-managed-fallback', 'hud-state.json'), JSON.stringify({
        last_turn_at: new Date(Date.now() - 6_000).toISOString(),
        turn_count: 7,
        last_agent_output: 'Keep going and finish the cleanup from here.',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'notify-fallback.pid'), JSON.stringify({
        pid: process.pid,
        cwd: wd,
        started_at: new Date().toISOString(),
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'notify-fallback-state.json'), JSON.stringify({
        pid: process.pid,
        cwd: wd,
      ...
      const watcherState = JSON.parse(await readFile(join(wd, '.omg', 'state', 'notify-fallback-state.json'), 'utf-8'));
  ...
  it('treats symlinked cwd aliases as the same primary watcher during authority handoff', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-cwd-alias-'));
  ...
  it('disables fallback watcher nudges when deep-interview state is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-deep-interview-suppressed-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state', 'team', 'dispatch-team'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      await writeFile(join(fakeBinDir, 'tmux'), buildFakeTmux(tmuxLogPath));
      await chmod(join(fakeBinDir, 'tmux'), 0o755);

      await writeFile(join(wd, '.omg', 'state', 'deep-interview-state.json'), JSON.stringify({
        active: true,
        mode: 'deep-interview',
        current_phase: 'deep-interview',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'ralph-state.json'), JSON.stringify({
        active: true,
        current_phase: 'executing',
        tmux_pane_id: '%42',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'team-state.json'), JSON.stringify({
        active: true,
        team_name: 'dispatch-team',
        current_phase: 'team-exec',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'hud-state.json'), JSON.stringify({
        last_turn_at: new Date(Date.now() - 300_000).toISOString(),
        turn_count: 3,
        last_agent_output: 'Would you like me to continue?',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'config.json'), JSON.stringify({
        name: 'dispatch-team',
        tmux_session: 'omg-team-dispatch-team',
        leader_pane_id: '%42',
      }, null, 2));
  ...
  it('disables fallback watcher nudges when only skill-active-state carries the deep-interview input lock', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-deep-interview-skill-lock-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
  ...
  it('runs leader nudge checks from the fallback watcher so stale alerts do not wait for a leader turn', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-leader-nudge-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      ...
      const deliveryLogPath = join(wd, '.omg', 'logs', `team-delivery-${new Date().toISOString().slice(0, 10)}.jsonl`);
  ...
  it('runs leader nudge checks from canonical fallback when coarse team-state is inactive', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-leader-nudge-canonical-inactive-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(fakeBinDir, { recursive: true });
  ...
  it('skips fallback watcher leader nudges when the leader is not stale even if mailbox messages exist', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-leader-nudge-fresh-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'mailbox'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      await writeFile(join(fakeBinDir, 'tmux'), buildFakeTmux(tmuxLogPath));
      await chmod(join(fakeBinDir, 'tmux'), 0o755);
      ...
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'mailbox', 'leader-fixed.json'), JSON.stringify({
        worker: 'leader-fixed',
        messages: [
          {
            message_id: 'msg-1',
  ...
  it('runs stalled-worker leader nudges from the fallback watcher even when the leader is not stale', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-worker-stall-nudge-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1'), { recursive: true });
      await mkdir(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'tasks'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
...
`;
      ...
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'tasks', 'task-1.json'), JSON.stringify({
        id: '1',
      ...
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1', 'status.json'), JSON.stringify({
        state: 'working',
        current_task_id: '1',
        updated_at: new Date(now - 180_000).toISOString(),
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1', 'heartbeat.json'), JSON.stringify({
        alive: true,
        pid: 101,
        turn_count: 2,
        last_turn_at: new Date(now - 180_000).toISOString(),
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'team-leader-nudge.json'), JSON.stringify({
        last_nudged_by_team: {
          'dispatch-team': {
  ...
  it('respects `.omg/tmux-hook.json` enabled:false for fallback auto-nudge', async () => {
    ...
    try {
      ...
      await writeFile(join(wd, '.omg', 'tmux-hook.json'), JSON.stringify({
        enabled: false,
  ...
  it('suppresses fallback unmanaged-session auto-nudge skip logs while idle', async () => {
    ...
    try {
      ...
      const tmuxHookLogPath = join(wd, '.omg', 'logs', `tmux-hook-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('does not fallback auto-nudge a stalled hud snapshot that notify-hook already nudged', async () => {
    ...
    try {
      ...
      await writeFile(join(wd, '.omg', 'state', 'auto-nudge-state.json'), JSON.stringify({
        nudgeCount: 1,
  ...
  it('watcher retry does not retype when pre-capture still contains trigger', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-dispatch-cm-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
  ...
  it('sends bounded periodic Ralph continue steer while Ralph state stays active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-active-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
  ...
  it('suppresses Ralph continue steer when hud progress is still fresh after cooldown', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-progress-fresh-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('still sends Ralph continue steer when hud progress is stale after cooldown', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-progress-stale-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('suppresses Ralph continue steer when session-scoped Ralph is stuck in stale starting phase', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-starting-stale-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
  ...
  it('suppresses Ralph continue steer while tracked native subagents are still active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-subagents-active-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
    ...
    try {
      ...
      await writeFile(join(fakeBinDir, 'tmux'), buildFakeTmux(tmuxLogPath));
      await chmod(join(fakeBinDir, 'tmux'), 0o755);
      await writeSessionStart(wd, omgSessionId);
      await writeFile(join(stateDir, 'sessions', omgSessionId, 'ralph-state.json'), JSON.stringify({
        active: true,
        current_phase: 'executing',
        tmux_pane_id: '%42',
  ...
  it('fails closed when Ralph hud progress is missing or invalid', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-progress-guard-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('fails closed when active Ralph state has no bound tmux pane', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-pane-missing-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('waits a full cadence from startup when persisted Ralph steer state is empty', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-startup-cooldown-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('falls back to an aged persisted cooldown anchor when last_sent_at is invalid', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-invalid-last-sent-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omg', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('stops Ralph continue steer immediately once Ralph state is terminal or cleared', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-terminal-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
    const watcherStatePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('treats a long-running starting phase as terminal so Ralph steer stops', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-starting-phase-stale-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
    const watcherStatePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('globally debounces Ralph continue steer across concurrent watcher instances', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-ralph-global-debounce-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
    const sharedTimestampPath = join(stateDir, 'ralph-last-steer-at');
  ...
  it('retypes on every retry when trigger is not in narrow input area', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-dispatch-cm-fallback-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
  ...
  it('exits when the tracked parent pid is gone', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-exit-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const logPath = join(wd, '.omg', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('ignores stale session-scoped Ralph state when the current session identity is stale', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-stale-session-ralph-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
  ...
  it('ignores stale root Ralph state when the current session has not started Ralph', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-stale-root-ralph-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
  ...
  it('keeps ticking for active session-scoped Ralph after parent loss, then stops once Ralph is terminal', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-ralph-active-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-ralph-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omg', 'state');
  ...
  it('stays alive after parent exit while an active team still has live worker panes', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-team-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const teamStatePath = join(wd, '.omg', 'state', 'team-state.json');
    let child: ReturnType<typeof spawn> | undefined;
  ...
  it('stays alive after parent exit when coarse team-state is missing but canonical team is active for the current session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-canonical-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-canonical-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    let child: ReturnType<typeof spawn> | undefined;
  ...
  it('does not defer parent-loss shutdown when canonical owner session is blank and coarse team-state is missing', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-ownerless-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-ownerless-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    let child: ReturnType<typeof spawn> | undefined;
  ...
  it('does not defer parent-loss shutdown for a team that is already terminal in phase.json', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-terminal-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-parent-gone-terminal-team-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    let child: ReturnType<typeof spawn> | undefined;
    ...
    try {
      ...
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'phase.json'), JSON.stringify({
        current_phase: 'complete',
      }, null, 2));
      await writeFile(join(wd, '.omg', 'state', 'team', 'dispatch-team', 'config.json'), JSON.stringify({
        name: 'dispatch-team',
  ...
  it('replaces a stale watcher from the per-cwd pid file', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-stale-pid-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-stale-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const pidPath = join(wd, '.omg', 'state', 'notify-fallback.pid');
  ...
  it('backs off idle polling and resets to the base cadence after fresh rollout activity', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-idle-backoff-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-idle-backoff-home-'));
    const sid = randomUUID();
    const sessionDir = todaySessionDir(tempHome);
    ...
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const watcherStatePath = join(wd, '.omg', 'state', 'notify-fallback-state.json');
    const turnLogPath = join(wd, '.omg', 'logs', `turns-${new Date().toISOString().split('T')[0]}.jsonl`);
    let child: ReturnType<typeof spawn> | undefined;
...
`);
  ...
  it('exits after the configured max lifetime', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-max-life-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omg-fallback-max-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const logPath = join(wd, '.omg', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
    let child: ReturnType<typeof spawn> | undefined;
  ...
  it('keeps the detached helper alive after the hidden bootstrap exits', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-fallback-bootstrap-survival-'));

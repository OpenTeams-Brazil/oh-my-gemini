...
describe('normalizeTmuxHookConfig', () => {
  ...
  it('normalizes valid config', () => {
    const config = normalizeTmuxHookConfig({
      enabled: true,
      target: { type: 'session', value: 'omg-work' },
      allowed_modes: ['team'],
      cooldown_ms: 2000,
    ...
    assert.deepEqual(config.target, { type: 'session', value: 'omg-work' });
...
describe('paneLooksReady', () => {
  ...
  it('accepts Gemini welcome-screen helper text', () => {
    assert.equal(paneLooksReady('How can I help you today?'), true);
  });

  it('accepts Gemini welcome-screen suggestion rows with a prompt glyph', () => {
  ...
  it('keeps viewport-only Gemini captures non-ready when the prompt is below the fold', () => {
...
describe('paneShowsGeminiViewport', () => {
  it('detects a live Gemini viewport without requiring the prompt to remain visible', () => {
    assert.equal(paneShowsGeminiViewport(`╭────────────────────────────────────────────╮
│ >_ OpenAI Gemini (v0.118.0)                 │
│                                            │
│ model:     gpt-5.4 high   /model to change │
│ directory: /tmp/demo                       │
...
⚠ MCP startup incomplete (failed: hf request)`), true);
  ...
  it('rejects status-only and bootstrapping captures', () => {
    assert.equal(paneShowsGeminiViewport('gpt-5 50% left'), false);
    assert.equal(paneShowsGeminiViewport(`╭────────────────────────────────────────────╮
│ >_ OpenAI Gemini (v0.118.0)                 │
│                                            │
│ model: loading                             │
...
describe('paneHasActiveTask', () => {
  it('detects Gemini and Claude activity markers', () => {
...
if [[ "$cmd" == "display-message" ]]; then
  ...
  if [[ "$format" == "#{pane_start_command}" && "$target" == "%2" ]]; then
    echo "node /pkg/dist/cli/omg.js hud --watch"
    exit 0
  fi
...
`);
      ...
      assert.equal(resolveGeminiPane(), '%42');
    } finally {

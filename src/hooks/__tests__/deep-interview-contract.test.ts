...
function readProjectAgents(startDir: string): string {
	...
	while (true) {
		const candidate = join(currentDir, "GEMINI.md");
	...
	return readFileSync(join(startDir, "GEMINI.md"), "utf-8");
}
...
const templateAgents = readFileSync(
	join(__dirname, "../../../templates/GEMINI.md"),
	"utf-8",
);
...
describe("deep-interview Ouroboros contract", () => {
	...
	it("uses OMX-native output paths", () => {
		assert.match(deepInterviewSkill, /\.omg\/interviews\//);
		assert.match(deepInterviewSkill, /\.omg\/specs\//);
	});
	...
	it("documents the autoresearch specialization contract", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.omg\/specs\/deep-interview-autoresearch-\{slug\}\.md/i,
		);

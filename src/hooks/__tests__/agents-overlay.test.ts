/**
 * Tests for GEMINI.md Runtime Overlay
 *
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "omg-overlay-test-"));
  await mkdir(join(dir, ".omg", "state"), { recursive: true });
  return dir;
}
...
describe("generateOverlay", () => {
  ...
  it("generates overlay with session-scoped active modes for current session", async () => {
    await mkdir(join(tempDir, ".omg", "state", "sessions", "sess1"), {
      recursive: true,
    });
    await writeFile(
      join(tempDir, ".omg", "state", "sessions", "sess1", "team-state.json"),
      JSON.stringify({
        active: true,
        iteration: 1,
        max_iterations: 5,
  ...
  it("lists both approved combined workflow members from canonical skill state", async () => {
    ...
    await mkdir(sessionDir, { recursive: true });
    await writeFile(
      join(tempDir, ".omg", "state", "session.json"),
  ...
  it("generates overlay with project memory summary", async () => {
    await writeFile(
      join(tempDir, ".omg", "project-memory.json"),
      JSON.stringify({
  ...
  it("skips inactive modes", async () => {
    await writeFile(
      join(tempDir, ".omg", "state", "autopilot-state.json"),
  ...
  it("adds blocked ralph planning gate when PRD/test spec are missing", async () => {
    ...
    await mkdir(join(tempDir, ".omg", "plans"), { recursive: true });
  ...
  it("unlocks ralph planning gate when PRD and test spec exist", async () => {
    ...
    const plansDir = join(tempDir, ".omg", "plans");
...
describe("resolveSessionOrchestrationMode", () => {
  ...
  it("falls back to root team skill state only when no session-scoped skill state exists", async () => {
    ...
    await writeFile(
      join(tempDir, ".omg", "state", "skill-active-state.json"),
      JSON.stringify({ active: true, skill: "team" }),
    );
  ...
  it("active mode summary follows canonical session skill state instead of stale root mode files", async () => {
    ...
    const rootStateDir = join(tempDir, ".omg", "state");
...
describe("applyOverlay + stripOverlay roundtrip", () => {
  let tempDir: string;
  const originalContent = `# My GEMINI.md
...
`;
  ...
  it("apply then strip restores original (roundtrip)", async () => {
    const agentsMd = join(tempDir, "GEMINI.md");
...
`;
  ...
  it("creates GEMINI.md if it does not exist during apply", async () => {
...
describe("session-scoped model instructions file", () => {
  let tempDir: string;
  let restoreGeminiHome: (() => void) | undefined;

  before(async () => {
    tempDir = await makeTempDir();
    restoreGeminiHome = setMockGeminiHome(join(tempDir, "home", ".gemini"));
  });
  after(async () => {
    restoreGeminiHome?.();
    await rm(tempDir, { recursive: true, force: true });
  ...
  it("writes user + project GEMINI.md + runtime overlay into session-scoped file", async () => {
    const userAgentsMd = join(tempDir, "home", ".gemini", "GEMINI.md");
    const projectAgentsMd = join(tempDir, "GEMINI.md");
    await mkdir(join(tempDir, "home", ".gemini"), { recursive: true });
  ...
  it("deduplicates duplicate skill references when project and user scopes both install the same skill", async () => {
    const userAgentsMd = join(tempDir, "home", ".gemini", "GEMINI.md");
    const projectAgentsMd = join(tempDir, "GEMINI.md");
    const userSkillDir = join(tempDir, "home", ".gemini", "skills", "help");
    const projectSkillDir = join(tempDir, ".gemini", "skills", "help");
    await mkdir(join(tempDir, "home", ".gemini"), { recursive: true });
    ...
    await writeFile(
      ...
      [
        "# User instructions",
        "",
        "- help: user copy (file: /tmp/home/.gemini/skills/help/SKILL.md)",
      ].join("\n"),
    );
    await writeFile(
      projectAgentsMd,
      [
        ...
        "",
        "- help: project copy (file: /tmp/project/.gemini/skills/help/SKILL.md)",
      ].join("\n"),
    );
  ...
  it("writes overlay-only session file when no base GEMINI.md files exist", async () => {
  ...
  it("removes session-scoped file without touching project GEMINI.md", async () => {
    const projectAgentsMd = join(tempDir, "GEMINI.md");

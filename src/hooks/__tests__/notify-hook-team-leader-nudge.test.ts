...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-notify-team-nudge-'));
  try {
    await run(cwd);
  } finally {
...
async function writeCanonicalTeamFixture(
  ...
  await mkdir(join(cwd, '.omg', 'logs'), { recursive: true });
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMG_TEAM_LEADER_NUDGE_MS: '10000',
      OMG_TEAM_LEADER_STALE_MS: '10000',
      OMG_TEAM_WORKER: '',
      OMG_TEAM_STATE_ROOT: '',
      OMG_TEAM_LEADER_CWD: '',
...
describe('notify-hook leader-side authority handoff', () => {
  it('does not inject leader nudge from notify-hook when team is active and stale', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, {
        OMG_SESSION_ID: 'sess-canonical-missing',
      });
  ...
  it('does not nudge stale leader when recent team status activity proves the leader is active', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-beta-active-status',
        leader_pane_id: '%92',
        workers: [
...
describe('notify-hook team leader nudge', () => {
  ...
  it('sends immediate all-workers-idle nudge for active team (leader context)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(tmuxLog, /\[OMG_INTENT:/, 'should keep orchestration intent out of injected display text');
      assert.match(tmuxLog, /\[OMG_TMUX_INJECT\]/, 'should include injection marker');
  ...
  it('suggests shutdown when all workers are idle and the current task set is complete', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: decide whether to reconcile\/merge results or gracefully shut down: omg team shutdown idle-shutdown\./);
  ...
  it('injects leader nudge into a busy live Gemini pane so the message can queue', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
...
`;
      ...
      assert.match(tmuxLog, /\[OMG_TMUX_INJECT\]/, 'should keep the injection marker on busy-pane sends');
  ...
  it('surfaces ack-like mailbox replies without work-start evidence as missing-start nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check worker-1 msg\/output, confirm task in omg team status ack-missing-start/);
  ...
  it('does not classify ack-like replies as missing-start after a worker has claimed work', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omg team shutdown ack-with-start\./);
  ...
  it('does not re-nudge for the same fresh mailbox message on repeated notify-hook runs', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_NUDGE_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_NUDGE_MS: '600000' });
...
`;
      ...
      assert.match(tmuxLog, /send-keys -t %73/, 'should inject into a busy leader pane so Gemini can queue the message');
  ...
  it('injects leader nudge when capture-pane fails but the leader pane is a live gemini pane', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omgDir = join(cwd, '.omg');
      const stateDir = join(omgDir, 'state');
      const logsDir = join(omgDir, 'logs');
...
`;
      ...
      assert.match(tmuxLog, /send-keys -t %74/, 'capture failures should not suppress leader injection into a live gemini pane');

      const eventsPath = join(teamDir, 'events', 'events.ndjson');
      if (existsSync(eventsPath)) {
        const events = (await readFile(eventsPath, 'utf-8')).trim().split('\n').filter(Boolean).map((line) => JSON.parse(line));
        const deferred = events.find((entry: { type?: string }) => entry.type === 'leader_notification_deferred');
        assert.equal(deferred, undefined, 'capture failure alone should not defer a live gemini leader pane');
      }
    });
  });
...
if [[ "$cmd" == "capture-pane" ]]; then
  cat <<'EOF'
Team same-classified-state: 1 msg(s) for leader. Next: read messages; keep orchestrating; if done, gracefully shut down: omg team shutdown same-classified-state.
EOF
  exit 0
fi
if [[ "$cmd" == "send-keys" ]]; then
...
`;
  ...
  it('nudges when worker panes are alive and leader is stale (no recent HUD turn)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check messages; keep orchestrating; if done, gracefully shut down: omg team shutdown beta\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMG_TMUX_INJECT\]/, 'should include injection marker');
    });
  });
  ...
  it('nudges when team progress is stalled even if timing signals are fresh or missing (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-stalled-progress',
        leader_pane_id: '%90',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
      ...
      assert.match(tmuxLog, /Next: omg team status stalled-progress; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /\[OMG_INTENT:/);
      assert.match(tmuxLog, /\[OMG_TMUX_INJECT\]/);
  ...
  it('nudges when team progress is stalled before the leader becomes stale (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-stalled-before-stale',
        leader_pane_id: '%89',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      assert.match(tmuxLog, /Next: omg team status stalled-before-stale; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /leader stale/);
  ...
  it('nudges stalled team after configurable worker turn stall window elapses (primary threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-worker-turn-stall-threshold',
        leader_pane_id: '%86',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
        ],
      ...
      const result = runNotifyHook(cwd, fakeBinDir, {
        OMG_TEAM_PROGRESS_STALL_MS: '120000',
        OMG_TEAM_WORKER_TURN_STALL_MS: '30000',
        OMG_TEAM_LEADER_NUDGE_MS: '30000',
        OMG_TEAM_LEADER_STALE_MS: '60000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('does not nudge stalled team when an in-progress worker is still advancing heartbeat turns', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-active-turns-no-stall',
        leader_pane_id: '%87',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
  ...
  it('bounds repeated stalled-team nudges before leader stale by cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-stalled-before-stale-bounded',
        leader_pane_id: '%88',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      assert.match(tmuxLog, /Next: omg team status stalled-before-stale-bounded; read worker messages; unblock\/reassign or shutdown\./);
    });
  });
  ...
  it('does not treat leader and HUD panes as active worker panes when worker pane ids are known', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-stale-no-workers',
        leader_pane_id: '%92',
        hud_pane_id: '%93',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
  ...
  it('does not send a generic periodic leader nudge when the leader is not stale', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_NUDGE_MS: '30000' });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('uses a 30s cadence for stale leader follow-up nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-stale-cadence',
        leader_pane_id: '%96',
      });
      ...
      const blocked = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_NUDGE_MS: '30000' });
      ...
      const allowed = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_NUDGE_MS: '30000' });
  ...
  it('suppresses stale leader follow-up when detached worktree progress is still fresh', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await mkdir(tasksDir, { recursive: true });
      await mkdir(join(workersDir, 'worker-1'), { recursive: true });
      await mkdir(join(workerWorktree, '.omg', 'state'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omg-team-fresh-detached-progress',
        leader_pane_id: '%99',
        workers: [
      ...
      await writeJson(join(workerWorktree, '.omg', 'state', 'current-task-baseline.json'), {
        version: 1,
        tasks: [],
      });
  ...
  it('bounds repeated all-workers-idle nudges by cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHook(cwd, fakeBinDir, { OMG_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(second.status, 0, `notify-hook failed: ${second.stderr || second.stdout}`);
  ...
  it('includes stale_leader_with_messages reason when both conditions met', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omg team shutdown delta\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMG_TMUX_INJECT\]/, 'should include injection marker');

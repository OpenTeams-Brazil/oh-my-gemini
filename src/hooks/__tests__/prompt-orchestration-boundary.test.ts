...
describe('prompt orchestration boundary', () => {
  ...
  it('root AGENTS contract states that child prompts report handoffs upward', () => {
    assert.match(loadSurface('GEMINI.md'), /report recommended handoffs upward/i);
    assert.match(loadSurface('templates/GEMINI.md'), /report recommended handoffs upward/i);
  });

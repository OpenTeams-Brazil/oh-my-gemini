...
`);
  ...
  describe('omg', () => {
    it('exposes only the explicit read-only omg readers', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omg-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });

        assert.deepEqual(Object.keys(sdk.omg).sort(), ['hud', 'notifyFallback', 'session', 'updateCheck']);
        assert.equal(typeof sdk.omg.session.read, 'function');
        assert.equal(typeof sdk.omg.hud.read, 'function');
        assert.equal(typeof sdk.omg.notifyFallback.read, 'function');
        assert.equal(typeof sdk.omg.updateCheck.read, 'function');
        assert.equal('pluginState' in sdk, false);
        assert.equal('readJson' in sdk.omg, false);
        assert.equal('list' in sdk.omg, false);
        assert.equal('exists' in sdk.omg, false);
        assert.equal('write' in sdk.omg.session, false);
        assert.equal('delete' in sdk.omg.session, false);
        assert.equal('write' in sdk.omg.hud, false);
        assert.equal('delete' in sdk.omg.hud, false);
        assert.equal('write' in sdk.omg.notifyFallback, false);
        assert.equal('delete' in sdk.omg.notifyFallback, false);
        assert.equal('write' in sdk.omg.updateCheck, false);
        assert.equal('delete' in sdk.omg.updateCheck, false);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });

    it('reads session state from .omg/state/session.json', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omg-sdk-'));
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        const state = await sdk.omg.session.read();
    ...
    it('returns null for invalid session state without session_id', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omg.session.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });

    it('reads hud, notifyFallback, and updateCheck state from root-scoped omg files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omg-sdk-'));
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.deepEqual(await sdk.omg.hud.read(), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        assert.deepEqual(await sdk.omg.notifyFallback.read(), {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        assert.deepEqual(await sdk.omg.updateCheck.read(), {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });
      } finally {
    ...
    it('reads hud state from the current session scope instead of stale root fallback', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omg-sdk-hud-session-'));
      try {
        await writeOmxStateFile(cwd, 'session.json', {
    ...
    it('returns null for missing omg reader files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omg-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omg.session.read(), null);
        assert.equal(await sdk.omg.hud.read(), null);
        assert.equal(await sdk.omg.notifyFallback.read(), null);
        assert.equal(await sdk.omg.updateCheck.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
...
describe('clearHookPluginState', () => {
  it('removes data.json and tmux.json for plugin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-clear-'));
    try {
      const pluginDir = join(cwd, '.omg', 'state', 'hooks', 'plugins', 'my-plugin');

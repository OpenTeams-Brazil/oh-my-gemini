...
describe('hooksDir', () => {
  it('returns .omg/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.omg', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .omg/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.omg', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-load-'));
    try {

...
describe('dispatchHookEvent', () => {
  it('returns disabled summary when plugins are disabled', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-dispatch-'));
    try {
...
import { join } from 'node:path';
appendFileSync(join(process.cwd(), '.omg', 'top-level-pids.log'), String(process.pid) + '\\n');
...
`,
      ...
      const pids = (await readFile(join(cwd, '.omg', 'top-level-pids.log'), 'utf-8'))
        .trim()
        .split('\n')
        .filter(Boolean);
  ...
  it('dedupes repeated native lifecycle hook dispatches for the same session/turn fingerprint', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-dispatch-dedupe-'));
    try {
      const dir = join(cwd, '.omg', 'hooks');
      await mkdir(dir, { recursive: true });
      await writeFile(

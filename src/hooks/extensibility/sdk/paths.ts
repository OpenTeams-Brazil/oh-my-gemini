...
export function hookPluginRootDir(cwd: string, pluginName: string): string {
  return join(cwd, '.omg', 'state', 'hooks', 'plugins', sanitizeHookPluginName(pluginName));
}
...
export function hookPluginLogPath(cwd: string, now = new Date()): string {
  ...
  return join(cwd, '.omg', 'logs', `hooks-${day}.jsonl`);
}

export function omgRootStateFilePath(cwd: string, fileName: string): string {
  return join(cwd, '.omg', 'state', fileName);
}

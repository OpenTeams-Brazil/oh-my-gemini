...
import { omgRootStateFilePath } from './paths.js';
...
export function createHookPluginOmxApi(cwd: string): HookPluginSdk['omg'] {
  return {
    session: {
      read: () => readOmxStateFile<HookPluginOmxSessionState>(
        omgRootStateFilePath(cwd, 'session.json'),
    ...
    notifyFallback: {
      read: () => readOmxStateFile<HookPluginOmxNotifyFallbackState>(
        omgRootStateFilePath(cwd, 'notify-fallback-state.json'),
      ),
    },
    updateCheck: {
      read: () => readOmxStateFile<HookPluginOmxUpdateCheckState>(
        omgRootStateFilePath(cwd, 'update-check.json'),
      ),
    },
  };
}

/**
 * GEMINI.md Runtime Overlay for oh-my-gemini
 *
 * Dynamically injects session-specific context into GEMINI.md before Gemini
...
function lockPath(cwd: string): string {
  return join(cwd, ".omg", "state", "agents-md.lock");
}
...
async function readNotepadPriority(cwd: string): Promise<string> {
  const notePath = omgNotepadPath(cwd);
...
async function readProjectMemorySummary(cwd: string): Promise<string> {
  const memPath = omgProjectMemoryPath(cwd);
...
/**
 * Generate the overlay content to inject into GEMINI.md.
...
/**
 * Apply overlay to GEMINI.md. Strips any existing overlay first (idempotent).
...
/**
 * Strip overlay from GEMINI.md, restoring it to clean state.
 * Uses file locking to prevent concurrent access corruption.
 */
...
/**
 * Check if GEMINI.md currently has an overlay applied.
 */
...
export function sessionModelInstructionsPath(
  ...
  return join(getStateDir(cwd, sessionId), "GEMINI.md");
}
...
/**
 * Build a session-scoped GEMINI.md that combines user-level GEMINI_HOME
 * instructions, project instructions (if any), and the runtime overlay,
 * without mutating the source GEMINI.md files.
 */
export async function writeSessionModelInstructionsFile(
  ...
  const sourcePaths = [join(geminiHome(), "GEMINI.md"), join(cwd, "GEMINI.md")];
  ...
  for (const sourcePath of sourcePaths) {
    ...
    if (sourcePath === join(geminiHome(), "GEMINI.md")) {

...
function commitHygieneReportsDir(cwd: string): string {
  return join(resolve(cwd), '.omg', 'reports', 'team-commit-hygiene')
}
...
export function buildTeamCommitHygieneContext(params: {
  ...
  const recommendedNextSteps = [
    ...
    'Derive semantic commit boundaries from completed task subjects, code diffs, and shutdown reports rather than from omg(team) operational commit subjects.',
...
export function renderTeamCommitHygieneMarkdown(context: TeamCommitHygieneContext): string {
  return [
    ...
    '',
    '1. Treat `omg(team): ...` runtime commits as temporary scaffolding, not as the final PR history.',

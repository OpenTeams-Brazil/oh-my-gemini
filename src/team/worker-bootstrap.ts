...
import { geminiHome, listInstalledSkillDirectories } from "../utils/paths.js";
...
const AGENTS_LOCK_PATH = [".omg", "state", "agents-md.lock"];
...
2. Load the worker skill from the first existing path:
   - \`${"${GEMINI_HOME:-~/.gemini}"}/skills/worker/SKILL.md\`
   - \`${options.leaderCwd}/.gemini/skills/worker/SKILL.md\`
...
6. Use claim-safe lifecycle APIs only:
   ...
   - \`omg team api transition-task-status --json\`
   - \`omg team api release-task-claim --json\` only for rollback to pending
7. Use mailbox delivery flow:
   - \`omg team api mailbox-list --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\"}" --json\`
   - \`omg team api mailbox-mark-delivered --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
8. Preserve leader steering via inbox/mailbox nudges; task payload stays in inbox/task JSON, not this file.
9. Do not pass \`workingDirectory\` to legacy team_* MCP tools; use \`omg team api\` CLI interop.
...
export async function writeWorkerWorktreeRootAgentsFile(
  ...
  const agentsPath = join(options.worktreePath, "GEMINI.md");
  const tracked = isTracked(options.worktreePath, "GEMINI.md");
  ...
  if (tracked) {
    try {
      execFileSync("git", ["update-index", "--skip-worktree", "GEMINI.md"], {
    ...
    await ensureGitInfoExcludePattern(options.worktreePath, "GEMINI.md");
  }
...
export async function removeWorkerWorktreeRootAgentsFile(
  ...
  const agentsPath = join(worktreePath, "GEMINI.md");
  const backupPath = buildWorkerRootAgentsBackupPath(
    teamStateRoot,
    teamName,
    workerName,
  ...
  if (backup.tracked && backup.skipWorktreeApplied) {
    try {
      execFileSync("git", ["update-index", "--no-skip-worktree", "GEMINI.md"], {
        cwd: worktreePath,
        encoding: "utf-8",
        stdio: ["ignore", "pipe", "pipe"],
      windowsHide: true,
...
/**
 * Generate generic GEMINI.md overlay for team workers.
...
2. Load the worker skill instructions from the first path that exists:
   - \`${"${GEMINI_HOME:-~/.gemini}"}/skills/worker/SKILL.md\`
   - \`<leader_cwd>/.gemini/skills/worker/SKILL.md\`
...
7. Request a claim via CLI interop (\`omg team api claim-task --json\`); do not directly set lifecycle fields in the task file
...
10. On completion/failure, use lifecycle transition APIs:
   - \`omg team api transition-task-status --json\` with from \`"in_progress"\` to \`"completed"\` or \`"failed"\`
   - Include \`result\` (for completed) or \`error\` (for failed) in the transition patch
11. Use \`omg team api release-task-claim --json\` only for rollback/requeue to \`pending\` (not for completion)
...
15. For legacy team_* MCP tools (hard-deprecated), switch to \`omg team api\` CLI interop; do not pass workingDirectory unless the lead explicitly tells you to

## Message Protocol
When calling \`omg team api send-message\`, you MUST always include:
...
Keep the body short and deterministic so all worker CLIs (Gemini/Claude) behave consistently.

Example:
omg team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"<your-worker-name>\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: <your-worker-name> initialized\"}" --json
...
1. Read: \`omg team api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\"}" --json\`
2. Mark delivered: \`omg team api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
...
- You may spawn Gemini native subagents when parallel execution improves throughput.
...
/**
 * Apply worker overlay to GEMINI.md. Idempotent -- strips existing overlay first.
 */
...
/**
 * Strip worker overlay from GEMINI.md content. Idempotent.
 */
 ...
 * GEMINI_HOME GEMINI.md, the project's GEMINI.md (if any), and the worker
 * overlay. This avoids mutating the source GEMINI.md files directly.
 *
...
export async function writeTeamWorkerInstructionsFile(
  ...
  const userAgentsPath = join(geminiHome(), "GEMINI.md");
  const sourcePaths = [userAgentsPath, join(cwd, "GEMINI.md")];
...
async function acquireAgentsMdLock(
  ...
  throw new Error("Failed to acquire GEMINI.md lock within timeout");
}
 ...
 * This is written to .omg/state/team/{team}/workers/{worker}/inbox.md by the lead.
 */
...
1. Load and follow the worker skill from the first existing path:
   - \`${"${GEMINI_HOME:-~/.gemini}"}/skills/worker/SKILL.md\`
   - \`${leaderCwd}/.gemini/skills/worker/SKILL.md\`
...
7. Request a claim via CLI interop (\`omg team api claim-task --json\`) to claim it
8. Complete the work described in the task
9. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
10. Complete/fail it via lifecycle transition API (\`omg team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
11. Use \`omg team api release-task-claim --json\` only for rollback to \`pending\`
...
14. For legacy team_* MCP tools (hard-deprecated), use \`omg team api\`; do not pass \`workingDirectory\` unless the lead explicitly asks (if resolution fails, use leader cwd: \`${leaderCwd}\`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   \`omg team api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\"}" --json\`
2. For each undelivered message, mark delivery:
   \`omg team api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`

Use terse ACK bodies (single line) for consistent parsing across Gemini and Claude workers.
...
## Message Protocol
When using \`omg team api send-message\`, ALWAYS include from_worker with YOUR worker name:
- from_worker: "${workerName}"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: omg team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: initialized\"}" --json
...
3. Request a claim via CLI interop (\`omg team api claim-task --json\`)
4. Complete the work
5. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
6. Complete/fail via lifecycle transition API (\`omg team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
7. Use \`omg team api release-task-claim --json\` only for rollback to \`pending\`
...
3. After writing the ack, exit your Gemini session.

Type \`exit\` or press Ctrl+C to end your Gemini session.
`;
}
...
export function generateTriggerMessage(
  workerName: string,
  teamName: string,
  teamStateRoot: string = ".omg/state",
): string {

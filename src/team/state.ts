...
export interface TeamConfig {
  ...
  workers: WorkerInfo[];
  created_at: string;
  tmux_session: string; // "omg-team-{name}"
  next_task_id: number;
  leader_cwd?: string;
...
// Creates: .omg/state/team/{name}/, workers/{worker-1}..{worker-N}/, tasks/
...
export async function markOwnedTeamsLeaderStopObserved(
  ...
  const teamsRoot = join(omgStateDir(cwd), 'team');

...
import { geminiPromptsDir, packageRoot } from '../utils/paths.js';
...
function defaultPromptDirs(projectRoot: string): string[] {
  return [
    ...
    join(projectRoot, '.gemini', 'prompts'),
...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `omg team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `omg ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `omg team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',

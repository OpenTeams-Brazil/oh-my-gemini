...
async function promptStaleCleanup(summary: StaleTeamSummary): Promise<boolean> {
  process.stderr.write(
    `\n[omg] Stale artifacts from previous team "${summary.teamName}":\n` +
...
export function buildTerminalCliResult(
  ...
  return {
    ...
    notice:
      ...
      + `Inspect with "omg team status ${teamName} --json" or "omg team api read-stall-state --input '{\"team_name\":\"${teamName}\"}' --json". `
      + `Run "omg team shutdown ${teamName}" (or --force after state capture) when explicit cleanup is desired.\n`,
  };
}

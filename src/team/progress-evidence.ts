...
async function readTeamNudgeProgressMs(cwd: string, teamName: string): Promise<number> {
  const nudgeState = await readJsonIfExists(join(cwd, '.omg', 'state', 'team-leader-nudge.json'));
...
async function readCurrentTaskBaselineMs(worktreePath: string): Promise<number> {
  return await statMsIfExists(join(worktreePath, '.omg', 'state', 'current-task-baseline.json'));
}

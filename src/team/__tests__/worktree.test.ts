...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-worktree-test-'));
...
describe('worktree parser', () => {
  ...
  // leak into the Gemini shell as input.
...
describe('worktree planning', () => {
  it('plans dedicated autoresearch branch and path naming', async () => {
    ...
    try {
      ...
      assert.match(planned.worktreePath.replace(/\\/g, '/'), /\.omg\/worktrees\/autoresearch-demo-mission-20260314t000000z$/);
    } finally {

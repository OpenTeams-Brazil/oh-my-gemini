...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-shutdown-fallback-'));
...
`,
    ...
    try {
      ...
      const reportPath = join(preservedWorktreePath as string, '.omg', 'diff.md');
      ...
      const commitHygieneJsonPath = join(repo, '.omg', 'reports', 'team-commit-hygiene', 'team-shutdown-fallback-report.context.json');
      const commitHygieneMarkdownPath = join(repo, '.omg', 'reports', 'team-commit-hygiene', 'team-shutdown-fallback-report.md');

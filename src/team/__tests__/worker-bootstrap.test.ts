...
function setMockGeminiHome(geminiHomePath: string): () => void {
  const previous = process.env.GEMINI_HOME;
  process.env.GEMINI_HOME = geminiHomePath;
  return () => {
    if (typeof previous === "string") process.env.GEMINI_HOME = previous;
...
describe("worker bootstrap", () => {
  it("worker skill lifecycle instructions are claim-safe (issue #448)", async () => {
    ...
    assert.match(workerSkill, /omg team api claim-task/);
    assert.match(workerSkill, /omg team api transition-task-status/);
    assert.match(workerSkill, /omg team api release-task-claim/);
    assert.match(
      workerSkill,
  ...
  it("generateWorkerOverlay includes the team name", () => {
    ...
    assert.match(overlay, /<leader_cwd>\/\.gemini\/skills\/worker\/SKILL\.md/);
    ...
    assert.match(overlay, /omg team api claim-task/);
    assert.match(overlay, /omg team api transition-task-status/);
    assert.match(overlay, /omg team api release-task-claim/);
    assert.doesNotMatch(
      overlay,
  ...
  it("applyWorkerOverlay appends to existing GEMINI.md content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-worker-bootstrap-"));
    try {
      const agentsMdPath = join(cwd, "GEMINI.md");
  ...
  it("applyWorkerOverlay reaps stale AGENTS lock directory", async () => {
    ...
    try {
      const agentsMdPath = join(cwd, "GEMINI.md");
      const lockPath = join(cwd, ".omg", "state", "agents-md.lock");
  ...
  it("generateInitialInbox includes worker name, team name, and all tasks", () => {
    ...
    assert.match(inbox, /omg team api claim-task/);
    assert.match(inbox, /omg team api transition-task-status/);
    assert.match(inbox, /omg team api release-task-claim/);
    assert.match(
      inbox,
      /\$\{GEMINI_HOME:-~\/\.gemini\}\/skills\/worker\/SKILL\.md/,
    );
    assert.match(inbox, /\/\.gemini\/skills\/worker\/SKILL\.md/);
  ...
  it("generateTriggerMessage contains the inbox path", () => {
    ...
    assert.match(
      message,
      /\.omg\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("generateMailboxTriggerMessage contains mailbox path and count", () => {
    ...
    assert.match(
      message,
      /Read .*\.omg\/state\/team\/team-mail\/mailbox\/worker-2\.json/,
    );
  ...
  it("generateLeaderMailboxTriggerMessage tells the leader to read the mailbox and decide the next step", () => {
    ...
    assert.match(
      message,
      /Read .*\.omg\/state\/team\/team-mail\/mailbox\/leader-fixed\.json/,
    );
  ...
  it("writeTeamWorkerInstructionsFile composes user + project GEMINI.md with overlay", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-worker-bootstrap-"));
    const restoreGeminiHome = setMockGeminiHome(join(cwd, "home", ".gemini"));
    try {
      await mkdir(join(cwd, "home", ".gemini"), { recursive: true });
      await writeFile(
        join(cwd, "home", ".gemini", "GEMINI.md"),
        "# User Instructions\n\nStart globally.\n",
        "utf8",
      );
      await writeFile(
      ...
      // Verify project GEMINI.md was NOT modified
      const projectContent = await readFile(join(cwd, "GEMINI.md"), "utf8");
  ...
  it("writeTeamWorkerInstructionsFile deduplicates duplicate skill references in favor of project scope", async () => {
    ...
    const restoreGeminiHome = setMockGeminiHome(join(cwd, "home", ".gemini"));
    try {
      const userAgentsPath = join(cwd, "home", ".gemini", "GEMINI.md");
      const projectAgentsPath = join(cwd, "GEMINI.md");
      const userSkillDir = join(cwd, "home", ".gemini", "skills", "help");
      const projectSkillDir = join(cwd, ".gemini", "skills", "help");
      ...
      await writeFile(
        userAgentsPath,
        "- help user (file: /tmp/home/.gemini/skills/help/SKILL.md)\n",
        "utf8",
      );
      await writeFile(
        projectAgentsPath,
        "- help project (file: /tmp/project/.gemini/skills/help/SKILL.md)\n",
        "utf8",
      );
  ...
  it("generateWorkerRootAgentsContent includes hardcoded paths and role prompt without base AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      leaderCwd: "/repo",
      worktreePath: "/repo/.omg/team/root-team/worktrees/worker-3",
    });
  ...
  it("generateWorkerRootAgentsContent hardcodes runtime paths and role prompt without inherited AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      workerRole: "writer",
      rolePromptContent: "<identity>You are Writer.</identity>",
      teamStateRoot: "/tmp/project/.omg/state",
      leaderCwd: "/tmp/project",
      worktreePath: "/tmp/project/.omg/team/root-team/worktrees/worker-2",
    });

    assert.match(content, /# Team Worker Runtime Instructions/);
    assert.match(content, /Inbox path: \/tmp\/project\/.omg\/state\/team\/root-team\/workers\/worker-2\/inbox\.md/);
    assert.match(content, /Mailbox path: \/tmp\/project\/.omg\/state\/team\/root-team\/mailbox\/worker-2\.json/);
    assert.match(content, /Leader mailbox path: \/tmp\/project\/.omg\/state\/team\/root-team\/mailbox\/leader-fixed\.json/);
  ...
  it("writeWorkerWorktreeRootAgentsFile writes disposable root AGENTS and remove restores tracked content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-worker-root-agents-"));
    const worktree = join(cwd, "worktree");
    try {
      await mkdir(join(cwd, ".omg", "state", "team", "restore-team", "workers", "worker-1"), { recursive: true });
      await mkdir(worktree, { recursive: true });
      await writeFile(join(worktree, "GEMINI.md"), "# Base tracked AGENTS\n", "utf8");
      ...
      const outPath = await writeWorkerWorktreeRootAgentsFile({
        ...
        teamStateRoot: join(cwd, ".omg", "state"),
        leaderCwd: cwd,
      ...
      await removeWorkerWorktreeRootAgentsFile("restore-team", "worker-1", join(cwd, ".omg", "state"), worktree);
      const restored = await readFile(join(worktree, "GEMINI.md"), "utf8");
  ...
  it("writeTeamWorkerInstructionsFile works without project GEMINI.md", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-worker-bootstrap-"));
    try {

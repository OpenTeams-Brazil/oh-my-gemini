...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-worktree-repo-'));
...
function expectedLowComplexityModel(geminiHomeOverride?: string): string {
  return resolveTeamLowComplexityDefaultModel(geminiHomeOverride);
}

async function readTeamDeliveryLog(cwd: string): Promise<Array<Record<string, unknown>>> {
  const path = join(cwd, '.omg', 'logs', `team-delivery-${new Date().toISOString().slice(0, 10)}.jsonl`);
...
const stateRoot = process.env.OMG_TEAM_STATE_ROOT;
const worker = String(process.env.OMG_TEAM_WORKER || '');
...
async function withPromptModeGeminiEnv<T>(
  binDir: string,
  ...
  const nextEnv: Record<string, string | undefined> = {
    ...
    OMG_TEST_ALLOW_NONTTY_GEMINI_PROMPT: '1',
    ...extraEnv,
  };
...
function teamStateTestPath(cwd: string, ...parts: string[]): string {
  const stateRoot = process.env.OMG_TEAM_STATE_ROOT ?? join(cwd, '.omg', 'state');
...
describe('runtime', () => {
  it('resolveWorkerLaunchArgsFromEnv injects low-complexity default model when missing', () => {
    const args = resolveWorkerLaunchArgsFromEnv(
      { OMG_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen' },
      'explore',
    );
  ...
  it('resolveWorkerLaunchArgsFromEnv reads low-complexity model from config when present', async () => {
    ...
    const tempGeminiHome = await mkdtemp(join(tmpdir(), 'omg-gemini-home-'));
    await writeFile(
      join(tempGeminiHome, '.omg-config.json'),
  ...
  it('resolveWorkerLaunchArgsFromEnv preserves explicit model in either syntax', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMG_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore'),
      ['--model', 'gpt-5'],
    );
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMG_TEAM_WORKER_LAUNCH_ARGS: '--model=gpt-5.3' }, 'explore'),
      ['--model', 'gpt-5.3'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv prefers explicit env model over inherited leader model', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMG_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore', 'gpt-4.1'),
      ['--model', 'gpt-5'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv preserves explicit reasoning and logs source=explicit', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        { OMG_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort=\"high\" --no-alt-screen' },
        'explore',
      );
      assert.deepEqual(
        args,
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps gemini thinking_level logging for mixed CLI maps', () => {
    const logs: string[] = [];
    const originalLog = console.log;
    console.log = (...args: unknown[]) => { logs.push(args.join(' ')); };
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMG_TEAM_WORKER_CLI_MAP: 'gemini,claude',
          OMG_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort="high" --model claude-3-7-sonnet',
        },
        'executor',
      );
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps claude and gemini startup logs free of thinking_level during teammate allocation', () => {
    ...
    try {
      ...
      const claudeArgs = resolveWorkerLaunchArgsFromEnv(
        { OMG_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen --model claude-3-7-sonnet' },
        'executor',
        undefined,
        'low',
        'claude',
  ...
  it('waitForClaudeStartupEvidence requires first-start ACK/task progress before startup dispatch is treated as settled', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-claude-startup-'));
    try {
      ...
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'claude-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
  ...
  it('waitForWorkerStartupEvidence ignores Gemini ACK-only startup replies until work is claimed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-gemini-startup-'));
    try {
      await initTeamState('gemini-startup', 'startup evidence test', 'executor', 1, cwd);

      await sendWorkerMessage('gemini-startup', 'worker-1', 'leader-fixed', 'ACK', cwd);
      const ackOnly = await waitForWorkerStartupEvidence({
        teamName: 'gemini-startup',
        workerName: 'worker-1',
        workerCli: 'gemini',
        cwd,
        timeoutMs: 25,
      ...
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'gemini-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
          current_task_id: 'task-1',
          updated_at: new Date().toISOString(),
  ...
  it('waitForWorkerStartupEvidence treats blocked worker status as settled progress even without a claimed task id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-gemini-blocked-startup-'));
    try {
      await initTeamState('gemini-blocked-startup', 'blocked startup evidence test', 'executor', 1, cwd);

      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'gemini-blocked-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'blocked',
      ...
      const progress = await waitForWorkerStartupEvidence({
        teamName: 'gemini-blocked-startup',
        workerName: 'worker-1',
        workerCli: 'gemini',
        cwd,
        timeoutMs: 25,
  ...
  it('startTeam rejects interactive startup when tmux fallback never produces worker startup evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-startup-no-evidence-'));
    ...
    const prevSkipReadyWait = process.env.OMG_TEAM_SKIP_READY_WAIT;
    let receiptFailer: NodeJS.Timeout | null = null;

    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-startup-no-evidence-bin-',
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'interactive';
          process.env.OMG_TEAM_WORKER_CLI = 'gemini';
          process.env.OMG_TEAM_SKIP_READY_WAIT = '1';
      ...
      if (typeof prevSkipReadyWait === 'string') process.env.OMG_TEAM_SKIP_READY_WAIT = prevSkipReadyWait;
      else delete process.env.OMG_TEAM_SKIP_READY_WAIT;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam rejects nested team invocation inside worker context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-'));
    const prev = process.env.OMG_TEAM_WORKER;
    process.env.OMG_TEAM_WORKER = 'alpha/worker-1';
    try {
      await assert.rejects(
  ...
  it('startTeam allows nested team invocation when parent governance enables it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-nested-allow-'));
...
`,
    ...
    const parentManifestPath = join(cwd, '.omg', 'state', 'team', 'parent-team', 'manifest.v2.json');
    ...
    const prevTmux = process.env.TMUX;
    const prevWorker = process.env.OMG_TEAM_WORKER;
    const prevStateRoot = process.env.OMG_TEAM_STATE_ROOT;
    ...
    process.env.OMG_TEAM_WORKER = 'parent-team/worker-1';
    ...
    try {
      ...
      if (typeof prevWorker === 'string') process.env.OMG_TEAM_WORKER = prevWorker;
      else delete process.env.OMG_TEAM_WORKER;
      if (typeof prevStateRoot === 'string') process.env.OMG_TEAM_STATE_ROOT = prevStateRoot;
      else delete process.env.OMG_TEAM_STATE_ROOT;
  ...
  it('startTeam rejects duplicate active same-name team state without mutating existing files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-duplicate-team-'));
    const prevSessionId = process.env.OMG_SESSION_ID;
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    try {
      process.env.OMG_SESSION_ID = 'sess-existing-team';
      await initTeamState(
        'dup-team',
        'existing task',
        'executor',
      ...
      process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'prompt';
      process.env.OMG_SESSION_ID = 'sess-second-team';
      ...
      if (typeof prevSessionId === 'string') process.env.OMG_SESSION_ID = prevSessionId;
  ...
  it('skips interactive worker process-tree prekill on native Windows split-pane sessions', async () => {
    await withNativeWindowsPlatform(async () => {
      ...
      assert.equal(shouldPrekillInteractiveShutdownProcessTrees('omg-team-alpha'), true);
    });
  ...
  it('startTeam accepts native Windows tmux clients even when TMUX env vars are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-win32-no-env-'));
    const prevTmux = process.env.TMUX;
    const prevTmuxPane = process.env.TMUX_PANE;
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-win32-no-env-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('applyCreatedInteractiveSessionToConfig persists worker pane ids before readiness waits', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-pane-persist-race-'));
    try {
  ...
  it('startTeam captures interactive worker pid from the resolved pane id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-pane-pid-'));
    const prevTmux = process.env.TMUX;
    const prevTmuxPane = process.env.TMUX_PANE;
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-pane-pid-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
  ...
  split-window)
    case "$*" in
      *" -h "*)
        mkdir -p "${cwd}/.omg/state/team/team-pane-pid/workers/worker-1"
        cat > "${cwd}/.omg/state/team/team-pane-pid/workers/worker-1/status.json" <<'EOF'
{
  "state": "working",
  "current_task_id": "1",
...
`,
        ...
        async () => {
          ...
          const identityPath = join(cwd, '.omg', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'identity.json');
  ...
  it('startTeam records recoverable startup issues per worker instead of failing launch early when panes stay alive', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-no-startup-evidence-'));
    ...
    const previousSkipReadyWait = process.env.OMG_TEAM_SKIP_READY_WAIT;
    const previousStartupEvidenceTimeout = process.env.OMG_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS;
    let runtime: TeamRuntime | null = null;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-no-startup-evidence-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async () => {
          ...
          process.env.OMG_TEAM_WORKER_CLI = 'gemini';
          process.env.OMG_TEAM_SKIP_READY_WAIT = '1';
          process.env.OMG_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS = '500';
      ...
      if (typeof previousSkipReadyWait === 'string') process.env.OMG_TEAM_SKIP_READY_WAIT = previousSkipReadyWait;
      else delete process.env.OMG_TEAM_SKIP_READY_WAIT;
      if (typeof previousStartupEvidenceTimeout === 'string') {
        process.env.OMG_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS = previousStartupEvidenceTimeout;
      } else {
        delete process.env.OMG_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS;
      }
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam still fails startup when the worker pane is dead/unrecoverable', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-dead-startup-pane-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-dead-startup-pane-bin-',
          tmuxScript: () => `#!/bin/sh
set -eu
case "$1" in
  -V)
...
`,
          binaries: [{ name: 'gemini', content: '#!/usr/bin/env node\nprocess.stdin.resume();\n' }],
        },
        async () => {
          delete process.env.TMUX;
          process.env.TMUX_PANE = '%1';
  ...
  it('startTeam materializes all worker identity/inbox files before worker-1 startup evidence can block later workers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-materialize-before-evidence-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-materialize-before-evidence-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async () => {
          ...
          const workerOneIdentity = join(cwd, '.omg', 'state', 'team', sanitizedTeamName, 'workers', 'worker-1', 'identity.json');
          const workerTwoIdentity = join(cwd, '.omg', 'state', 'team', sanitizedTeamName, 'workers', 'worker-2', 'identity.json');
          const workerTwoInbox = join(cwd, '.omg', 'state', 'team', sanitizedTeamName, 'workers', 'worker-2', 'inbox.md');
          ...
          assert.equal(
            existsSync(join(cwd, '.omg', 'state', 'team', sanitizedTeamName)),
            false,
          );
        },
      );
  ...
  it('startTeam rejects dirty leader workspace before provisioning worker worktrees', async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        /leader_workspace_dirty_for_worktrees:.*M README\.md.*\?\? notes\.txt.*commit_or_stash_before_omg_team/s,
      );
      ...
      assert.equal(existsSync(join(repo, '.omg', 'state', 'team', 'team-dirty-preflight')), false);
    } finally {
      if (typeof prevLaunchMode === 'string') process.env.OMG_TEAM_WORKER_LAUNCH_MODE = prevLaunchMode;
      else delete process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
  ...
  it('startTeam launches gemini workers with startup prompt and no default model passthrough', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-gemini-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
...
`,
    ...
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    const prevLaunchArgs = process.env.OMG_TEAM_WORKER_LAUNCH_ARGS;
    const prevCapture = process.env.OMG_GEMINI_ARGV_CAPTURE_PATH;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMG_TEAM_WORKER_CLI = 'gemini';
    process.env.OMG_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.3-gemini-spark';
    process.env.OMG_GEMINI_ARGV_CAPTURE_PATH = capturePath;
    ...
    try {
      ...
      const expectedArgv = [
        '-i',
        'Read .omg/state/team/team-gemini-prompt/workers/worker-1/inbox.md, start work now, report concrete progress, then continue assigned work or next feasible task.',
      ];
      ...
      if (typeof prevLaunchArgs === 'string') process.env.OMG_TEAM_WORKER_LAUNCH_ARGS = prevLaunchArgs;
      else delete process.env.OMG_TEAM_WORKER_LAUNCH_ARGS;
      if (typeof prevCapture === 'string') process.env.OMG_GEMINI_ARGV_CAPTURE_PATH = prevCapture;
      else delete process.env.OMG_GEMINI_ARGV_CAPTURE_PATH;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam rejects gemini prompt mode even when explicit launch args are provided', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-gemini-explicit-launch-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
`,
    ...
    process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMG_TEAM_WORKER_CLI = 'gemini';
    process.env.OMG_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.3-gemini-spark -c model_reasoning_effort="low"';
    try {
      await assert.rejects(
        () => withoutTeamWorkerEnv(() =>
          startTeam(
            'team-gemini-explicit-launch',
            'gemini prompt-mode team bootstrap',
            'explore',
            1,
            [{ subject: 's', description: 'd', owner: 'worker-1' }],
        ...
        /prompt_mode_gemini_requires_tty/,
      );
      assert.equal(existsSync(join(cwd, '.omg', 'state', 'team', 'team-gemini-explicit-launch')), false);
    } finally {
      if (typeof prevPath === 'string') process.env.PATH = prevPath;
      else delete process.env.PATH;
      if (typeof prevTmux === 'string') process.env.TMUX = prevTmux;
  ...
  it('startTeam preserves routed task roles into team state and worker launch args', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-role-routing-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.gemini', 'prompts');
    await mkdir(binDir, { recursive: true });
...
const fs = require('fs');
const path = require('path');
const worker = String(process.env.OMG_TEAM_WORKER || 'unknown').replace(/[^a-zA-Z0-9_-]+/g, '__');
const out = path.join(process.env.OMG_ARGV_CAPTURE_DIR, worker + '.json');
...
`,
    ...
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    const prevCaptureDir = process.env.OMG_ARGV_CAPTURE_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMG_TEAM_WORKER_CLI = 'gemini';
    process.env.OMG_ARGV_CAPTURE_DIR = captureDir;

    let runtime: TeamRuntime | null = null;
    try {
      runtime = await withMockPromptModeGeminiAllowed(() =>
        withoutTeamWorkerEnv(() =>
          startTeam(
            'team-role-routing',
      ...
      const worker1Instructions = await readFile(join(cwd, '.omg', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'GEMINI.md'), 'utf-8');
      const worker2Instructions = await readFile(join(cwd, '.omg', 'state', 'team', runtime.teamName, 'workers', 'worker-2', 'GEMINI.md'), 'utf-8');
      ...
      if (typeof prevCaptureDir === 'string') process.env.OMG_ARGV_CAPTURE_DIR = prevCaptureDir;
      else delete process.env.OMG_ARGV_CAPTURE_DIR;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam does not apply mini guidance for exact-match negatives like gpt-5.4-mini-tuned', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-mini-tuned-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.gemini', 'prompts');
...
`,
    ...
    process.env.OMG_TEAM_WORKER_CLI = 'gemini';
    process.env.OMG_ARGV_CAPTURE_DIR = captureDir;
    process.env.OMG_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.4-mini-tuned';
    ...
    try {
      ...
      const workerInstructions = await readFile(join(cwd, '.omg', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'GEMINI.md'), 'utf-8');
  ...
  it('startTeam rejects gemini prompt mode without tmux with an explicit non-tty error', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(cwd, '.omg', 'state', 'team', 'team-prompt')), false);
    } finally {
      if (typeof prevPath === 'string') process.env.PATH = prevPath;
      else delete process.env.PATH;
      if (typeof prevTmux === 'string') process.env.TMUX = prevTmux;
  ...
  it('startTeam relaunch re-creates HUD pane and re-registers reconcile hooks after shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-relaunch-hud-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-relaunch-hud-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('startTeam routes detached worktree worker inbox and mailbox triggers through leader-root state references', async () => {
    const repo = await initRepo();
    const toolingDir = await mkdtemp(join(tmpdir(), 'omg-runtime-worktree-tools-'));
...
const fs = require('fs');
const path = require('path');
const logDir = process.env.OMG_TEST_LOG_DIR;
fs.mkdirSync(logDir, { recursive: true });
fs.writeFileSync(path.join(logDir, 'env.json'), JSON.stringify({
  cwd: process.cwd(),
  teamStateRoot: process.env.OMG_TEAM_STATE_ROOT || '',
  worker: process.env.OMG_TEAM_WORKER || '',
}));
...
`,
    ...
    const prevLaunchMode = process.env.OMG_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMG_TEAM_WORKER_CLI;
    const prevLogDir = process.env.OMG_TEST_LOG_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMG_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMG_TEAM_WORKER_CLI = 'gemini';
    process.env.OMG_TEST_LOG_DIR = logDir;
    ...
    try {
      ...
      const workerAgents = await readFile(join(workerPath as string, 'GEMINI.md'), 'utf-8');
      ...
      assert.match(
        startupLog,
        /\$OMG_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      assert.doesNotMatch(
        startupLog,
        /Read \.omg\/state\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      ...
      assert.equal(envLog.teamStateRoot, join(repo, '.omg', 'state'));
      assert.equal(envLog.worker, 'team-detached-worktree-paths/worker-1');
      const rootAgents = await readFile(join(workerPath, 'GEMINI.md'), 'utf-8');
      ...
      assert.match(
        mailboxLog,
        /\$OMG_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/mailbox\/worker-1\.json/,
      );
      ...
      if (typeof prevLogDir === 'string') process.env.OMG_TEST_LOG_DIR = prevLogDir;
      else delete process.env.OMG_TEST_LOG_DIR;
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(worktreePath as string, 'GEMINI.md')), true);

      await shutdownTeam(runtime.teamName, repo);
      runtime = null;

      assert.equal(existsSync(worktreePath as string), false);
      assert.equal(existsSync(join(repo, '.omg', 'state', 'team', 'team-detached-worktree-shutdown')), false);
    } finally {
      if (runtime) {
        await shutdownTeam(runtime.teamName, repo, { force: true }).catch(() => {});
      }
  ...
  it('resumeTeam preserves detached worktree metadata for live prompt workers', async () => {
    const repo = await initRepo();
    const binDir = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-bin-'));
    const fakeGeminiPath = join(binDir, 'gemini');
    const logDir = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-logs-'));
    const envLogPath = join(logDir, 'env.json');
    await writeFile(
      fakeGeminiPath,
      `#!/usr/bin/env node
...
`,
    ...
    try {
      ...
      assert.equal(resumed?.config.workers[0]?.team_state_root, join(repo, '.omg', 'state'));
      ...
      assert.equal(identity.team_state_root, join(repo, '.omg', 'state'));
  ...
  it('shutdownTeam force-kills prompt workers that ignore SIGTERM', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-stubborn-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
    await mkdir(binDir, { recursive: true });
...
`,
    ...
    let workerPid = 0;
    try {
      runtime = await withPromptModeGeminiEnv(binDir, {}, () =>
        withoutTeamWorkerEnv(() =>
          startTeam(
  ...
  it('shutdownTeam reaps detached prompt-worker descendants', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-descendants-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
...
helper.unref();
writeFileSync(process.env.OMG_HELPER_PID_PATH, String(helper.pid));
process.stdin.resume();
setInterval(() => {}, 1000);
process.on('SIGTERM', () => process.exit(0));
`,
    ...
    let helperPid = 0;
    try {
      runtime = await withPromptModeGeminiEnv(binDir, { OMG_HELPER_PID_PATH: helperPidPath }, () =>
        withoutTeamWorkerEnv(() =>
          startTeam(
  ...
  it('monitorTeam surfaces reclaimed work pickup attempts when an idle worker is available', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-reassign-reclaimed-'));
    ...
    try {
      ...
      const taskPath = join(cwd, '.omg', 'state', 'team', 'team-runtime-reassign', 'tasks', `task-${task.id}.json`);
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-runtime-reassign', 'manifest.v2.json');
      ...
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'working', current_task_id: task.id, updated_at: new Date().toISOString() }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-2', 'status.json'),
  ...
  it('monitorTeam reclaims expired task claims and surfaces the recovery in recommendations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-reclaim-'));
    const prevTeamStateRoot = process.env.OMG_TEAM_STATE_ROOT;
    delete process.env.OMG_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omg', 'state', 'team', 'team-runtime-reclaim', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeAtomic(taskPath, JSON.stringify(current, null, 2));
  ...
  it('monitorTeam keeps phase in team-verify when completed code tasks lack verification evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-verify-gate-'));
    const prevTeamStateRoot = process.env.OMG_TEAM_STATE_ROOT;
    delete process.env.OMG_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omg', 'state', 'team', 'team-verify-gate', 'tasks', `task-${task.id}.json`);
  ...
  it('monitorTeam deactivates root team-state.json when the local phase becomes terminal', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-root-team-state-'));
    try {
      ...
      const rootStatePath = join(cwd, '.omg', 'state', 'team-state.json');
  ...
  it('monitorTeam emits worker_state_changed, worker_idle, and task_completed events based on transitions', async () => {
    ...
    try {
      ...
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'team-events', 'tasks', `task-${t.id}.json`),
        JSON.stringify({ ...t, status: 'completed', owner: 'worker-1' }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'team-events', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'idle', updated_at: new Date().toISOString() }, null, 2),
      );

      await monitorTeam('team-events', cwd);

      const eventsPath = join(cwd, '.omg', 'state', 'team', 'team-events', 'events', 'events.ndjson');
  ...
  it('monitorTeam persists integration ledger and cherry-picks unseen worker HEADs once', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'worker-1-branch', 'omg-runtime-worker-1-wt-');
  ...
  it('monitorTeam auto-commits dirty worker worktree before integration', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-ac-branch', 'omg-runtime-wk1-auto-commit-');
      ...
      assert.match(log, /omg\(team\): auto-checkpoint worker-1 \[1\]/, 'commit message should match auto-checkpoint pattern');
      ...
      const ledgerPath = join(repo, '.omg', 'reports', 'team-commit-hygiene', 'team-auto-commit.ledger.json');
  ...
  it('monitorTeam uses merge for worker cleanly ahead of leader (hybrid merge path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-merge-branch', 'omg-runtime-wk1-merge-clean-');
  ...
  it('monitorTeam integrates detached worker worktree by commit sha instead of merging leader HEAD into itself', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-detached-merge-branch', 'omg-runtime-wk1-detached-merge-');
      ...
      const ledgerPath = join(repo, '.omg', 'reports', 'team-commit-hygiene', 'team-merge-detached.ledger.json');
      const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8')) as {
        entries: Array<{
          operation: string;
          status: string;
  ...
  it('monitorTeam does not emit INTEGRATED when merge reports success but leader HEAD never advances', async () => {
    const repo = await initRepo();
    let workerPath = '';
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-runtime-fake-git-'));
    const previousPath = process.env.PATH;
    const previousFakeMode = process.env.OMG_FAKE_GIT_SUCCESS_NOOP;
    try {
      workerPath = await addWorktree(repo, 'wk1-merge-noadvance-branch', 'omg-runtime-wk1-merge-noadvance-');
      await writeFile(join(workerPath, 'feature.txt'), 'new feature\n', 'utf-8');
      execFileSync('git', ['add', 'feature.txt'], { cwd: workerPath, stdio: 'ignore' });
      execFileSync('git', ['commit', '-m', 'worker feature'], { cwd: workerPath, stdio: 'ignore' });
      ...
      await writeFile(
        ...
        `#!/usr/bin/env bash
set -euo pipefail
if [[ "\${OMG_FAKE_GIT_SUCCESS_NOOP:-}" == "merge" && "\${1:-}" == "merge" ]]; then
  exit 0
fi
exec "${realGit}" "$@"
`,
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath ?? ''}`;
      process.env.OMG_FAKE_GIT_SUCCESS_NOOP = 'merge';
      ...
      if (typeof previousFakeMode === 'string') process.env.OMG_FAKE_GIT_SUCCESS_NOOP = previousFakeMode;
      else delete process.env.OMG_FAKE_GIT_SUCCESS_NOOP;
      await rm(fakeBinDir, { recursive: true, force: true });
      if (workerPath) {
        await rm(workerPath, { recursive: true, force: true });
      }
  ...
  it('monitorTeam uses cherry-pick for diverged worker (hybrid cherry-pick path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-div-branch', 'omg-runtime-wk1-diverged-');
  ...
  it('monitorTeam rebases idle workers after integration lands on leader (cross-worker rebase)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-xr-branch', 'omg-runtime-wk1-cross-rebase-');
      worker2Path = await addWorktree(repo, 'wk2-xr-branch', 'omg-runtime-wk2-cross-rebase-');
      ...
      const ledgerPath = join(repo, '.omg', 'reports', 'team-commit-hygiene', 'team-cross-rebase.ledger.json');
      const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8')) as {
  ...
  it('monitorTeam auto-resolves conflicts with -X theirs (worker wins on leader)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-cr-branch', 'omg-runtime-wk1-conflict-resolve-');
  ...
  it('monitorTeam skips rebase for workers with "working" status (idle-only gating)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-gate-branch', 'omg-runtime-wk1-rebase-gate-');
      worker2Path = await addWorktree(repo, 'wk2-gate-branch', 'omg-runtime-wk2-rebase-gate-');
  ...
  it('monitorTeam aborts failed rebase and leaves worktree in clean state', async () => {
    ...
    try {
      ...
      worker1Path = await addWorktree(repo, 'wk1-rf-branch', 'omg-runtime-wk1-rebase-fail-');
      worker2Path = await addWorktree(repo, 'wk2-rf-branch', 'omg-runtime-wk2-rebase-fail-');
      ...
      const reportPath = join(repo, '.omg', 'state', 'team', 'team-rebase-fail', 'integration-report.md');
  ...
  it('shutdownTeam cleans up state even when tmux session doesn\'t exist', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown');
  ...
  it('shutdownTeam clean fast path ignores worker shutdown ack files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-clean-fast-'));
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-clean-fast');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam blocks when pending tasks remain (shutdown gate)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-pending');
  ...
  it('shutdownTeam honors governance cleanup override when active tasks remain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-override-'));
    try {
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-override', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.governance = {
        ...(manifest.governance || {}),
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-override');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam honors legacy policy cleanup override after governance hydration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-legacy-'));
    try {
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-legacy', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.policy = {
        ...(manifest.policy || {}),
        cleanup_requires_all_workers_inactive: false,
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-legacy');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam requires explicit issue confirmation when failed tasks remain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-failed-'));
    try {
      ...
      await assert.rejects(
        ...
        /shutdown_confirm_issues_required:failed=1:rerun=omg team shutdown team-shutdown-gate-failed --confirm-issues/,
      );
  ...
  it('shutdownTeam force=true bypasses shutdown gate and cleans up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-force-'));
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-gate-force');
  ...
  it('shutdownTeam force=true emits shutdown_gate_forced audit event', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-gate-forced-event-'));
    try {
      ...
      const eventsPath = join(cwd, '.omg', 'state', 'team', 'team-gate-forced-event', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-gate-forced-event');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam handles persisted resize hook metadata during cleanup', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-resize-meta-'));
    try {
      const configPath = join(cwd, '.omg', 'state', 'team', 'team-resize-meta', 'config.json');
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-resize-meta', 'manifest.v2.json');
      ...
      config.resize_hook_name = 'omg_resize_team_resize_meta_test';
      config.resize_hook_target = 'omg-team-team-resize-meta:0';
      await writeFile(configPath, JSON.stringify(config, null, 2));
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
      manifest.resize_hook_name = 'omg_resize_team_resize_meta_test';
      manifest.resize_hook_target = 'omg-team-team-resize-meta:0';
      await writeFile(manifestPath, JSON.stringify(manifest, null, 2));

      await shutdownTeam('team-resize-meta', cwd);
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-resize-meta');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam continues cleanup when resize hook unregister fails while session remains active', async () => {
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-fake-tmux-',
...
case "$1" in
  -V)
    ...
    ;;
  list-sessions)
    echo "omg-team-team-shutdown-gate-failed"
    exit 0
    ;;
  set-hook)
    if [ "\${2:-}" = "-u" ]; then
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const config = JSON.parse(await readFile(configPath, 'utf-8')) as Record<string, unknown>;
          config.tmux_session = 'omg-team-team-shutdown-gate-failed';
          config.resize_hook_name = 'omg_resize_team_shutdown_gate_failed_test';
          config.resize_hook_target = 'omg-team-team-shutdown-gate-failed:0';
          await writeFile(configPath, JSON.stringify(config, null, 2));
          const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
          manifest.tmux_session = 'omg-team-team-shutdown-gate-failed';
          manifest.resize_hook_name = 'omg_resize_team_shutdown_gate_failed_test';
          manifest.resize_hook_target = 'omg-team-team-shutdown-gate-failed:0';
          await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
          ...
          const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
          assert.match(tmuxLog, /set-hook -u -t omg-team-team-shutdown-gate-failed:0 client-resized\[\d+\]/);
          assert.match(tmuxLog, /kill-session -t omg-team-team-shutdown-gate-failed/);
        },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
  ...
  it('shutdownTeam emits shutdown_ack event when worker ack is received', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omg', 'state', 'team', 'team-ack-evt', 'events', 'events.ndjson');
  ...
  it('shutdownTeam emits shutdown_ack event with accept reason for accepted acks', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omg', 'state', 'team', 'team-ack-accept', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-ack-accept');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam force=true ignores rejection and cleans up team state', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-force');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam ignores stale rejection ack from a prior request', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-stale-ack');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam confirmIssues=true allows failed-task shutdown without worker ack handshake', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-confirm-issues-'));
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-confirm-issues');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam applies best-effort teardown even when worker pane is already dead', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-dead-pane-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-shutdown-dead-pane-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam reconciles persisted worker panes with live tmux panes before teardown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-pane-reconcile-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-shutdown-pane-reconcile-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
    ...
    case "$*" in
      ...
      *"-t leader:0 -F #{pane_id}"*"#{pane_current_command}"*)
        ...
        if [ -f "$restored_marker" ]; then
          printf "%%44\\tnode\\tnode /tmp/bin/omg.js hud --watch\\n"
        fi
        exit 0
        ;;
      *)
...
`,
  ...
  it('shutdownTeam skips prekill and keeps the leader pane alive on native Windows split-pane shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-win32-split-'));
    try {
      await withNativeWindowsPlatform(async () => {
        await withMockTmuxFixture(
          {
            dirPrefix: 'omg-runtime-shutdown-win32-split-bin-',
            tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
          ...
          async ({ tmuxLogPath }) => {
            ...
            const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-win32-split');
            assert.equal(existsSync(teamRoot), false);
  ...
  it('shutdownTeam reconciles stale leader and hud pane ids before native Windows split-pane teardown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-win32-stale-topology-'));
    const teamName = 'team-win32-stale-topo';
    try {
      await withNativeWindowsPlatform(async () => {
        await withMockTmuxFixture(
          {
            dirPrefix: 'omg-runtime-shutdown-win32-stale-topology-bin-',
            tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
  ...
  list-panes)
    case "$*" in
      *"-F #{pane_dead} #{pane_pid}"*)
        ...
        ;;
      *"-t leader:0 -F #{pane_id}"*"#{pane_current_command}"*)
        printf "%%21\\tpwsh\\tpwsh\\n%%22\\tnode\\tnode /tmp/bin/omg.js hud --watch\\n%%23\\tgemini\\tgemini\\n%%24\\tgemini\\tgemini\\n"
        exit 0
        ;;
      *)
        exit 1
...
`,
  ...
  it('shutdownTeam skips prekill and keeps the leader pane alive on shared-session shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-shared-session-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-shutdown-shared-session-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-shutdown-shared-session');
          assert.equal(existsSync(teamRoot), false);
  ...
  it('shutdownTeam restores a standalone HUD pane after tearing down the team HUD', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-restore-hud-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-shutdown-restore-hud-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam preserves leader exclusion while tearing down the hud pane', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-shutdown-exclusions-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-shutdown-exclusions-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          assert.ok(config);
          if (!config) return;
          config.tmux_session = 'omg-team-team-shutdown-exclusions';
          config.leader_pane_id = '%11';
          config.hud_pane_id = '%12';
  ...
  it('shutdownTeam still requires confirm-issues on failed tasks', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-normal-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omg', 'state', 'team', 'team-normal-gate');
      assert.equal(existsSync(teamRoot), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('resumeTeam returns null for prompt teams when worker handles are missing after restart', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-prompt-resume-'));
    ...
    const prevLeaderCwd = process.env.OMG_TEAM_LEADER_CWD;
    delete process.env.OMG_TEAM_STATE_ROOT;
    delete process.env.OMG_TEAM_LEADER_CWD;

    try {
      await initTeamState('team-prompt-resume', 'prompt resume test', 'executor', 1, cwd);
      const configPath = join(cwd, '.omg', 'state', 'team', 'team-prompt-resume', 'config.json');
  ...
  it('assignTask enforces delegation_only policy for leader-fixed worker', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-delegation', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('assignTask rolls back claim when inbox write fails after claim', async () => {
    ...
    try {
      ...
      const workerDir = join(cwd, '.omg', 'state', 'team', 'team-inbox-fail', 'workers', 'worker-1');
  ...
  it('assignTask enforces plan approval for code-change tasks when required', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-approval', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('monitorTeam does not re-notify already-notified mailbox messages (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-no-spam-'));
    try {
      ...
      const mailboxDir = join(cwd, '.omg', 'state', 'team', 'team-no-spam', 'mailbox');
  ...
  it('monitorTeam only notifies once per new message even without notified_at (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-new-msg-'));
    try {
      await initTeamState('team-new-msg', 'new msg test', 'executor', 1, cwd);

      const mailboxDir = join(cwd, '.omg', 'state', 'team', 'team-new-msg', 'mailbox');
      await mkdir(mailboxDir, { recursive: true });
  ...
  it('monitorTeam does not emit duplicate task_completed when transitionTaskStatus completed the task first (issue #161)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-no-dup-'));
    try {
      ...
      const eventsPath = join(cwd, '.omg', 'state', 'team', 'team-no-dup', 'events', 'events.ndjson');
      const content = await readFile(eventsPath, 'utf-8');
  ...
  it('sendWorkerMessage keeps hook-preferred duplicate leader mailbox sends idempotent after notification', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-leader-dedupe-notified-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-leader-dedupe-notified-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path persists leader mailbox guidance when leader pane exists', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-leader-inject-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-leader-inject-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage keeps failed hook receipts failed when fallback mailbox persistence confirms delivery', { concurrency: false }, async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-leader-failed-receipt-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omg-runtime-leader-failed-receipt-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path for leader waits for receipt then falls back to mailbox persistence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-leader-hook-'));
    try {
  ...
  it('sendWorkerMessage transport_direct keeps leader-fixed request pending when leader_pane_id missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-leader-direct-'));
    try {
      ...
      const manifestPath = join(cwd, '.omg', 'state', 'team', 'team-leader-direct', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8'));

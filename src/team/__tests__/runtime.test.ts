...
function teamStateTestPath(cwd: string, ...parts: string[]): string {
  const stateRoot = process.env.OMX_TEAM_STATE_ROOT ?? join(cwd, '.omg', 'state');
...
describe('runtime', () => {
  ...
  it('resolveWorkerLaunchArgsFromEnv reads low-complexity model from config when present', async () => {
    ...
    const tempGeminiHome = await mkdtemp(join(tmpdir(), 'omg-codex-home-'));
    await writeFile(
  ...
  it('waitForWorkerStartupEvidence ignores Gemini ACK-only startup replies until work is claimed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-codex-startup-'));
    try {
      ...
      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'codex-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
          current_task_id: 'task-1',
          updated_at: new Date().toISOString(),
  ...
  it('waitForWorkerStartupEvidence treats blocked worker status as settled progress even without a claimed task id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-codex-blocked-startup-'));
    try {
      await initTeamState('codex-blocked-startup', 'blocked startup evidence test', 'executor', 1, cwd);

      await writeAtomic(
        join(cwd, '.omg', 'state', 'team', 'codex-blocked-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'blocked',
...
`,
  ...
  it('startTeam rejects codex prompt mode even when explicit launch args are provided', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-codex-explicit-launch-'));
    const binDir = join(cwd, 'bin');
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(cwd, '.omg', 'state', 'team', 'team-codex-explicit-launch')), false);
    } finally {
      if (typeof prevPath === 'string') process.env.PATH = prevPath;
      else delete process.env.PATH;
      if (typeof prevTmux === 'string') process.env.TMUX = prevTmux;
...
`,
    ...
    let helperPid = 0;
    try {
      runtime = await withPromptModeGeminiEnv(binDir, { OMX_HELPER_PID_PATH: helperPidPath }, () =>
        withoutTeamWorkerEnv(() =>
          startTeam(

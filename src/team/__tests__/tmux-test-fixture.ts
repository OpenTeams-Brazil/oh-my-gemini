...
export async function withTempTmuxSession<T>(
  ...
  const fixtureCwd = await mkdtemp(join(tmpdir(), 'omg-tmux-fixture-'));
  const sessionName = uniqueTmuxIdentifier('omg-test');
  const serverName = options.useAmbientServer ? '' : uniqueTmuxIdentifier('omg-fixture');

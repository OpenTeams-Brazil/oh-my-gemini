...
async function initCommittedGitRepo(cwd: string): Promise<void> {
  ...
  execFileSync('git', ['config', 'user.email', 'omg@example.com'], { cwd, stdio: 'pipe' });
  execFileSync('git', ['add', '.'], { cwd, stdio: 'pipe' });
  execFileSync('git', ['commit', '-m', 'init'], { cwd, stdio: 'pipe' });
}

async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-worktree-repo-'));
...
describe('WorkerStatus draining state', () => {
  it('writeWorkerStatus writes draining status and readWorkerStatus reads it back', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-drain-'));
    try {
  ...
  it('readWorkerStatus returns unknown for non-existent worker', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-nw-'));
    try {
...
describe('Monotonic worker index counter', () => {
  it('initTeamState sets next_worker_index to workerCount + 1', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-idx-'));
    try {
      ...
      const diskCfg = JSON.parse(
        readFileSync(join(cwd, '.omg', 'state', 'team', 'idx-test', 'config.json'), 'utf8'),
  ...
  it('next_worker_index is present in manifest.v2.json', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-manif-'));
    try {
      await initTeamState('manif-test', 'task', 'executor', 2, cwd);
      const manifest = JSON.parse(
        readFileSync(join(cwd, '.omg', 'state', 'team', 'manif-test', 'manifest.v2.json'), 'utf8'),
      ) as { next_worker_index?: number };
  ...
  it('readTeamConfig preserves next_worker_index', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-read-'));
    try {
...
describe('withScalingLock', () => {
  it('acquires and releases lock for successful operations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-lock-'));
    try {
      await initTeamState('lock-test', 'task', 'executor', 1, cwd);
      const lockDir = join(cwd, '.omg', 'state', 'team', 'lock-test', '.lock.scaling');
  ...
  it('releases lock even when function throws', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-lock-err-'));
    try {
      await initTeamState('lock-err', 'task', 'executor', 1, cwd);
      const lockDir = join(cwd, '.omg', 'state', 'team', 'lock-err', '.lock.scaling');
  ...
  it('serializes concurrent operations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-lock-con-'));
    try {
...
describe('scaleUp', () => {
  ...
  it('persists scaled-up task roles in canonical task state and inbox ids', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-role-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-role-bin-'));
    ...
    try {
      ...
      await mkdir(join(cwd, '.gemini', 'prompts'), { recursive: true });
      await writeFile(join(cwd, '.gemini', 'prompts', 'writer.md'), '<identity>You are Writer.</identity>');
      await mkdir(join(cwd, '.omg', 'state', 'team', 'scale-up-role'), { recursive: true });
      await writeFile(join(cwd, '.omg', 'state', 'team', 'scale-up-role', 'worker-agents.md'), '# Base worker instructions\n');
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-scale-up-role';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'scale-up-role', 'manifest.v2.json');
      ...
      const workerIdentity = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'scale-up-role', 'workers', 'worker-2', 'identity.json'), 'utf-8')) as { role?: string };
      assert.equal(workerIdentity.role, 'writer');

      const inbox = await readFile(join(cwd, '.omg', 'state', 'team', 'scale-up-role', 'workers', 'worker-2', 'inbox.md'), 'utf-8');
  ...
  it('removes generated worktree-root AGENTS when scale-up rolls back', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-rollback-worktree-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-rollback-worktree-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      await mkdir(join(cwd, '.gemini', 'prompts'), { recursive: true });
      await writeFile(join(cwd, '.gemini', 'prompts', 'writer.md'), '<identity>You are Writer.</identity>');
      await writeFile(join(cwd, 'GEMINI.md'), '# Root project instructions\n');
      await initCommittedGitRepo(cwd);
      await initTeamState('rollback-worktree', 'task', 'executor', 1, cwd, undefined, process.env, {
        workspace_mode: 'worktree',
        leader_cwd: cwd,
        team_state_root: join(cwd, '.omg', 'state'),
      });

      const config = await readTeamConfig('rollback-worktree', cwd);
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-rollback-worktree';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'rollback-worktree', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerRootAgents = join(cwd, '.omg', 'team', 'rollback-worktree', 'worktrees', 'worker-2', 'GEMINI.md');
      assert.equal(await readFile(workerRootAgents, 'utf-8'), '# Root project instructions\n');
      const backupPath = join(cwd, '.git', 'worktrees', 'worker-2', 'omg', 'root-agents-backup.json');
  ...
  it('uses canonical root AGENTS bootstrap for scaled worktree workers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-canonical-root-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-canonical-root-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-canonical-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'canonical-root', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const inbox = await readFile(join(cwd, '.omg', 'state', 'team', 'canonical-root', 'workers', 'worker-2', 'inbox.md'), 'utf-8');
      assert.doesNotMatch(inbox, /## Your Specialization/);
      assert.match(inbox, /\*\*Role:\*\* writer/);

      const rootAgents = await readFile(join(cwd, '.omg', 'team', 'canonical-root', 'worktrees', 'worker-2', 'GEMINI.md'), 'utf-8');
  ...
  it('does not apply mini guidance during scale-up when the final worker model is gpt-5.4', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-frontier-role-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-frontier-role-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      await mkdir(join(cwd, '.gemini', 'prompts'), { recursive: true });
      await writeFile(join(cwd, '.gemini', 'prompts', 'test-engineer.md'), '<identity>Test Engineer</identity>');
      await mkdir(join(cwd, '.omg', 'state', 'team', 'frontier-role'), { recursive: true });
      await writeFile(join(cwd, '.omg', 'state', 'team', 'frontier-role', 'worker-agents.md'), '# Base worker instructions\n');
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-frontier-role';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'frontier-role', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerAgents = await readFile(join(cwd, '.omg', 'state', 'team', 'frontier-role', 'workers', 'worker-2', 'GEMINI.md'), 'utf-8');
  ...
  it('does not apply mini guidance during scale-up for gpt-5.4-mini-tuned overrides', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-mini-tuned-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-mini-tuned-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-mini-tuned-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'mini-tuned-root', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const rootAgents = await readFile(join(cwd, '.omg', 'team', 'mini-tuned-root', 'worktrees', 'worker-2', 'GEMINI.md'), 'utf-8');
      assert.match(rootAgents, /You are operating as the \*\*writer\*\* role/);
      assert.match(rootAgents, /<identity>You are Writer\.<\/identity>/);
  ...
  it('preserves leader/HUD layout by avoiding tiled relayout during scale-up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-up-layout-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-layout-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omg-team-scale-up-layout';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.omg', 'state', 'team', 'scale-up-layout', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
  ...
  it('provisions detached worktrees for scaled-up workers from persisted team worktree mode', async () => {
    ...
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-detached-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    ...
    try {
      ...
      await mkdir(join(repo, '.omg', 'state', 'team', teamName), { recursive: true });
      await writeFile(join(repo, '.omg', 'state', 'team', teamName, 'worker-agents.md'), '# Base worker instructions\n');
      await initTeamState(
        teamName,
        'task',
        'executor',
        ...
        {
          leader_cwd: repo,
          team_state_root: join(repo, '.omg', 'state'),
          workspace_mode: 'worktree',
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = `omg-team-${teamName}`;
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, repo);

      const manifestPath = join(repo, '.omg', 'state', 'team', teamName, 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
  ...
  it('provisions named worktrees for scaled-up workers from persisted team worktree mode', async () => {
    const repo = await initRepo();
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-up-named-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
describe('scaleDown', () => {
  ...
  it('returns error when team not found', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-nf-'));
    try {
  ...
  it('returns error when trying to remove all workers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-all-'));
    try {
  ...
  it('returns error for worker not in team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-miss-'));
    try {
  ...
  it('returns error when not enough idle workers and force=false', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-busy-'));
    try {
...
describe('scaleDown worktree AGENTS cleanup', () => {
  it('removes generated worktree-root AGENTS during scale-down', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-worktree-agents-'));
    try {
      ...
      const worktree = join(cwd, '.omg', 'team', 'scale-down-worktree', 'worktrees', 'worker-2');
      await mkdir(worktree, { recursive: true });
      await writeFile(join(worktree, 'GEMINI.md'), '# Tracked root instructions\n', 'utf8');
      await mkdir(join(cwd, '.omg', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2'), { recursive: true });
      await writeFile(
        join(cwd, '.omg', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2', 'root-agents-backup.json'),
        JSON.stringify({ existed: true, tracked: false, previousContent: '# Tracked root instructions\n' }, null, 2),
        'utf8',
      );
      await writeFile(join(worktree, 'GEMINI.md'), '# Generated runtime instructions\n', 'utf8');
      ...
      assert.equal(await readFile(join(worktree, 'GEMINI.md'), 'utf-8'), '# Tracked root instructions\n');
      assert.equal(existsSync(join(cwd, '.omg', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2', 'root-agents-backup.json')), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
describe('scaleDown teardown hardening', () => {
  it('scaleDown removes workers when pane is already dead or missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-dead-'));
    try {
  ...
  it('scaleDown never targets leader or hud panes during teardown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-scale-down-exclusions-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omg-scale-down-fake-tmux-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    try {

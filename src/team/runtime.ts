...
import { geminiPromptsDir } from '../utils/paths.js';
...
async function assertTeamStartupIsNonDestructive(
  ...
  const tmuxSession = existingConfig?.tmux_session ?? existingManifest?.tmux_session ?? `omg-team-${teamName}`;
  ...
  throw new Error(
    ...
    + `Use "omg team status ${teamName}", "omg team resume ${teamName}", or "omg team shutdown ${teamName}" instead of launching a duplicate team.`,
  );
}
...
function summarizeWorktreeDiffWithSparkShell(worktreePath: string): string | null {
  ...
  const result = runCommand('omg', ['sparkshell', 'sh', '-lc', shellCommand], worktreePath);
...
function autoCommitDirtyWorktree(
  ...
  const msg = `omg(team): auto-checkpoint ${worker.name} [${taskId}]`;
...
async function integrateWorkerCommitsIntoLeader(params: {
  ...
  for (const worker of config.workers) {
    ...
    if (workerIsAheadOfLeader) {
      ...
      const merge = runGitCommand(repoRoot, ['merge', '--no-ff', '-X', 'theirs', '-m', `omg(team): merge ${worker.name}`, branchRef], cwd);
...
async function prepareShutdownMergeReport(
  ...
  if (hadChanges) {
    ...
    if (!addResult.ok) {
      return {
        workerName: worker.name,
        worktreePath,
        reportPath: join(worktreePath, '.omg', 'diff.md'),
        sourceRef: null,
    ...
    const commitResult = runGitCommand(
      repoRoot,
      ['commit', '--no-verify', '-m', `omg(team): checkpoint ${worker.name} shutdown changes`],
      worktreePath,
    );
    if (commitResult.ok) {
  ...
  const reportPath = join(worktreePath, '.omg', 'diff.md');
  ...
  await mkdir(join(worktreePath, '.omg'), { recursive: true });
...
async function prepareWorkerWorktreeShutdownReports(config: TeamConfig, leaderCwd: string): Promise<WorkerShutdownMergeReport[]> {
  ...
  for (const worker of config.workers) {
    ...
    try {
      ...
      await mkdir(join(worktreePath, '.omg'), { recursive: true }).catch(() => {});
...
export function resolveWorkerLaunchArgsFromEnv(
  ...
  if (effectiveWorkerCli === 'claude') {
    console.log('[omg:team] worker startup resolution: model=claude source=local-settings');
  } else if (effectiveWorkerCli === 'gemini') {
    console.log('[omg:team] worker startup resolution: model=gemini source=local-settings');
  } else {
    console.log(`[omg:team] worker startup resolution: model=${resolvedModel} thinking_level=${thinkingLevel} source=${source}`);
  }
...
export async function startTeam(
  ...
  let sessionName = `omg-team-${sanitized}`;
  ...
  try {
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      const workerRole = taskRoles.length > 0 && uniqueTaskRoles.size === 1
        ? taskRoles[0]
        : agentType;
      const rawRolePromptContent = await loadRolePrompt(workerRole, join(leaderCwd, '.gemini', 'prompts'))
        ?? await loadRolePrompt(workerRole, geminiPromptsDir());
      ...
      const fallbackInstructionsPath = workerInstructionsPath ?? join(leaderCwd, 'GEMINI.md');
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      if (workerTasks.map(t => t.role).filter(Boolean).length > 0 && new Set(workerTasks.map(t => t.role).filter(Boolean)).size > 1) {
        console.log(`[omg:team] ${workerName}: mixed task roles [${[...new Set(workerTasks.map(t => t.role).filter(Boolean))].join(', ')}], falling back to ${agentType}`);
      }
      ...
      // Retry dispatch up to 3 times to handle Gemini trust prompts that may
...
export async function shutdownTeam(teamName: string, cwd: string, options: ShutdownOptions = {}): Promise<TeamShutdownSummary> {
  ...
  if (!config) {
    ...
    try {
      destroyTeamSession(`omg-team-${sanitized}`);
    } catch (err) {
      process.stderr.write(`[team/runtime] operation failed: ${err}\n`);
    }
  ...
  if (!force) {
    ...
    if (!gate.allowed) {
      if (requiresIssueConfirmation) {
        throw new Error(
          `shutdown_confirm_issues_required:failed=${gate.failed}:rerun=omg team shutdown ${sanitized} --confirm-issues`,
        );
      }
      throw new Error(
  ...
  if (!skipWorkerAcks) {
    ...
    if (anyAliveAfterWait && !force) {
      // Workers may have accepted shutdown but not exited (Gemini TUI requires explicit exit).
...
async function findActiveTeams(cwd: string, leaderSessionId: string): Promise<string[]> {
  ...
  for (const e of entries) {
    ...
    const workerLaunchMode = cfg?.worker_launch_mode
      ...
      ?? 'interactive';
    const tmuxSession = (manifest?.tmux_session || cfg?.tmux_session || `omg-team-${teamName}`).split(':')[0];
    if (leaderSessionId) {
...
async function detectAndCleanStaleTeam(
  ...
  const stateDir = join(leaderCwd, '.omg', 'state', 'team', teamName);
  if (!existsSync(stateDir)) return;

  const sessions = new Set(listTeamSessions());
  if (sessions.has(`omg-team-${teamName}`)) return;
  ...
  for (let i = 1; i <= workerCount; i++) {
    const wtPath = join(repoRoot, '.omg', 'team', teamName, 'worktrees', `worker-${i}`);

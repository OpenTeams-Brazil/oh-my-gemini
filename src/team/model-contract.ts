...
export function parseTeamWorkerLaunchArgs(args: string[]): ParsedTeamWorkerLaunchArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === MADMAX_FLAG) {
      // --madmax is ignored in oh-my-gemini
      continue;
    }
...
export function resolveAgentDefaultModel(
  ...
  if (normalized.endsWith('-low')) return resolveTeamLowComplexityDefaultModel(geminiHomeOverride);
  if (normalized === 'executor') return getMainDefaultModel(geminiHomeOverride);
  ...
  switch (getAgent(normalized)?.modelClass) {
    ...
    case 'standard':
      return getStandardDefaultModel(geminiHomeOverride);
    default:
      return undefined;
  }
}
...
export function resolveTeamLowComplexityDefaultModel(geminiHomeOverride?: string): string {
  return getSparkDefaultModel(geminiHomeOverride);
}

...
async function readJsonIfExists(
  ...
  try {
    ...
    if (options?.throwOnParseError && options.mode) {
      throw new Error(
        `Cannot read ${options.mode} workflow state at ${path}. Repair or clear that workflow state yourself via \`omg state clear --mode ${options.mode}\` or the \`omg_state.*\` MCP tools.`,
      );
    }
    return null;
  }

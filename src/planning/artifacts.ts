...
import { omgPlansDir } from '../utils/paths.js';
...
export function readPlanningArtifacts(cwd: string): PlanningArtifacts {
  const plansDir = omgPlansDir(cwd);
  const specsDir = join(cwd, '.omg', 'specs');
...
export function readApprovedExecutionLaunchHint(
  ...
  if (mode === 'team') {
    const teamPattern = /(?<command>(?:omg\s+team|\$team)\s+(?<ralph>ralph\s+)?(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;
  ...
  const ralphPattern = /(?<command>(?:omg\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;

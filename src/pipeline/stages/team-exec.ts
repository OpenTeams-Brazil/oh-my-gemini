 ...
 *
 * Wraps the existing team mode (tmux-based Gemini CLI workers) into a
...
export interface TeamExecStageOptions {
  /** Number of Gemini CLI workers to launch. Defaults to 2. */
  workerCount?: number;
 ...
 *
 * This stage delegates to the existing `omg team` infrastructure, which
 * starts real Gemini CLI workers in tmux panes. The stage collects the
...
/**
 * Build the `omg team` CLI instruction from a descriptor.
 */
export function buildTeamInstruction(descriptor: TeamExecDescriptor): string {
  const launchCommand = `omg team ${descriptor.workerCount}:${descriptor.agentType} ${JSON.stringify(descriptor.task)}`;

...
const defaultTarget = join(repoRoot, 'dist', 'cli', 'omg.js');
...
function normalizeInstallDoctorOutput(text: string, home: string, cwd: string): string {
  const repoStateDir = join(cwd, '.omg', 'state').replace(/\\/g, '/');
  return text
    .replaceAll(join(home, '.gemini').replace(/\\/g, '/'), '<GEMINI_HOME>')
    ...
    .map((line) => {
      ...
      if (line.startsWith('Run "omg setup')) {
...
describe('compat doctor contract', () => {
  it('matches onboarding warning copy for first setup expectations', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-compat-doctor-'));
    const home = join(wd, 'home');
    const geminiHome = join(home, '.gemini');
    await mkdir(geminiHome, { recursive: true });
    await writeFile(join(geminiHome, 'config.toml'), '[mcp_servers.non_omg]\ncommand = "node"\n');

    try {
      const result = runCompatTarget(wd, ['doctor'], { HOME: home, GEMINI_HOME: geminiHome });
  ...
  it('matches doctor --team resume_blocker behavior', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-compat-doctor-team-'));
    try {
      const teamRoot = join(wd, '.omg', 'state', 'team', 'alpha');
      await mkdir(join(teamRoot, 'workers', 'worker-1'), { recursive: true });
      await writeFile(join(teamRoot, 'config.json'), JSON.stringify({ name: 'alpha', tmux_session: 'omg-team-alpha' }));

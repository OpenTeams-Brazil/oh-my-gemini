...
describe('rust runtime legacy-reader compatibility', () => {
  it('keeps team status on the manifest-authored compatibility view', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-rust-compat-team-'));
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'omg-team-legacy-rust-compat-team';
        manifest.workspace_mode = 'worktree';
        manifest.tmux_session = 'omg-team-rust-compat-team';
  ...
  it('keeps doctor --team on the manifest-authored tmux session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-rust-compat-doctor-'));
    try {
      const teamStateRoot = join(wd, '.omg', 'state');
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'omg-team-legacy-rust-compat-doctor';
        manifest.tmux_session = 'omg-team-rust-compat-doctor';
        ...
        await writeFile(
          tmuxPath,
          '#!/bin/sh\nif [ "$1" = "list-sessions" ]; then echo "omg-team-rust-compat-doctor"; exit 0; fi\nexit 0\n',
        );
  ...
  it('keeps HUD team state on the session-scoped compatibility file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-rust-compat-hud-'));
    try {
      const stateRoot = join(wd, '.omg', 'state');

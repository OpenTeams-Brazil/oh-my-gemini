/**
 * TS Runtime Bridge — thin wrapper over omg-runtime binary.
 *
...
export function resolveRuntimeBinaryPath(options: RuntimeBinaryDiscoveryOptions = {}): string {
  ...
  const workspaceRelease = options.releasePath ?? resolve(__bridge_dirname, '../../target/release/omg-runtime');
  if (exists(workspaceRelease)) return workspaceRelease;

  return options.fallbackBinary ?? 'omg-runtime';
}
...
export class RuntimeBridge {
  ...
  private validateSchemaOnce(): void {
    ...
    try {
      ...
      if (missing.length > 0) {
        throw new Error(
          `omg-runtime schema missing commands: ${missing.join(', ')}. ` +
  ...
  private run(args: string[]): string {
    try {
      ...
      throw new Error(`omg-runtime ${args[0]} failed: ${stderr}`);
    }
  }
}

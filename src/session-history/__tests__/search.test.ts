...
async function writeRollout(
  ...
  const dir = join(geminiHomeDir, 'sessions', year, month, day);
...
describe('searchSessionHistory', () => {
  it('returns structured matches with snippets from rollout transcripts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-search-'));
    const geminiHomeDir = join(cwd, '.gemini-home');
    try {
      await writeRollout(geminiHomeDir, '2026-03-10T12:00:00.000Z', 'rollout-2026-03-10T12-00-00-session-a.jsonl', [
        {
          type: 'session_meta',
          payload: {
            id: 'session-a',
  ...
  it('supports session, project, and limit filters', async () => {
    ...
    try {
      ...
      await writeRollout(geminiHomeDir, '2026-03-09T12:00:00.000Z', 'rollout-2026-03-09T12-00-00-session-b.jsonl', [
        {
          type: 'session_meta',
          payload: {
            id: 'session-b',

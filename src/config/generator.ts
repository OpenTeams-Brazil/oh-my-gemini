/**
 * Config.toml generator/merger for oh-my-gemini
...
function getOmxTopLevelLines(
  ...
  const lines = [
    ...
    `developer_instructions = "You have oh-my-gemini installed. GEMINI.md is your orchestration brain and the main orchestration surface. Use skill/keyword routing like $name plus spawned role-specialized subagents for specialized work. Gemini native subagents are available via .gemini/agents and may be used for independent parallel subtasks within a single session or team pane. Skills are loaded from installed SKILL.md files under .gemini/skills, not from native agent TOMLs. Use workflow skills via $name when explicitly invoked or clearly routed by GEMINI.md. Treat installed prompts as narrower internal execution surfaces under GEMINI.md authority, even when user-facing docs prefer $name keywords."`,
  ];
...
export function stripOmxFeatureFlags(config: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > featuresStart && i < sectionEnd) {
      const isOmxFlag = omgFlags.some((f) =>
 ...
 *
 * Targets: [mcp_servers.omg_*], legacy [agents.<name>] entries, [tui]
 */
function stripOrphanedOmxSections(config: string): string {
  ...
  while (i < lines.length) {
    ...
    if (tableMatch) {
      ...
      const isOmxSection =
        /^mcp_servers\.omg_/.test(tableName) ||
      ...
      if (isOmxSection) {
        ...
        while (result.length > 0) {
          ...
          if (last.trim() === "" || /^#\s*(OMX|oh-my-gemini)/i.test(last)) {
            result.pop();
          } else {
            break;
          }
...
export function stripExistingOmxBlocks(config: string): {
  ...
  const marker = "oh-my-gemini (OMX) Configuration";
  const endMarker = "# End oh-my-gemini";
  let cleaned = config;
  let removed = 0;
...
function findLauncherTimeoutRepairTargets(
  ...
  for (let start = 0; start < lines.length; start += 1) {
    ...
    if (!name || name.startsWith("omg_") || typeof value !== "object" || !value) {
      start = end - 1;
      continue;
    }
...
function getSharedMcpRegistryBlock(
  ...
  const lines = [
    ...
    "# Managed by omg setup - edit the registry file instead",
  ];
  if (sourcePath) {
...
export function buildMergedConfig(
  ...
  if (existing.includes("oh-my-gemini (OMX) Configuration")) {
 ...
 *
 * After an omg version upgrade the OLD setup code (still loaded in memory)
 * may leave a config with duplicate [tui] sections or the retired
 * [mcp_servers.omg_team_run] table. Gemini rejects duplicate tables and newer

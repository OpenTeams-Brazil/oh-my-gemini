...
describe('config generator', () => {
  it('places top-level keys before [features]', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-config-gen-'));
    try {
  ...
  it('writes model_reasoning_effort and strengthened developer_instructions', async () => {
    ...
    try {
      ...
      assert.match(toml, /^developer_instructions = "You have oh-my-gemini installed/m);
      ...
      assert.match(toml, /Gemini native subagents are available via \.gemini\/agents/);
  ...
  it('handles paths with spaces in notify array', async () => {
    const base = await mkdtemp(join(tmpdir(), 'omg config gen space-'));
  ...
  it('re-runs setup replacing OMX config cleanly', async () => {
    ...
    try {
      ...
      assert.equal(
        (rerun.match(/# oh-my-gemini \(OMX\) Configuration/g) ?? []).length,
        1
      );
      assert.equal((rerun.match(/# End oh-my-gemini/g) ?? []).length, 1);
      ...
      assert.match(rerun, /^gemini_hooks = true$/m);
  ...
  it('migrates a legacy OMX block and preserves user settings', async () => {
    ...
    try {
      ...
      assert.equal(
        (toml.match(/oh-my-gemini \(OMX\) Configuration/g) ?? []).length,
        1
      );
  ...
  it('escapes Windows-style backslashes for MCP server args', async () => {
    ...
    try {
      const configPath = join(wd, 'config.toml');
      const windowsPkgRoot = 'C:\\Users\\alice\\oh-my-gemini';
      await mergeConfig(configPath, windowsPkgRoot);
      const toml = await readFile(configPath, 'utf-8');

      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-gemini\/dist\/mcp\/state-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-gemini\/dist\/mcp\/memory-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-gemini\/dist\/mcp\/code-intel-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-gemini\/dist\/mcp\/trace-server\.js"\]/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }

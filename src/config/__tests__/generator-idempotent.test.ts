 ...
 * Verifies that repeated `omg setup` runs do not duplicate OMX sections.
 */
...
function assertSingleOmxBlock(toml: string): void {
  assert.equal(
    count(toml, /# oh-my-gemini \(OMX\) Configuration/g),
    1,
    "OMX marker should appear once",
  );
  assert.equal(
    count(toml, /# End oh-my-gemini/g),
    1,
    "End marker should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_state\]$/gm),
    1,
    "[mcp_servers.omg_state] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_memory\]$/gm),
    1,
    "[mcp_servers.omg_memory] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_code_intel\]$/gm),
    1,
    "[mcp_servers.omg_code_intel] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_trace\]$/gm),
    1,
    "[mcp_servers.omg_trace] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_wiki\]$/gm),
    1,
    "[mcp_servers.omg_wiki] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omg_team_run\]$/gm),
    0,
    "[mcp_servers.omg_team_run] should not be emitted",
  );
  assert.doesNotMatch(
    toml,
...
describe("config generator idempotency (#384)", () => {
  ...
  it("handles config with only orphaned agents sections", async () => {
    ...
    try {
      ...
      const orphanedAgents = [
        ...
        "multi_agent = true",
        "",
        "# OMX Native Agent Roles (Gemini multi-agent)",
        "",
        "[agents.executor]",
  ...
  it("repairs config with duplicate [tui] sections from upgrade", async () => {
    ...
    try {
      const configPath = join(wd, "config.toml");
      // Simulate a broken config left by an older omg setup: an orphaned
  ...
  it("mergeConfig removes legacy omg_team_run tables during setup upgrade", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const legacy = [
      ...
      assertSingleOmxBlock(toml);
      assert.doesNotMatch(toml, /^\[mcp_servers\.omg_team_run\]$/m);
  ...
  it("repairConfigIfNeeded removes legacy omg_team_run tables during launch repair", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const legacy = [
  ...
  it("syncs shared MCP registry entries in a dedicated managed block", async () => {
    ...
    try {
      const first = buildMergedConfig("", wd, {
        ...
        sharedMcpRegistrySource: "/tmp/.omg/mcp-registry.json",
      });
      ...
      assert.equal(
        count(second, /oh-my-gemini \(OMX\) Shared MCP Registry Sync/g),
        1,
      ...
      assert.match(second, /# Source: \/tmp\/\.omg\/mcp-registry\.json/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("skips shared MCP entries that already exist in user config", () => {
    ...
    const merged = buildMergedConfig(existing, "/tmp/omg", {
      sharedMcpServers: [
        {
  ...
  it("adds a default startup timeout to launcher-backed non-OMX MCP servers and stays idempotent", () => {
    ...
    const first = buildMergedConfig(existing, "/tmp/omg");
    const second = buildMergedConfig(first, "/tmp/omg");

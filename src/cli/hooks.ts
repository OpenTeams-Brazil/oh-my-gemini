...
const HELP = `
Usage:
  omg hooks init       Create .omg/hooks/sample-plugin.mjs scaffold
  omg hooks status     Show plugin directory + discovered plugins
  omg hooks validate   Validate plugin exports/signatures
  omg hooks test       Dispatch synthetic turn-complete event to plugins

Notes:
  - This command is additive. Existing \`omg tmux-hook\` behavior is unchanged.
  - Plugins are enabled by default. Disable with OMG_HOOK_PLUGINS=0.
`;
...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with OMG_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OMG_HOOK_PLUGINS=0)'}`,
  );
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: omg hooks init');
    return;
  }
...
async function testHooks(): Promise<void> {
  ...
  const event = buildHookEvent('turn-complete', {
    source: 'native',
    context: {
      reason: 'omg-hooks-test',
    },
    session_id: 'omg-hooks-test',
  ...
  const logPath = join(cwd, '.omg', 'logs', `hooks-${new Date().toISOString().split('T')[0]}.jsonl`);

/**
 * omg setup - Automated installation of oh-my-gemini
 * Installs skills, prompts, MCP servers config, and GEMINI.md
 */
...
interface SetupOptions {
  geminiVersionProbe?: () => string | null;
  force?: boolean;
  dryRun?: boolean;
  scope?: SetupScope;
  verbose?: boolean;
...
const LEGACY_PROJECT_GITIGNORE_ENTRIES = [".gemini/"] as const;
...
function applyScopePathRewritesToAgentsTemplate(
  ...
  return content.replaceAll("~/.gemini", "./.gemini");
}
...
function getBackupContext(
  ...
  if (scope === "project") {
    return {
      backupRoot: join(projectRoot, ".omg", "backups", "setup", timestamp),
      baseRoot: projectRoot,
    };
  }
  return {
    backupRoot: join(homedir(), ".omg", "backups", "setup", timestamp),
    baseRoot: homedir(),
  };
}
...
async function buildLegacySkillOverlapNotice(
  ...
  if (overlap.overlappingSkillNames.length === 0) {
    return {
      shouldWarn: true,
      message:
        `Legacy ~/.agents/skills still exists (${overlap.legacySkillCount} skills) alongside canonical ${overlap.canonicalDir}. Gemini may still discover both roots; archive or remove ~/.agents/skills if Enable/Disable Skills shows duplicates.`,
    };
  }
  ...
  return {
    shouldWarn: true,
    message:
      `Detected ${overlap.overlappingSkillNames.length} overlapping skill names between canonical ${overlap.canonicalDir} and legacy ${overlap.legacyDir}.${mismatchSuffix} Remove or archive ~/.agents/skills after confirming ${overlap.canonicalDir} is the version you want Gemini to load.`,
  };
}
...
function getScopeFilePath(projectRoot: string): string {
  return join(projectRoot, ".omg", "setup-scope.json");
}
...
export function resolveScopeDirectories(
  ...
  if (scope === "project") {
    const geminiHomeDir = join(projectRoot, ".gemini");
    return {
      geminiConfigFile: join(geminiHomeDir, "config.toml"),
      geminiHomeDir,
      geminiHooksFile: join(geminiHomeDir, "hooks.json"),
      nativeAgentsDir: join(geminiHomeDir, "agents"),
      promptsDir: join(geminiHomeDir, "prompts"),
      skillsDir: join(geminiHomeDir, "skills"),
    };
  }
  return {
    geminiConfigFile: geminiConfigPath(),
    geminiHomeDir: geminiHome(),
    geminiHooksFile: join(geminiHome(), "hooks.json"),
    nativeAgentsDir: geminiAgentsDir(),
...
async function readPersistedSetupPreferences(
  ...
  try {
    ...
    if (parsed && typeof parsed.scope === "string") {
      ...
      if (migrated) {
        console.warn(
          `[omg] Migrating persisted setup scope "${parsed.scope}" → "${migrated}" ` +
...
async function promptForSetupScope(
  ...
  try {
    ...
    console.log(
      `  1) user (default) — installs to ~/.gemini (skills default to ~/.gemini/skills)`,
    );
    console.log("  2) project — installs to ./.gemini (local to project)");
...
function probeInstalledGeminiVersion(): string | null {
...
function shouldOmxManageTuiFromGeminiVersion(versionOutput: string | null): boolean {
...
async function promptForAgentsOverwrite(
  ...
  try {
    const answer = (
      await rl.question(
        `Overwrite existing GEMINI.md at "${destinationPath}"? [y/N]: `,
      )
    )
      .trim()
      .toLowerCase();
...
async function ensureProjectGitignore(
  ...
  if (options.verbose) {
    const changedDetails = [
      normalized.removed ? "removed legacy .gemini/" : "",
...
export async function setup(options: SetupOptions = {}): Promise<void> {
  ...
  const scopeSourceMessage =
    resolvedScope.source === "persisted" ? " (from .omg/setup-scope.json)" : "";
  const backupContext = getBackupContext(resolvedScope.scope, projectRoot);

  console.log("oh-my-gemini setup");
  console.log("=================\n");
  console.log(
  ...
  const dirs = [
    scopeDirs.geminiHomeDir,
    scopeDirs.promptsDir,
    scopeDirs.skillsDir,
    scopeDirs.nativeAgentsDir,
    omgStateDir(projectRoot),
    omgPlansDir(projectRoot),
    omgLogsDir(projectRoot),
  ];
  ...
  if (resolvedScope.scope === "project") {
    ...
    if (gitignoreResult === "created") {
      console.log(
        "  Created .gitignore with OMX project ignore rules so local runtime state stays out of source control while .gemini agents, skills, and prompts remain trackable.\n",
      );
    } else if (gitignoreResult === "updated") {
      console.log(
        "  Updated .gitignore with OMX project ignore rules so local runtime state stays out of source control while .gemini agents, skills, and prompts remain trackable.\n",
      );
    }
  }
  ...
  const managedConfig = await updateManagedConfig(
    scopeDirs.geminiConfigFile,
    pkgRoot,
    sharedMcpRegistry,
    summary.config,
    backupContext,
    { geminiVersionProbe: options.geminiVersionProbe, dryRun, verbose, modelUpgradePrompt },
  );
  const resolvedConfig = managedConfig.finalConfig;
  if (managedConfig.repairedLegacyTeamRunTable) {
    console.log(
      "  Removed retired [mcp_servers.omg_team_run] config during refresh.",
    );
  }
  ...
  console.log(`  Config refresh complete (${scopeDirs.geminiConfigFile}).\n`);
  ...
  await syncManagedContent(
    hooksConfig,
    scopeDirs.geminiHooksFile,
    summary.config,
    backupContext,
    { dryRun, verbose },
    `native hooks ${scopeDirs.geminiHooksFile}`,
  );
  console.log(
    `  Native Gemini hooks refresh complete (${scopeDirs.geminiHooksFile}).\n`,
  );
  ...
  if (teamToolsCheck.ok) {
    console.log("  omg team api command detected (CLI-first interop ready)");
  } else {
    console.log(`  WARNING: ${teamToolsCheck.message}`);
    console.log("  Run `npm run build` and then re-run `omg setup`.");
  }
  console.log();

  // Step 6: Generate GEMINI.md
  console.log("[6/8] Generating GEMINI.md...");
  const agentsMdSrc = join(pkgRoot, "templates", "GEMINI.md");
  const agentsMdDst =
    resolvedScope.scope === "project"
      ? join(projectRoot, "GEMINI.md")
      : join(scopeDirs.geminiHomeDir, "GEMINI.md");
  const agentsMdExists = existsSync(agentsMdDst);

  // Guard: refuse to overwrite project-root GEMINI.md during active session
  const activeSession =
    resolvedScope.scope === "project"
  ...
  if (existsSync(agentsMdSrc)) {
    ...
    const modelTableContext = resolveAgentsModelTableContext(resolvedConfig, {
      geminiHomeOverride: scopeDirs.geminiHomeDir,
    });
    ...
    if (agentsMdExists) {
      ...
      if (hasOmgManagedAgentsSections(existing)) {
    ...
    if (
      ...
      console.log(
        "  WARNING: Active omg session detected (pid " +
          activeSession?.pid +
          ").",
      );
      console.log(
        "  Skipping GEMINI.md overwrite to avoid corrupting runtime overlay.",
      );
      ...
      console.log(
        resolvedScope.scope === "project"
          ? "  Refreshed GEMINI.md model capability table in project root."
          : `  Refreshed GEMINI.md model capability table in ${scopeDirs.geminiHomeDir}.`,
      );
    } else {
      ...
      if (result === "updated") {
        console.log(
          resolvedScope.scope === "project"
            ...
            : `  Generated GEMINI.md in ${scopeDirs.geminiHomeDir}.`,
        );
      } else if (result === "unchanged") {
        console.log(
          resolvedScope.scope === "project"
            ? "  GEMINI.md already up to date in project root."
            : `  GEMINI.md already up to date in ${scopeDirs.geminiHomeDir}.`,
        );
      } else if (agentsMdExists) {
        console.log(
          `  Skipped GEMINI.md overwrite for ${agentsMdDst}. Re-run interactively to confirm or use --force.`,
        );
      }
    }
    if (resolvedScope.scope === "user") {
      console.log("  User scope leaves project GEMINI.md unchanged.");
    }
  } else {
    summary.agentsMd.skipped += 1;
    console.log("  GEMINI.md template not found, skipping.");
  }
  console.log();
  ...
  const hudConfigPath = join(projectRoot, ".omg", "hud-config.json");
  ...
  console.log('Setup complete! Run "omg doctor" to verify installation.');
  console.log("\nNext steps:");
  console.log("  1. Start Gemini CLI in your project directory");
  console.log(
    "  2. Use role/workflow keywords like $architect, $executor, and $plan in Gemini",
  );
  console.log("  3. Browse skills with /skills; AGENTS keyword routing can also activate them implicitly");
  console.log("  4. The GEMINI.md orchestration brain is loaded automatically");
  console.log(
...
async function refreshNativeAgentConfigs(
  ...
  for (const [name, agent] of Object.entries(AGENT_DEFINITIONS)) {
    ...
    const toml = generateAgentToml(agent, promptContent, {
      geminiHomeOverride: join(agentsDir, ".."),
    });
...
async function updateManagedConfig(
  ...
  options: Pick<
    SetupOptions,
    "geminiVersionProbe" | "dryRun" | "verbose" | "modelUpgradePrompt"
  >,
  ...
  const geminiVersion =
    options.geminiVersionProbe?.() ?? probeInstalledGeminiVersion();
  const omgManagesTui = shouldOmxManageTuiFromGeminiVersion(geminiVersion);

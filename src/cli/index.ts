/**
 * oh-my-gemini CLI
 * Multi-agent orchestration for OpenAI Gemini CLI
 */
...
import { geminiConfigPath, rememberOmxLaunchContext, resolveOmxEntryPath } from "../utils/paths.js";
...
const HELP = `
oh-my-gemini (omg) - Multi-agent orchestration for Gemini CLI
...
Usage:
  ...
  omg agents-init [path]
                Bootstrap lightweight GEMINI.md files for a repo/subtree
  omg agents    Manage Gemini native agent TOML files
  omg deepinit [path]
                Alias for agents-init (lightweight AGENTS bootstrap only)
  omg team      Spawn parallel worker panes in tmux and bootstrap inbox/task state
  omg ralph     Launch Gemini with ralph persistence mode active
  omg autoresearch Launch thin-supervisor autoresearch with keep/discard/reset parity
  omg version   Show version information
  ...
  omg help      Show this help message
  omg status    Show active modes and state
  omg cancel    Cancel active execution modes
  omg reasoning Show or set model reasoning effort (low|medium|high|xhigh)

Options:
  --yolo        Launch Gemini in yolo mode (shorthand for: omg launch --yolo)
  --high        Launch Gemini with high reasoning effort
                (shorthand for: -c model_reasoning_effort="high")
  --xhigh       Launch Gemini with xhigh reasoning effort
                (shorthand for: -c model_reasoning_effort="xhigh")
  --madmax      DANGEROUS: bypass Gemini approvals and sandbox
                (alias for --dangerously-bypass-approvals-and-sandbox)
  --spark       Use the Gemini spark model (~1.3x faster) for team workers only
  ...
  -w, --worktree[=<name>]
                Launch Gemini in a git worktree (detached when no name is given)
  --force       Force reinstall (overwrite existing files)
  --dry-run     Show what would be done without doing it
  --keep-config Skip config.toml cleanup during uninstall
  --purge       Remove .omg/ cache directory during uninstall
  --verbose     Show detailed output
  --scope       Setup scope for "omg setup" only:
                user | project
  --skill-target
...
const REASONING_USAGE = "Usage: omg reasoning <low|medium|high|xhigh>";
...
export function resolveGeminiHomeForLaunch(
  cwd: string,
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  if (env.GEMINI_HOME && env.GEMINI_HOME.trim() !== "") return env.GEMINI_HOME;
...
export function resolveGeminiConfigPathForLaunch(
  cwd: string,
  env: NodeJS.ProcessEnv = process.env,
): string {
  const geminiHomeOverride = resolveGeminiHomeForLaunch(cwd, env);
...
export type GeminiLaunchPolicy = "inside-tmux" | "detached-tmux" | "direct";

function splitLeaderLaunchPolicyArgs(args: string[]): {
  explicitPolicy?: GeminiLaunchPolicy;
  remainingArgs: string[];
} {
  const remainingArgs: string[] = [];
  let explicitPolicy: GeminiLaunchPolicy | undefined;
...
export function resolveLeaderLaunchPolicyOverride(
  args: string[],
): GeminiLaunchPolicy | undefined {
  return splitLeaderLaunchPolicyArgs(args).explicitPolicy;
}

export function resolveGeminiLaunchPolicy(
  env: NodeJS.ProcessEnv = process.env,
...
export interface GeminiExecFailureClassification {
...
export function classifyGeminiExecFailure(
  error: unknown,
): GeminiExecFailureClassification {
...
export async function launchWithHud(args: string[]): Promise<void> {
  if (isNativeWindows()) {
    ...
    if (result.error) {
      ...
      if (kind === "missing") {
        console.warn(
          "[omg] warning: tmux was not found on native Windows. Continuing without tmux/HUD.\n" +
            "[omg] To enable tmux-backed features, install psmux:\n" +
            "[omg]   winget install psmux\n" +
            "[omg] See: https://github.com/marlocarlo/psmux",
        );
      } else {
        console.warn(
          `[omg] warning: tmux probe failed on native Windows (${errno.code || errno.message}). Continuing without tmux/HUD.`,
        );
      }
    } else if (result.status !== 0 && !isTmuxAvailable()) {
      const stderr = (result.stderr || "").trim();
      console.warn(
        `[omg] warning: tmux reported an error on native Windows${stderr ? ` (${stderr})` : ""}. Continuing without tmux/HUD.`,
      );
    }
  }
  ...
  const geminiHomeOverride = resolveGeminiHomeForLaunch(launchCwd, process.env);
  ...
  const normalizedArgs = normalizeGeminiLaunchArgs(
    notifyTempResult.passthroughArgs,
  );
  let cwd = launchCwd;
  ...
  if (parsedWorktree.mode.enabled) {
    ...
    if (ensured.enabled) {
      ...
      if (depBootstrap.strategy === "symlink") {
        console.log(`[omg] Reusing node_modules from ${depBootstrap.sourceNodeModulesPath}`);
      } else if (depBootstrap.strategy === "missing" && depBootstrap.warning) {
        console.warn(`[omg] ${depBootstrap.warning}`);
      }
    }
  }
  ...
  // After an omg version upgrade the OLD setup code (still in memory) may
  // have written a config.toml with duplicate [tui] sections.  Gemini CLI's
  // TOML parser rejects duplicates, so we repair before spawning the CLI.
  try {
    const repaired = await repairConfigIfNeeded(
      resolveGeminiConfigPathForLaunch(launchCwd, process.env),
      getPackageRoot(),
    );
    if (repaired) {
      console.log("[omg] Repaired managed config.toml compatibility issue.");
    }
  } catch {
  ...
  // ── Phase 1: preLaunch ──────────────────────────────────────────────────
  try {
    await preLaunch(cwd, sessionId, notifyTempResult.contract, geminiHomeOverride, enableNotifyFallbackAuthority, worktreeDirty);
  } catch (err) {
    // preLaunch errors must NOT prevent Gemini from starting
    console.error(
      `[omg] preLaunch warning: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    // ── Phase 3: postLaunch ─────────────────────────────────────────────
    await postLaunch(cwd, sessionId, geminiHomeOverride, enableNotifyFallbackAuthority);
  }
}
...
export async function execWithOverlay(args: string[]): Promise<void> {
  ...
  try {
    await preLaunch(cwd, sessionId, notifyTempResult.contract, geminiHomeOverride, true, worktreeDirty);
  } catch (err) {
    console.error(
      `[omg] preLaunch warning: ${err instanceof Error ? err.message : err}`,
    );
...
export function buildTmuxSessionName(cwd: string, sessionId: string): string {
  ...
  const repoDir = parentDir.endsWith(".omg-worktrees")
    ? parentDir.slice(0, -".omg-worktrees".length)
    : parentDir === "worktrees" && grandparentDir === ".omg"
  ...
  const sessionToken = sanitizeTmuxToken(sessionId.replace(/^omg-/, ""));
  const prefix = `omg-${dirToken}-${branchToken}`;
...
function tmuxExtendedKeysLeaseRoot(cwd: string): string {
  return join(cwd, ".omg", "state", TMUX_EXTENDED_KEYS_LEASE_DIR);
}
  ...
  for (const stateDir of scopedDirs) {
    ...
    for (const file of files) {
      ...
      if (result.kind !== "ok") {
        if (result.kind === "recoverable") {
          try {
            ...
            writeWarn(
              `[omg] postLaunch: failed to recover mode state ${path}: ${err instanceof Error ? err.message : err}`,
            );
          }
        } else if (result.kind === "malformed") {
          writeWarn(
            `[omg] postLaunch: skipped malformed mode state ${path}: ${result.message}`,
          );
        }
        continue;
      }
      ...
      try {
        ...
        writeWarn(
          `[omg] postLaunch: failed to update mode state ${path}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
...
${launchAppendix}${dirtyWorktreeGuidance}`
  ...
  try {
    await startNotifyFallbackWatcher(cwd, { geminiHomeOverride, enableAuthority: enableNotifyFallbackAuthority, sessionId });
  } catch (err) {
    process.stderr.write(`[cli/index] operation failed: ${err}\n`);
    // Non-fatal
  }
...
function runGemini(
  ...
  geminiHomeOverride?: string,
  notifyTempContractRaw?: string | null,
  explicitLaunchPolicy?: GeminiLaunchPolicy,
): void {
      ...
      for (const step of bootstrapSteps) {
        ...
        if (step.name === "split-and-capture-hud-pane") {
          ...
          if (nativeWindows && detachedWindowsGeminiCmd) {
            scheduleDetachedWindowsGeminiLaunch(
              sessionName,
              detachedWindowsGeminiCmd,
            );
          }
...
function scheduleDetachedWindowsGeminiLaunch(
  sessionName: string,
  commandText: string,
): void {
  const child = spawn(
...
/**
 * postLaunch: Clean up after Gemini exits.
...
async function postLaunch(
  ...
  try {
    ...
    console.error(
      `[omg] postLaunch: model instructions cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omg] postLaunch: session archive failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omg] postLaunch: mode cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
...
function notifyFallbackPidPath(cwd: string): string {
  return join(cwd, ".omg", "state", "notify-fallback.pid");
}

function hookDerivedWatcherPidPath(cwd: string): string {
  return join(cwd, ".omg", "state", "hook-derived-watcher.pid");
}
...
export async function reapStaleNotifyFallbackWatcher(
  ...
  try {
    ...
    if (!hasErrnoCodeImpl(error, "ESRCH")) {
      warn(
        "[omg] warning: failed to stop stale notify fallback watcher",
        {
          path: pidPath,
...
async function startNotifyFallbackWatcher(
  cwd: string,
  options: { geminiHomeOverride?: string; enableAuthority?: boolean; sessionId?: string } = {},
): Promise<void> {
  ...
  await mkdir(join(cwd, ".omg", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omg] warning: failed to create notify fallback watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  const watcherEnv = buildNotifyFallbackWatcherEnv(process.env, {
    geminiHomeOverride: options.geminiHomeOverride,
  ...
  try {
    ...
    console.warn("[omg] warning: failed to launch notify fallback watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omg] warning: failed to write notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function startHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  if (existsSync(pidPath)) {
    try {
      ...
      console.warn("[omg] warning: failed to stop stale hook-derived watcher", {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  await mkdir(join(cwd, ".omg", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omg] warning: failed to create hook-derived watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  try {
    ...
    console.warn("[omg] warning: failed to launch hook-derived watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omg] warning: failed to write hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopNotifyFallbackWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    if (!hasErrnoCode(error, "ESRCH")) {
      console.warn(
        "[omg] warning: failed to stop notify fallback watcher process",
        {
          path: pidPath,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omg] warning: failed to remove notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[omg] warning: failed to stop hook-derived watcher process", {
      path: pidPath,
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omg] warning: failed to remove hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },

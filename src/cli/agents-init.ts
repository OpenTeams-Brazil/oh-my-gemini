...
export const AGENTS_INIT_USAGE = [
  "Usage: omg agents-init [path] [--dry-run] [--force] [--verbose]",
  "       omg deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
  "Bootstrap lightweight GEMINI.md files for the target directory and its direct child directories.",
  "",
  "Options:",
  "  --dry-run   Show planned file updates without writing files",
  "  --force     Overwrite existing unmanaged GEMINI.md files after taking a backup",
...
async function readProjectRootTemplate(): Promise<string> {
  ...
  const templatePath = join(pkgRoot, "templates", "GEMINI.md");
...
async function snapshotDirectory(dir: string): Promise<DirectorySnapshot> {
  ...
  for (const entry of entries) {
    if (entry.name === "GEMINI.md") continue;
...
function renderParentReference(
  ...
  const parentAgentsPath = join(dirname(dir), "GEMINI.md");
...
async function syncManagedAgentsFile(
  ...
  if (!isManagedAgentsInitFile(existingContent ?? "") && !options.force) {
    ...
    return {
      action: "skipped",
      reason: "existing unmanaged GEMINI.md (re-run with --force to adopt it)",
      backedUp: false,
    };
  }
...
export async function agentsInit(
  ...
  console.log("oh-my-gemini AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const destinationPath = join(dir, "GEMINI.md");
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active omg session detected for project root GEMINI.md"
        : undefined,
    );
    ...
    if (verbose || decision.action !== "unchanged") {
      ...
      console.log(`  ${label} ${relativeDir}/GEMINI.md${reason}`);
    }
  }
  ...
  console.log(
    "- Preserves manual notes only for files already managed by omg agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged GEMINI.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root GEMINI.md while an active omg session is running.\n",
  );

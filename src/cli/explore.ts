...
export const EXPLORE_USAGE = [
  ...
  '   or: omg explore --prompt-file <file>',
].join('\n');
...
const WEAK_WIKI_NOTE =
  'Wiki evidence is weak or missing. Fall back to broader repository search and recommend that the user build an initial project wiki under .omg/wiki/ if this repo benefits from persistent project knowledge.';
...
export function packagedExploreHarnessBinaryName(platform: NodeJS.Platform = process.platform): string {
  return platform === 'win32' ? 'omg-explore-harness.exe' : 'omg-explore-harness';
}
...
export function resolvePackagedExploreHarnessCommand(
  ...
  const metadataPath = join(packageRoot, 'bin', 'omg-explore-harness.meta.json');
...
export async function resolveExploreHarnessCommandWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('omg-explore-harness', version, process.platform, process.arch, env)) {
...
export async function exploreCommand(args: string[]): Promise<void> {
  ...
  if (sparkShellRoute) {
    try {
      ...
      process.stderr.write(`[omg explore] sparkshell backend unavailable (${message}). Falling back to the explore harness.\n`);
    }
  }

...
export const SPARKSHELL_USAGE = [
  ...
  '   or: omg sparkshell --tmux-pane <pane-id> [--tail-lines <100-1000>]',
  'Runs the native omg-sparkshell sidecar with direct argv execution or explicit tmux pane summarization.',
...
export function sparkshellBinaryName(platform: NodeJS.Platform = process.platform): string {
  return platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell';
}
...
export function nestedRepoLocalSparkShellBinaryPath(
  ...
  return join(packageRoot, 'native', 'omg-sparkshell', 'target', 'release', sparkshellBinaryName(platform));
}
...
export async function resolveSparkShellBinaryPathWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('omg-sparkshell', version, platform, arch, env, {
  ...
  const hydrated = await hydrateNativeBinary('omg-sparkshell', { packageRoot, env, platform, arch });
...
export function runSparkShellBinary(
  ...
  const configEnvOverrides = readConfiguredEnvOverrides(env.GEMINI_HOME);

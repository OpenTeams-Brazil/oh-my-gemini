...
const HELP = `omg session - Search prior local session history

Usage:
  omg session search <query> [options]
...
Examples:
  omg session search "worker inbox path"
  omg session search all_workers_idle --since 7d --limit 5
  omg session search "team api" --project current --json
`;

 ...
 * State stored globally (~/.omg/state/star-prompt.json) so it shows once per user.
 */
...
const REPO = 'Yeachan-Heo/oh-my-gemini';
...
export function starPromptStatePath(): string {
  return join(homedir(), '.omg', 'state', 'star-prompt.json');
}
...
export async function markPrompted(): Promise<void> {
  const stateDir = join(homedir(), '.omg', 'state');
...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[omg] Enjoying oh-my-gemini? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[omg] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[omg] Could not star repository automatically: ${star.error}`);
}

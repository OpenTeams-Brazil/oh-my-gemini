...
function runOmx(
  ...
  const result = spawnSync('node', [omgBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
describe('resolveSparkShellBinaryPath', () => {
  ...
  it('falls back from packaged binary to repo-local build artifact', () => {
    ...
    const packaged = join(packageRoot, 'bin', 'native', `${process.platform}-${process.arch}`, process.platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell');
  ...
  it('checks Linux musl packaged paths before glibc and legacy paths', () => {
    assert.deepEqual(
      ...
      [
        '/repo/bin/native/linux-x64-musl/omg-sparkshell',
        '/repo/bin/native/linux-x64-glibc/omg-sparkshell',
        '/repo/bin/native/linux-x64/omg-sparkshell',
      ],
    );
  });
  ...
  it('tries Linux musl packaged binaries before glibc fallbacks', () => {
    const packageRoot = '/repo';
    const seen: string[] = [];
    const glibcPath = '/repo/bin/native/linux-x64-glibc/omg-sparkshell';
  ...
  it('hydrates a native binary when packaged and repo-local binaries are absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-hydrated-'));
    try {
      ...
      const stagedBinary = join(stagingDir, process.platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell');
      ...
      const archiveName = process.platform === 'win32'
        ? 'omg-sparkshell-x86_64-pc-windows-msvc.zip'
      ...
      const buildArchive = process.platform === 'win32'
        ...
        : spawnSync('tar', ['-czf', archivePath, '-C', stagingDir, 'omg-sparkshell'], { encoding: 'utf-8' });
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          ...
          assets: [{
            ...
            arch: 'x64',
            archive: archiveName,
            binary: process.platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell',
            binary_path: process.platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell',
            sha256: checksum,
  ...
  it('falls back cleanly when hydration manifest is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-hydration-missing-'));
    try {
...
describe('runSparkShellBinary', () => {
  it('passes argv directly to the native sidecar', () => {
    ...
    runSparkShellBinary('/fake/omg-sparkshell', ['git', 'diff --stat', 'a|b'], {
      cwd: '/tmp/example',
      env: { TEST_ENV: '1' },
    ...
    assert.deepEqual(invoked, {
      binaryPath: '/fake/omg-sparkshell',
      args: ['git', 'diff --stat', 'a|b'],
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  });

  it('merges .omg-config.json env overrides behind explicit shell env', async () => {
    ...
    try {
      ...
      runSparkShellBinary('/fake/omg-sparkshell', ['git', 'status'], {
        cwd: geminiHome,
        env: {
  ...
  it('detects GLIBC symbol version failures from the native loader', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: "omg-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found\n",
        status: 1,
        signal: null,
      }),
      true,
  ...
  it('ignores non-compatibility stderr failures', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: 'omg sparkshell: summary unavailable (tmux failed)\n',
        status: 1,
        signal: null,
      }),
      false,
...
describe('omg sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-help-'));
    try {
      const result = runOmx(cwd, ['--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /omg explore\s+Default read-only exploration entrypoint \(may adaptively use sparkshell backend\)/);
      assert.match(result.stdout, /omg sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /omg sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-subhelp-'));
    try {
      const result = runOmx(cwd, ['sparkshell', '--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: omg sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: omg sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('preserves child stdout, stderr, and exit code through the JS bridge', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-bridge-'));
    try {
      const binDir = join(cwd, 'bin');
      const stubPath = join(binDir, process.platform === 'win32' ? 'omg-sparkshell.cmd' : 'omg-sparkshell');
  ...
  it('falls back to raw execution when the packaged native binary is GLIBC-incompatible', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-glibc-fallback-'));
    try {
      const testDir = dirname(fileURLToPath(import.meta.url));
      const repoRoot = join(testDir, '..', '..', '..');
      ...
      const binDir = join(cacheDir, packageJson.version, `${process.platform}-${process.arch}`, 'omg-sparkshell');
      await mkdir(binDir, { recursive: true });
      const stubPath = join(binDir, process.platform === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell');
      await writeFile(
        stubPath,
        process.platform === 'win32'
          ? '@echo off\r\n>&2 echo omg-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39\' not found\r\nexit /b 1\r\n'
  ...
  it('fails clearly when the configured native binary path does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-sparkshell-missing-'));
    try {

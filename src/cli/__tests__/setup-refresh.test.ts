...
describe("omg setup refresh summary and dry-run behavior", () => {
  ...
  it("prints per-category summary and verbose changed-file detail", async () => {
    ...
    try {
      ...
      const skillPath = join(wd, ".gemini", "skills", "help", "SKILL.md");
  ...
  it("creates .gitignore with OMX project ignore rules during project-scoped setup", async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, ".omg", "state")), true);
      assert.equal(
  ...
  it("appends missing OMX project ignore rules to an existing project .gitignore without duplicating them", async () => {
    ...
    try {
      ...
      assert.equal(gitignore.match(/^\.omg\/$/gm)?.length ?? 0, 1);
      assert.equal(gitignore.match(/^\.gemini\/\*$/gm)?.length ?? 0, 1);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("ignores project-local config while keeping .gemini agents, skills, and prompts trackable", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-refresh-"));
    try {
      ...
      await runSetupInTempDir(wd, { scope: "project" });
      await mkdir(join(wd, ".gemini", "skills", ".system"), { recursive: true });
      await writeFile(join(wd, ".gemini", "agents", "local.toml"), "# local\n");
      await writeFile(join(wd, ".gemini", "prompts", "local.md"), "# local\n");
      await writeFile(
        join(wd, ".gemini", "skills", ".system", "cache.json"),
        "{}\n",
      );
      ...
      const status = spawnSync(
        ...
        [
          ...
          "--ignored",
          ".gemini/config.toml",
          ".gemini/agents/local.toml",
          ".gemini/prompts/local.md",
          ".gemini/skills/help/SKILL.md",
          ".gemini/skills/.system/cache.json",
        ],
        { cwd: wd, encoding: "utf-8" },
      );
      assert.equal(status.status, 0);
      assert.match(status.stdout, /^!! \.gemini\/config\.toml$/m);
      assert.match(status.stdout, /^\?\? \.gemini\/agents\/local\.toml$/m);
      assert.match(status.stdout, /^\?\? \.gemini\/prompts\/local\.md$/m);
      assert.match(status.stdout, /^\?\? \.gemini\/skills\/help\/SKILL\.md$/m);
      assert.match(status.stdout, /^!! \.gemini\/skills\/\.system\/cache\.json$/m);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("replaces legacy .gemini/ ignores so the project allowlist can take effect", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-refresh-"));
    try {
      await writeFile(join(wd, ".gitignore"), ".omg/\n.gemini/\n");

      await runSetupInTempDir(wd, { scope: "project" });

      const gitignore = await readFile(join(wd, ".gitignore"), "utf-8");
      assert.equal(gitignore, EXPECTED_PROJECT_GITIGNORE);
      assert.equal(gitignore.match(/^\.gemini\/$/gm)?.length ?? 0, 0);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("creates backup files under the scope-specific setup backup root when refreshing modified managed files", async () => {
    ...
    try {
      ...
      const promptPath = join(wd, ".gemini", "prompts", "executor.md");
      const oldPrompt = "# local prompt\n";
      await writeFile(promptPath, oldPrompt);

      await runSetupInTempDir(wd, { scope: "project" });

      const backupsRoot = join(wd, ".omg", "backups", "setup");
  ...
  it("skips OMX-managed [tui] writes for Gemini CLI >= 0.107.0 and preserves an existing [tui] table", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omg", "state"), { recursive: true });
      await mkdir(join(wd, ".gemini"), { recursive: true });
      ...
      assert.match(
        output,
        /Gemini CLI >= 0\.107\.0 manages \[tui\]; OMX left that section untouched\./,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it("keeps OMX-managed [tui] writes for older Gemini CLI versions", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omg", "state"), { recursive: true });
  ...
  it("syncs shared MCP registry entries into config.toml during setup", async () => {
    ...
    try {
      ...
      const config = await readFile(join(wd, ".gemini", "config.toml"), "utf-8");
      assert.match(config, /oh-my-gemini \(OMX\) Shared MCP Registry Sync/);
  ...
  it("repairs retired omg_team_run config during setup refresh", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omg", "state"), { recursive: true });
      await mkdir(join(wd, ".gemini"), { recursive: true });
      ...
      assert.doesNotMatch(config, /^\[mcp_servers\.omg_team_run\]$/m);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

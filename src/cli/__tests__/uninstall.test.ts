...
function runOmx(
  ...
  const result = spawnSync(process.execPath, [omgBin, ...argv], {
    ...
    env: {
      ...process.env,
      ...(resolvedHome && !envOverrides.GEMINI_HOME ? { GEMINI_HOME: join(resolvedHome, '.gemini') } : {}),
      ...envOverrides,
    },
  });
  return {
...
function buildOmxConfig(): string {
  return [
    '# oh-my-gemini top-level settings (must be before any [table])',
    'notify = ["node", "/path/to/notify-hook.js"]',
    'model_reasoning_effort = "high"',
    'developer_instructions = "You have oh-my-gemini installed."',
    '',
    '[features]',
    'multi_agent = true',
    ...
    '# Managed by omg setup - manual edits preserved on next setup',
    '# ============================================================',
    '',
...
describe('omg uninstall', () => {
  ...
  it('removes OMX block from config.toml', async () => {
    ...
    try {
      ...
      assert.doesNotMatch(config, /omg_code_intel/);
      assert.doesNotMatch(config, /omg_trace/);
      assert.doesNotMatch(config, /omg_wiki/);
  ...
  it('--purge removes .omg/ cache directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      // Create .omg/ directory with some files
      const omgDir = join(wd, '.omg');
      await mkdir(join(omgDir, 'state'), { recursive: true });
      await writeFile(join(omgDir, 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(join(omgDir, 'notepad.md'), '# notes');
      await writeFile(join(omgDir, 'state', 'ralph-state.json'), '{}');

      const res = runOmx(wd, ['uninstall', '--keep-config', '--purge'], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.match(res.stdout, /\.omg\/ cache directory/);

      assert.equal(existsSync(omgDir), false, '.omg/ directory should be removed');
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('shows summary of what was removed', async () => {
    ...
    try {
      ...
      assert.match(res.stdout, /MCP servers: omg_state, omg_memory, omg_code_intel, omg_trace, omg_wiki/);
  ...
  it('warns when overlapping legacy ~/.agents/skills remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: 1 overlapping skill names remain between .*\.gemini[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content\. omg uninstall only removes the active canonical skill root; archive or remove ~\/\.agents\/skills if Gemini still shows duplicates/,
      );
  ...
  it('warns when a distinct legacy ~/.agents/skills root remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: legacy ~\/\.agents\/skills still exists \(1 skills\)\. omg uninstall does not remove that historical root automatically; archive or remove ~\/\.agents\/skills if Gemini still shows stale or duplicate skills/,
      );
      assert.equal(existsSync(canonicalHelp), false, 'canonical OMX skill should be removed');
      assert.equal(existsSync(join(home, '.agents', 'skills')), true, 'legacy skill root should remain for manual cleanup');
    } finally {
  ...
  it('does not warn about legacy ~/.agents/skills when none exists', async () => {
    ...
    try {
      ...
      assert.doesNotMatch(res.stdout, /omg uninstall does not remove legacy ~\/\.agents\/skills/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('does not warn about legacy ~/.agents/skills during project-scope uninstall', async () => {
    ...
    try {
      const home = join(wd, 'home');
      const projectSkillsHelp = join(wd, '.gemini', 'skills', 'help');
      const legacyHelp = join(home, '.agents', 'skills', 'help');
      ...
      await writeFile(join(legacyHelp, 'SKILL.md'), '# legacy help\n');
      await writeFile(join(wd, '.omg', 'setup-scope.json'), JSON.stringify({ scope: 'project' }));
  ...
  it('--dry-run --purge does not actually remove .omg/ directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      ...
      // .omg/ should still exist
      assert.equal(existsSync(omgDir), true, '.omg/ should NOT be removed in dry-run');
      assert.equal(existsSync(join(omgDir, 'notepad.md')), true);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('does not delete user GEMINI.md that merely mentions oh-my-gemini', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      const userAgentsMd = '# My Agents\n\nDo not use oh-my-gemini for this project.\n';
      await writeFile(join(wd, 'GEMINI.md'), userAgentsMd);

      const res = runOmx(wd, ['uninstall'], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);

      // User GEMINI.md should be preserved
      assert.equal(existsSync(join(wd, 'GEMINI.md')), true);
      const content = await readFile(join(wd, 'GEMINI.md'), 'utf-8');
  ...
  it('removes managed user-scope GEMINI.md from GEMINI_HOME', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-uninstall-'));
    try {
      const home = join(wd, 'home');
      ...
      await mkdir(join(wd, '.omg'), { recursive: true });
      await writeFile(join(wd, '.omg', 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(
        join(geminiHome, 'GEMINI.md'),
      ...
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.equal(existsSync(join(geminiHome, 'GEMINI.md')), false);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('removes setup-scope.json and hud-config.json without --purge', async () => {
    ...
    try {
      ...
      await mkdir(omgDir, { recursive: true });
      await writeFile(join(omgDir, 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(join(omgDir, 'hud-config.json'), JSON.stringify({ preset: 'focused' }));
      await writeFile(join(omgDir, 'notepad.md'), '# keep this');

      const res = runOmx(wd, ['uninstall', '--keep-config'], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);

      assert.equal(existsSync(join(omgDir, 'setup-scope.json')), false);
      assert.equal(existsSync(join(omgDir, 'hud-config.json')), false);

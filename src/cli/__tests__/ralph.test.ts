...
describe('filterRalphGeminiArgs', () => {
  ...
  it('consumes --PRD case-insensitively', () => {
    assert.deepEqual(filterRalphGeminiArgs(['--PRD', '--model', 'gpt-5']), ['--model', 'gpt-5']);
  });
  it('preserves non-omg flags', () => {
    assert.deepEqual(filterRalphGeminiArgs(['--model', 'gpt-5', '--yolo', 'fix', 'it']), ['--model', 'gpt-5', '--yolo', 'fix', 'it']);
  });
});
...
const approvedHint: ApprovedExecutionLaunchHint = {
  mode: 'ralph',
  command: 'omg ralph "Execute approved issue 1072 plan"',
  task: 'Execute approved issue 1072 plan',
  sourcePath: '.omg/plans/prd-issue-1072.md',
  testSpecPaths: ['.omg/plans/test-spec-issue-1072.md'],
  deepInterviewSpecPaths: ['.omg/specs/deep-interview-issue-1072.md'],
};

describe('assertRequiredRalphPrdJson', () => {
  it('throws when --prd mode starts without .omg/prd.json', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-prd-gate-'));
    try {
      assert.throws(
        () => assertRequiredRalphPrdJson(cwd, ['--prd', 'ship release checklist']),
        /Missing required PRD\.json at \.omg\/prd\.json/,
      );
    } finally {
  ...
  it('still requires legacy .omg/prd.json even when canonical PRD markdown exists', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-prd-gate-'));
    try {
  ...
  it('rejects completed stories without architect approval', async () => {
    ...
    try {
      ...
      await writeFile(join(cwd, '.omg', 'prd.json'), JSON.stringify({
        project: 'Issue 1555',
        userStories: [{
          id: 'US-001',
  ...
  it('allows --prd mode when .omg/prd.json exists', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-ralph-prd-gate-'));
    try {
      await mkdir(join(cwd, '.omg'), { recursive: true });
      await writeFile(join(cwd, '.omg', 'prd.json'), JSON.stringify({
...
describe('ralph deslop launch wiring', () => {
  ...
  it('documents changed-files-only deslop guidance by default', () => {
    const instructions = buildRalphAppendInstructions('fix issue 920', {
      changedFilesPath: '.omg/ralph/changed-files.txt',
      noDeslop: false,
      approvedHint: null,
    });
    assert.match(instructions, /ai-slop-cleaner/i);
    assert.match(instructions, /changed files only/i);
    assert.match(instructions, /\.omg\/ralph\/changed-files\.txt/);
  ...
  it('includes approved plan and deep-interview handoff context when available', () => {
    ...
    assert.match(instructions, /approved plan: \.omg\/plans\/prd-issue-1072\.md/i);
    assert.match(instructions, /test specs: \.omg\/plans\/test-spec-issue-1072\.md/i);
    assert.match(instructions, /deep-interview specs: \.omg\/specs\/deep-interview-issue-1072\.md/i);

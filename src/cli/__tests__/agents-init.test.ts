...
describe('omg agents-init', () => {
  it('creates a managed root GEMINI.md plus direct-child GEMINI.md files while skipping ignored directories', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-agents-init-'));
    try {
      ...
      const rootAgents = await readFile(join(wd, 'GEMINI.md'), 'utf-8');
      const srcAgents = await readFile(join(wd, 'src', 'GEMINI.md'), 'utf-8');
      const docsAgents = await readFile(join(wd, 'docs', 'GEMINI.md'), 'utf-8');
      ...
      assert.equal(existsSync(join(wd, 'dist', 'GEMINI.md')), false);
    } finally {
  ...
  it('refreshes managed subtree files while preserving the manual notes block', async () => {
    ...
    try {
      ...
      const agentsPath = join(wd, 'src', 'GEMINI.md');
      ...
      assert.equal(existsSync(join(wd, 'src', 'lib', 'GEMINI.md')), true);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it('skips unmanaged GEMINI.md files by default but can adopt them with --force and a backup', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-agents-init-'));
    const original = '# custom root guidance\n';
    try {
      await mkdir(join(wd, 'src'), { recursive: true });
      await writeFile(join(wd, 'GEMINI.md'), original);
      await writeFile(join(wd, 'src', 'index.ts'), 'export const x = 1;\n');

      await withCwd(wd, async () => {
        await agentsInit();
      });
      assert.equal(await readFile(join(wd, 'GEMINI.md'), 'utf-8'), original);
      assert.equal(existsSync(join(wd, 'src', 'GEMINI.md')), true);

      await withCwd(wd, async () => {
        await agentsInit({ force: true });
      });

      const adopted = await readFile(join(wd, 'GEMINI.md'), 'utf-8');
      assert.match(adopted, /OMX:AGENTS-INIT:MANAGED/);
      const backupRoot = join(wd, '.omg', 'backups', 'agents-init');
      assert.equal(existsSync(backupRoot), true);
      const timestamps = await readdir(backupRoot);
      assert.equal(timestamps.length > 0, true);
      const backupContent = await readFile(join(backupRoot, timestamps[0], 'GEMINI.md'), 'utf-8');
  ...
  it('protects project-root GEMINI.md during an active OMX session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-agents-init-'));
    try {
      const pidStartTicks = await readCurrentLinuxStartTicks();
      await mkdir(join(wd, '.omg', 'state'), { recursive: true });
      await mkdir(join(wd, 'src'), { recursive: true });
      await writeFile(join(wd, 'GEMINI.md'), '# unmanaged\n');
      await writeFile(join(wd, 'src', 'index.ts'), 'export const x = 1;\n');
      await writeFile(
      ...
      assert.equal(await readFile(join(wd, 'GEMINI.md'), 'utf-8'), '# unmanaged\n');
      assert.equal(existsSync(join(wd, 'src', 'GEMINI.md')), true);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('exposes help for agents-init and the deepinit alias', async () => {
    ...
    try {
      ...
      assert.match(helpRes.stdout, /Usage: omg agents-init/);
      ...
      assert.match(aliasRes.stdout, /Usage: omg agents-init/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

...
const CURRENT_SESSION_PROCESSES: ProcessEntry[] = [
  ...
  },
  {
    pid: 900,
    ppid: 1,
    command: 'node /tmp/not-omg/other-server.js',
  },
];
...
describe('findCleanupCandidates', () => {
  ...
  it('limits launch-safe cleanup to OMX MCP processes with no live Gemini or OMX launch ancestor', () => {
    assert.deepEqual(
  ...
  it('keeps detached MCP candidates whose ancestor chain is live but unrelated to Gemini or OMX launchers', () => {
  ...
  it('parses valid Windows process discovery rows on win32', () => {
    ...
    try {
      const processes = listOmxProcesses(() => [
        JSON.stringify({ pid: 800, ppid: 1, command: 'node C:/tmp/oh-my-gemini/dist/mcp/state-server.js' }),
        JSON.stringify({ pid: 810, ppid: 42, command: 'node C:/tmp/oh-my-gemini/dist/mcp/trace-server.js' }),
      ].join('\n'));
      assert.deepEqual(processes, [
        { pid: 800, ppid: 1, command: 'node C:/tmp/oh-my-gemini/dist/mcp/state-server.js' },
        { pid: 810, ppid: 42, command: 'node C:/tmp/oh-my-gemini/dist/mcp/trace-server.js' },
      ]);
    } finally {
  ...
  it('feeds parsed Windows rows through existing cleanup candidate selection unchanged', () => {
    ...
    try {
      const parsed = listOmxProcesses(() => [
        ...
        JSON.stringify({ pid: 900, ppid: 1, command: 'node C:/tmp/not-omg/other-server.js' }),
      ].join('\n'));
      ...
      assert.deepEqual(findCleanupCandidates(parsed, 701), [
        {
          pid: 800,
          ppid: 1,
          command: 'node C:/tmp/oh-my-gemini/dist/mcp/memory-server.js',
          reason: 'ppid=1',
        },
        {
          pid: 810,

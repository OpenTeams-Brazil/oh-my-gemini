...
describe('omg agents', () => {
  it('lists project and user native agents with name, description, and model', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-agents-cli-'));
    const home = join(wd, 'home');
    try {
      const projectAgentsDir = join(wd, '.gemini', 'agents');
      const userAgentsDir = join(home, '.gemini', 'agents');
  ...
  it('adds a scaffolded agent TOML file with required fields and commented optional fields', async () => {
    ...
    try {
      ...
      assert.equal(result.status, 0, result.stderr || result.stdout);
      const agentPath = join(wd, '.gemini', 'agents', 'my-helper.toml');

...
describe("omg setup/uninstall shared ownership for native hooks", () => {
  ...
  it("setup preserves user hooks while deduping stale OMX wrappers", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-setup-hooks-ownership-"));
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });
      ...
      const hooksPath = join(wd, ".gemini", "hooks.json");
  ...
  it("uninstall removes only OMX-managed wrappers and preserves user hook content", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-uninstall-hooks-ownership-"));
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });

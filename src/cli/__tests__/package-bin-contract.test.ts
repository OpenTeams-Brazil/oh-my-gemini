...
describe('package bin contract', () => {
  it('declares omg with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    const packageJsonPath = join(process.cwd(), 'package.json');
    const pkg = JSON.parse(readFileSync(packageJsonPath, 'utf-8')) as PackageJson;
    const binaryName = platform() === 'win32' ? 'omg-sparkshell.exe' : 'omg-sparkshell';
    ...
    assert.deepEqual(pkg.bin, { omg: 'dist/cli/omg.js' });
    assert.equal(pkg.scripts?.['build:explore'], 'cargo build -p omg-explore-harness');
    ...
    assert.equal(pkg.scripts?.['test:explore'], 'cargo test -p omg-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js');
    ...
    const binPath = join(process.cwd(), 'dist', 'cli', 'omg.js');
    ...
    const binEntry = results[0]?.files?.find((file) => file.path === 'dist/cli/omg.js');
    assert.ok(binEntry, 'expected npm pack output to include dist/cli/omg.js');

    const packagedHarnessPath = process.platform === 'win32' ? 'bin/omg-explore-harness.exe' : 'bin/omg-explore-harness';
    const packagedHarnessEntry = results[0]?.files?.find((file) => file.path === packagedHarnessPath);
    const packagedHarnessMetaEntry = results[0]?.files?.find((file) => file.path === 'bin/omg-explore-harness.meta.json');
    ...
    const crateManifestEntry = results[0]?.files?.find((file) => file.path === 'crates/omg-explore/Cargo.toml');
    const crateMainEntry = results[0]?.files?.find((file) => file.path === 'crates/omg-explore/src/main.rs');
    ...
    assert.ok(crateManifestEntry, 'expected npm pack output to include crates/omg-explore/Cargo.toml');
    assert.ok(crateMainEntry, 'expected npm pack output to include crates/omg-explore/src/main.rs');
  });
});

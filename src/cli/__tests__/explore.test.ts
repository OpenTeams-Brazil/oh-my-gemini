...
function runOmx(
  ...
  const nodeWrapper = join(cwd, '.omg-test-node.sh');
  if (!existsSync(nodeWrapper)) {
    writeFileSync(nodeWrapper, '#!/bin/sh\nexec node "$@"\n');
    chmodSync(nodeWrapper, 0o755);
  }
  const r = spawnSync(nodeWrapper, [omgBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
describe('loadExplorePrompt', () => {
  it('reads prompt file content', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-prompt-'));
    try {
...
describe('buildExplorePromptWithWikiContext', () => {
  it('injects wiki matches into the explore prompt when local wiki pages exist', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-wiki-'));
    try {
      writePage(wd, {
        filename: 'runtime.md',
        frontmatter: {
      ...
      assert.equal(existsSync(join(wd, '.omg', 'wiki', 'log.md')), false);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('does not mutate wiki logs when building read-only wiki context', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-wiki-log-'));
    try {
      writePage(wd, {
        filename: 'runtime.md',
        frontmatter: {
      ...
      const logPath = join(wd, '.omg', 'wiki', 'log.md');
  ...
  it('warns when wiki pages are missing or too weak', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-no-wiki-'));
    try {
  ...
  it('warns when the wiki directory is missing entirely', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-missing-wiki-'));
    try {
      const prompt = buildExplorePromptWithWikiContext('find auth', wd);
...
describe('resolvePackagedExploreHarnessCommand', () => {
  it('uses a packaged native binary when metadata matches the current platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-packaged-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
      await writeFile(join(binDir, 'omg-explore-harness.meta.json'), JSON.stringify({
  ...
  it('ignores packaged binaries built for a different platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-packaged-mismatch-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
...
describe('resolveExploreHarnessCommand', () => {
  ...
  it('prefers a packaged native harness binary when present', async () => {
    await withPackagedExploreHarnessLock(async () => {
      const wd = await mkdtemp(join(tmpdir(), 'omg-explore-native-'));
      try {
        const binDir = join(wd, 'bin');
        await mkdir(binDir, { recursive: true });
        await writeFile(join(wd, 'package.json'), '{}\n');
  ...
  it('uses an existing repo-built native harness before cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-target-'));
    try {
      ...
      await mkdir(join(wd, 'crates', 'omg-explore'), { recursive: true });
      await writeFile(join(wd, 'crates', 'omg-explore', 'Cargo.toml'), '[package]\nname="omg-explore-harness"\nversion="0.0.0"\n');
  ...
  it('builds cargo fallback command otherwise', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-fallback-'));
    try {
      const crateDir = join(wd, 'crates', 'omg-explore');
      ...
      assert.ok(resolved.args.includes(join(wd, 'crates', 'omg-explore', 'Cargo.toml')));
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('hydrates a native harness for packaged installs before attempting cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-hydrated-'));
    try {
      ...
      const archivePath = join(assetRoot, 'omg-explore-harness-x86_64-unknown-linux-musl.tar.gz');
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          version: '0.8.15',
          assets: [{
            product: 'omg-explore-harness',
            version: '0.8.15',
            platform: 'linux',
            arch: 'x64',
            archive: 'omg-explore-harness-x86_64-unknown-linux-musl.tar.gz',
            binary: 'omg-explore-harness',
            binary_path: 'omg-explore-harness',
            sha256: checksum,
            size: archiveBuffer.length,
            download_url: `${server.baseUrl}/omg-explore-harness-x86_64-unknown-linux-musl.tar.gz`,
          }],
        }, null, 2));
  ...
  it('reports a clean fallback error when the native manifest is unavailable for packaged installs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-missing-manifest-'));
    try {
      await writeFile(join(wd, 'package.json'), JSON.stringify({
        version: '0.8.15',
        repository: { url: 'git+https://github.com/Yeachan-Heo/oh-my-gemini.git' },
...
describe('exploreCommand', () => {
  it('routes qualifying read-only shell commands through sparkshell instead of the direct harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-sparkshell-route-'));
    try {
  ...
  it('falls back to the explore harness when sparkshell backend is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-sparkshell-fallback-'));
    try {
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
        harnessStub,
  ...
  it('falls back to the explore harness when sparkshell is GLIBC-incompatible', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-sparkshell-glibc-'));
    try {
      const sparkshellStub = join(wd, 'sparkshell-stub.sh');
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
        sparkshellStub,
        "#!/bin/sh\necho \"omg-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version \\`GLIBC_2.39' not found\" 1>&2\nexit 1\n",
      );
      await writeFile(
        harnessStub,
        '#!/bin/sh\nprintf "%s\\n" "# Answer" "- fallback harness recovered the lookup"\n',
  ...
  it('passes prompt to harness and preserves markdown stdout', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-cmd-'));
    try {
  ...
  it('works end-to-end through omg explore', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-e2e-'));
    try {
      const stub = join(wd, 'explore-stub.sh');
      await writeFile(
        stub,
  ...
  it('launches an env-node gemini binary while keeping model shell commands allowlisted', async () => {
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        ...
        assert.match(captured, /SHELL=.*omg-explore-allowlist-.*\/bin\/bash$/m);
        ...
        assert.match(captured, /--BLOCKED_STDERR--[\s\S]*not on the omg explore allowlist/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('supports --prompt-file end-to-end with the harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-harness-prompt-file-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const capturePath = join(wd, 'capture.json');
        const geminiStub = await writeEnvNodeGeminiStub(wd, capturePath);
  ...
  it('preserves must-preserve facts in a long noisy summary fixture', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-fidelity-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '%s\n' '# Answer' '## Critical facts' '- MUST: summary mode stayed read-only' '- MUST: blocked command stayed node --version' '- MUST: next command is omg team status <team-name>' '' '## Noise'
i=0
while [ "$i" -lt 80 ]; do
  printf '%s\n' "- distractor line $i"
  i=$((i + 1))
...
`,
        ...
        assert.match(result.stdout, /MUST: next command is omg team status <team-name>/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('preserves buried critical facts in adversarial noisy output', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-adversarial-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
...
`,
  ...
  it('falls back after spark failure and preserves actionable stderr guidance', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-fallback-success-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
...
`,
  ...
  it('reports both failed attempts with codes and final actionable stderr end-to-end', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-explore-fallback-failure-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,

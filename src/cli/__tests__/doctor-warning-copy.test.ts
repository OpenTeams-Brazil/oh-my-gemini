...
describe('omg doctor onboarding warning copy', () => {
...
`.trimStart(),
      ...
      assert.match(
        res.stdout,
        /Config: config\.toml exists but no OMX entries yet \(expected before first setup; run "omg setup --force" once\)/,
      );
      assert.match(
        res.stdout,
        /MCP Servers: 1 servers but no OMX servers yet \(expected before first setup; run "omg setup --force" once\)/,
      );
    } finally {
  ...
  it('warns about retired omg_team_run config left behind after upgrade', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-doctor-copy-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.gemini');
...
`.trimStart(),
      ...
      assert.match(
        res.stdout,
        /Config: retired \[mcp_servers\.omg_team_run\] table still present; run "omg setup --force" to repair the config/,
      );
      assert.match(
        res.stdout,
        /MCP Servers: 1 servers configured, but retired \[mcp_servers\.omg_team_run\] is not supported; run "omg setup --force" to repair the config/,
      );
  ...
  it('warns when explore harness sources are packaged but cargo is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-doctor-explore-copy-'));
    try {
  ...
  it('passes explore harness check when a packaged native binary is present even without cargo', async () => {
    await withPackagedExploreHarnessLock(async () => {
      ...
      try {
        ...
        const packagedMeta = join(packageBinDir, 'omg-explore-harness.meta.json');
        ...
        await writeFile(packagedMeta, JSON.stringify({ binaryName: process.platform === 'win32' ? 'omg-explore-harness.exe' : 'omg-explore-harness', platform: process.platform, arch: process.arch }));
...
`.trimStart(),
  ...
  it('warns when canonical and legacy skill roots overlap', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Legacy skill roots: 1 overlapping skill names between .*\.gemini[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content; Gemini Enable\/Disable Skills may show duplicates until ~\/\.agents\/skills is cleaned up/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('warns when hooks.json is missing OMX-managed native hook coverage', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Native hooks: hooks\.json is missing OMX-managed coverage for PreToolUse, PostToolUse, UserPromptSubmit, Stop; run "omg setup --force" to restore native hooks/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
...
`.trimStart(),
      ...
      assert.match(
        res.stdout,
        /Native hooks: hooks\.json not found even though config\.toml has OMX entries; run "omg setup --force" to restore native hook coverage/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('fails when hooks.json is invalid and native hook coverage cannot be read', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /\[XX\] Native hooks: invalid hooks\.json; Gemini may skip OMX hook coverage until "omg setup --force" repairs it/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('passes when legacy skill root is a link to the canonical skills directory', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Legacy skill roots: ~\/\.agents\/skills links to canonical .*\.gemini[\\/]+skills; treating both paths as one shared skill root/,
      );

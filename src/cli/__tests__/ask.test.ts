...
describe('omg ask', () => {
  it('script usage documents provider-specific long flags from CLI help', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-usage-'));
    try {
  ...
  it('uses package-root advisor script path from non-package cwd and still writes artifact', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-nonroot-'));
    try {
      ...
      assert.ok(artifactPath.startsWith(join(wd, '.omg', 'artifacts', 'claude-')));
  ...
  it('supports claude --print and gemini --prompt end-to-end through omg ask', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-provider-flags-'));
    try {
      const fakeBin = join(wd, 'bin');
      await mkdir(fakeBin, { recursive: true });
  ...
  it('adds Claude skip-permissions for issue-work prompts only', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-issue-permissions-'));
    try {
      const fakeBin = join(wd, 'bin');
      await mkdir(fakeBin, { recursive: true });
  ...
  it('injects --agent-prompt content into final prompt while keeping Original task raw', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-agent-prompt-'));
    try {
      const fakeBin = join(wd, 'bin');
      const geminiHome = join(wd, '.gemini-home');
      const promptsDir = join(geminiHome, 'prompts');
      await mkdir(fakeBin, { recursive: true });
      ...
      const res = runOmx(
        ...
        { PATH: `${fakeBin}:${process.env.PATH || ''}`, GEMINI_HOME: geminiHome },
      );
      if (shouldSkipForSpawnPermissions(res.error)) return;
  ...
  it('fails clearly when --agent-prompt role is missing from prompts directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-ask-agent-prompt-missing-'));
    try {
      const fakeBin = join(wd, 'bin');
      const geminiHome = join(wd, '.gemini-home');
      const promptsDir = join(geminiHome, 'prompts');

...
import {
  ...
  resolveGeminiLaunchPolicy,
  resolveLeaderLaunchPolicyOverride,
  classifyGeminiExecFailure,
  ...
  resolveGeminiConfigPathForLaunch,
  resolveGeminiHomeForLaunch,
...
function expectedLowComplexityModel(geminiHomeOverride?: string): string {
  return getTeamLowComplexityModel(geminiHomeOverride);
}
...
describe("normalizeGeminiLaunchArgs", () => {
  ...
  it("does not forward --madmax and preserves other args", () => {
    assert.deepEqual(
      normalizeGeminiLaunchArgs(["--model", "gpt-5", "--madmax", "--yolo"]),
      [
        "--model",
        "gpt-5",
        "--yolo",
  ...
  it("leaves unrelated args unchanged", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--model", "gpt-5", "--yolo"]), [
      "--model",
      "gpt-5",
      "--yolo",
    ]);
  ...
  it("maps --high to reasoning override", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--high"]), [
      "-c",
  ...
  it("maps --xhigh to reasoning override", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--xhigh"]), [
      "-c",
  ...
  it("uses the last reasoning shorthand when both are present", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--high", "--xhigh"]), [
      "-c",
      'model_reasoning_effort="xhigh"',
    ]);
  });
  ...
  it("--spark is stripped from leader args (model goes to workers only)", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--spark", "--yolo"]), [
      "--yolo",
    ]);
  });

  it("--spark alone produces no leader args", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--spark"]), []);
  });

  it("--madmax-spark adds bypass flag to leader args and is otherwise consumed", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--madmax-spark"]), [
      "--dangerously-bypass-approvals-and-sandbox",
    ]);
  });

  it("--madmax-spark deduplicates bypass when --madmax also present", () => {
    assert.deepEqual(normalizeGeminiLaunchArgs(["--madmax", "--madmax-spark"]), [
      "--dangerously-bypass-approvals-and-sandbox",
    ]);
  });

  it("--madmax-spark does not inject spark model into leader args", () => {
    const args = normalizeGeminiLaunchArgs(["--madmax-spark"]);
    assert.ok(
      !args.includes("--model"),
  ...
  it("does not forward notify-temp flags/selectors to leader codex args", () => {
    ...
    assert.deepEqual(normalizeGeminiLaunchArgs(parsed.passthroughArgs), [
      "--model",
      "gpt-5",
    ]);
  });
  ...
  it("reaps only detached OMX MCP processes without a live Gemini ancestor", async () => {
    const processes: ProcessEntry[] = [
      ...
      { pid: 701, ppid: 700, command: "node /repo/bin/omg.js" },
      {
        pid: 710,
        ppid: 700,
        command: "node /repo/oh-my-gemini/dist/mcp/state-server.js",
      },
      {
        pid: 800,
        ppid: 1,
        command: "node /tmp/oh-my-gemini/dist/mcp/memory-server.js",
      },
      {
        pid: 810,
        ppid: 42,
        command: "node /tmp/oh-my-gemini/dist/mcp/trace-server.js",
      },
      {
        pid: 820,
        ppid: 50,
      ...
      },
      {
        pid: 830,
        ppid: 50,
        command: "node /repo/bin/omg.js autoresearch --topic launch",
      },
      {
        pid: 831,
        ppid: 830,
    ...
    assert.equal(
      ...
      false,
      "launch-safe cleanup must preserve OMX MCP processes still attached to another live Gemini tree",
    );
    assert.equal(
...
describe("cleanupPostLaunchModeStateFiles", () => {
  it("repairs empty or truncated mode state files and still cancels valid siblings", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-postlaunch-mode-cleanup-"));
  ...
  it("retries a transient parse failure before cancelling the rewritten mode state", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-postlaunch-mode-retry-"));
  ...
  it("warns on structurally complete malformed JSON without aborting sibling cleanup", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-postlaunch-mode-malformed-"));
  ...
  it("clears canonical skill-active entries during cleanup and hides them from HUD/overlay readers", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-postlaunch-skill-active-cleanup-"));
...
describe("reapStaleNotifyFallbackWatcher", () => {
  it("stops an existing watcher even when a later startup gate would skip relaunch", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-stale-notify-fallback-"));
    try {
  ...
  it("ignores missing pid files", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-missing-notify-fallback-"));
    try {
      const pidPath = join(cwd, "notify-fallback.pid");
      let killCalls = 0;
  ...
  it("suppresses ESRCH cleanup errors but warns on unexpected failures", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-esrch-notify-fallback-"));
    try {
      const pidPath = join(cwd, "notify-fallback.pid");
      ...
      assert.equal(warned[0]?.message, "[omg] warning: failed to stop stale notify fallback watcher");
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
describe("resolveWorkerSparkModel", () => {
  ...
  it("reads low-complexity team model from config when geminiHomeOverride is provided", async () => {
    ...
    try {
      ...
      assert.equal(
        resolveWorkerSparkModel(["--spark"], geminiHome),
        "gpt-4.1-mini",
      );
    } finally {
...
describe("project launch scope helpers", () => {
  it("reads persisted setup scope when valid", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-launch-scope-"));
    try {
  ...
  it("ignores malformed persisted setup scope", async () => {
    ...
    try {
      await mkdir(join(wd, ".omg"), { recursive: true });
      await writeFile(join(wd, ".omg", "setup-scope.json"), "{not-json");
  ...
  it("uses project GEMINI_HOME when persisted scope is project", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-launch-scope-"));
    try {
      await mkdir(join(wd, ".omg"), { recursive: true });
      await writeFile(
      ...
      assert.equal(resolveGeminiHomeForLaunch(wd, {}), join(wd, ".gemini"));
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("keeps explicit GEMINI_HOME override from env", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-launch-scope-"));
    try {
      await mkdir(join(wd, ".omg"), { recursive: true });
      await writeFile(
  ...
  it("uses explicit GEMINI_HOME config.toml for launch repair overrides", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-launch-scope-"));
    try {
      await mkdir(join(wd, ".omg"), { recursive: true });
      await writeFile(
  ...
  it('resolves GEMINI_HOME for legacy "project-local" persisted scope', async () => {
    const wd = await mkdtemp(join(tmpdir(), "omg-launch-scope-"));
    try {
      await mkdir(join(wd, ".omg"), { recursive: true });
      await writeFile(
...
describe("resolveGeminiLaunchPolicy", () => {
  it("uses detached tmux on macOS when outside tmux and tmux is available", () => {
    assert.equal(
      resolveGeminiLaunchPolicy({}, "darwin", true, false, true, true),
      "detached-tmux",
    );
  });
  ...
  it("uses detached tmux on non-macOS hosts when outside tmux and tmux is available", () => {
    assert.equal(
      resolveGeminiLaunchPolicy({}, "linux", true, false, true, true),
      "detached-tmux",
    );
  });

  it("launches directly on native Windows even when tmux is available", () => {
    assert.equal(resolveGeminiLaunchPolicy({}, "win32", true, true), "direct");
  });
  ...
  it("launches directly when stdin is not a tty outside tmux", () => {
    assert.equal(resolveGeminiLaunchPolicy({}, "linux", true, false, false, true), "direct");
  });

  it("launches directly when stdout is not a tty outside tmux", () => {
    assert.equal(resolveGeminiLaunchPolicy({}, "linux", true, false, true, false), "direct");
  });

  it("launches directly when tmux is unavailable outside tmux", () => {
    assert.equal(resolveGeminiLaunchPolicy({}, "linux", false), "direct");
  });

  it("launches directly on native Windows when tmux is unavailable", () => {
    assert.equal(resolveGeminiLaunchPolicy({}, "win32", false, true), "direct");
  });
});
...
describe("detached tmux new-session sequencing", () => {
  it("buildDetachedSessionBootstrapSteps uses shared HUD height and split-capture ordering", () => {
    ...
    assert.equal(steps[1]?.args[6], "omg-demo");
  ...
  it("buildDetachedSessionBootstrapSteps kills detached tmux session on normal shell exit", () => {
    ...
    assert.match(leaderCmd!, /"omg-demo"/);
    assert.match(leaderCmd!, /exit \$status/);
  });

  it("detached leader command keeps stdin open for the Gemini child", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-detached-leader-stdin-"));
...
`,
  ...
  it("detached leader command preserves cwd and cleanup without shell-quote breakage", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-detached-leader-"));
    const fakeBin = join(cwd, "bin");
...
`,
      ...
      assert.match(log, /tmux:kill-session -t omg-demo/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("detached leader command preserves the detached tmux session on signal-derived exits", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-detached-leader-signal-"));
    const fakeBin = join(cwd, "bin");
    const logPath = join(cwd, "leader.log");
...
`,
      ...
      assert.match(log, /tmux:set-option -sq extended-keys always/);
      assert.match(log, /tmux:set-option -sq extended-keys off/);
      assert.doesNotMatch(log, /tmux:kill-session -t omg-demo/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
`,
  ...
  it("overlapping tmux extended-keys leases restore only after the last holder exits", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-tmux-lease-overlap-"));
    const calls: string[][] = [];
    ...
    const leaseDir = join(cwd, ".omg", "state", "tmux-extended-keys");
  ...
  it("withTmuxExtendedKeys degrades cleanly when tmux option probing fails", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-tmux-lease-fail-"));
    const calls: string[][] = [];
    const result = withTmuxExtendedKeys(
      cwd,
      () => {
  ...
  it("buildDetachedSessionRollbackSteps unregisters hooks before killing session", () => {
    const steps = buildDetachedSessionRollbackSteps(
      "omg-demo",
      "omg-demo:0",
      "omg_resize_launch_demo_0_12",
      "omg_attached_launch_demo_0_12",
    );
    assert.deepEqual(
      steps.map((step) => step.name),
      [
    ...
    assert.equal(steps[0]?.args[3], "omg-demo:0");
    assert.match(steps[0]?.args[4] ?? "", /^client-attached\[\d+\]$/);
    assert.match(steps[1]?.args[4] ?? "", /^client-resized\[\d+\]$/);
    assert.deepEqual(steps[2]?.args, ["kill-session", "-t", "omg-demo"]);
  });
...
describe("buildTmuxSessionName", () => {
  it("uses detached fallback quietly outside git repos", () => {
    const name = buildTmuxSessionName(
      "/tmp/My Repo",
      "omg-1770992424158-abc123",
    );
    assert.equal(name, "omg-my-repo-detached-1770992424158-abc123");
  });

  it("sanitizes invalid characters", () => {
    const name = buildTmuxSessionName("/tmp/@#$", "omg-+++");
    assert.match(
      name,
      /^omg-(unknown|[a-z0-9-]+)-[a-z0-9-]+-(unknown|[a-z0-9-]+)$/,
    );
    assert.equal(name.includes("_"), false);
    assert.equal(name.includes(" "), false);
  });

  it("includes repo name when cwd is inside .omg-worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo.omg-worktrees/launch-feature-x",
      "omg-123-abc",
    );
    assert.match(name, /^omg-my-repo-launch-feature-x-/);
  });

  it("includes repo name for detached worktree paths", () => {
    const name = buildTmuxSessionName(
      "/projects/cool-project.omg-worktrees/launch-detached",
      "omg-456-def",
    );
    assert.match(name, /^omg-cool-project-launch-detached-/);
  });

  it("includes repo name when cwd is inside .omg/worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo/.omg/worktrees/autoresearch-demo",
      "omg-789-ghi",
    );
    assert.match(name, /^omg-my-repo-autoresearch-demo-/);
  });
});
...
describe("buildDetachedTmuxSessionName", () => {
  it("reuses the OMX session id for the detached tmux session name", () => {
    ...
    assert.equal(sessionName, "omg-my-repo-detached-1770992424158-abc123");
  });
});
...
describe("native Windows psmux-compatible tmux resolution", () => {
  it("resolveNativeSessionName uses the shared tmux-aware resolver for current session lookup", async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), "omg-psmux-native-session-"));
...
`,
      ...
      const sessionName = resolveNativeSessionName("/tmp/repo", "omg-abc123", {
        ...process.env,
        TMUX: "1",
        TMUX_PANE: "%7",
      });
  ...
  it("detectDetachedSessionWindowIndex uses the shared tmux-aware resolver on native Windows", async () => {
    ...
    const originalPath = process.env.PATH;
    const originalPathext = process.env.PATHEXT;
    const wd = await mkdtemp(join(tmpdir(), "omg-psmux-window-index-"));
    const fakeBin = join(wd, "bin");
    Object.defineProperty(process, "platform", { value: "win32", configurable: true });
    ...
    try {
      ...
      await writeFile(
        join(fakeBin, "psmux.exe"),
        `#!/bin/sh
if [ "$1" = "display-message" ] && [ "$2" = "-p" ] && [ "$3" = "-t" ] && [ "$4" = "omg-demo" ] && [ "$5" = "#{window_index}" ]; then
  printf '3\\n'
  exit 0
fi
printf 'unexpected:%s\\n' "$*" >&2
...
`,
      ...
      process.env.PATH = fakeBin;
      process.env.PATHEXT = ".EXE";
      assert.equal(detectDetachedSessionWindowIndex("omg-demo"), "3");
    } finally {
      process.env.PATH = originalPath;
      process.env.PATHEXT = originalPathext;
      if (originalPlatform) Object.defineProperty(process, "platform", originalPlatform);
...
describe("injectModelInstructionsBypassArgs", () => {
  it("appends model_instructions_file override by default", () => {
    ...
    assert.deepEqual(args, [
      ...
      "gpt-5",
      "-c",
      'model_instructions_file="/tmp/my-project/GEMINI.md"',
    ]);
  });
  ...
  it("uses session-scoped default model_instructions_file when provided", () => {
    const args = injectModelInstructionsBypassArgs(
      ...
      ["--model", "gpt-5"],
      {},
      "/tmp/my-project/.omg/state/sessions/session-1/GEMINI.md",
    );
    assert.deepEqual(args, [
      "--model",
      "gpt-5",
      "-c",
      'model_instructions_file="/tmp/my-project/.omg/state/sessions/session-1/GEMINI.md"',
    ]);
  });
});

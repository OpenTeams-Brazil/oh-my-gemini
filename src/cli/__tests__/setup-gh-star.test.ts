...
describe('omg setup (gh star hint)', () => {
  it('prints a star hint when GitHub CLI is configured', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-setup-gh-'));
    try {
      ...
      assert.match(res.stdout, /gh repo star Yeachan-Heo\/oh-my-gemini/);
    } finally {
  ...
  it('does not print a star hint when GitHub CLI is missing', async () => {
    ...
    try {
      ...
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.doesNotMatch(res.stdout, /gh repo star Yeachan-Heo\/oh-my-gemini/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

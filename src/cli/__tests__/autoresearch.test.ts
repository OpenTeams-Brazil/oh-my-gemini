...
import { autoresearchCommand, normalizeAutoresearchGeminiArgs, parseAutoresearchArgs } from '../autoresearch.js';
...
async function initRepo(): Promise<string> {
  const raw = await mkdtemp(join(tmpdir(), 'omg-autoresearch-test-'));
...
describe('normalizeAutoresearchGeminiArgs', () => {
  it('adds sandbox bypass by default for autoresearch workers', () => {
    assert.deepEqual(normalizeAutoresearchGeminiArgs(['--model', 'gpt-5']), ['--model', 'gpt-5', '--dangerously-bypass-approvals-and-sandbox']);
  });

  it('deduplicates explicit bypass flags', () => {
    assert.deepEqual(normalizeAutoresearchGeminiArgs(['--dangerously-bypass-approvals-and-sandbox']), ['--dangerously-bypass-approvals-and-sandbox']);
  });

  it('normalizes --madmax to the canonical bypass flag', () => {
    assert.deepEqual(normalizeAutoresearchGeminiArgs(['--madmax']), ['--dangerously-bypass-approvals-and-sandbox']);
  });
});

describe('omg autoresearch', () => {
  it('documents autoresearch in top-level help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-autoresearch-help-'));
    try {
      const result = runOmx(cwd, ['--help']);
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /omg autoresearch\s+Launch thin-supervisor autoresearch with keep\/discard\/reset parity/i);
    } finally {
  ...
  it('routes autoresearch --help to command-local help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-autoresearch-local-help-'));
    try {
      const result = runOmx(cwd, ['autoresearch', '--help']);
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage:[\s\S]*omg autoresearch run <mission-dir>/i);
      assert.match(result.stdout, /omg autoresearch init/i);
  ...
  it('documents --resume in command-local help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-autoresearch-resume-help-'));
    try {
      const result = runOmx(cwd, ['autoresearch', '--help']);
      assert.equal(result.status, 0, result.stderr || result.stdout);
  ...
  it('fails fast when mission dir is missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-autoresearch-missing-arg-'));
    try {
      const result = runOmx(cwd, ['autoresearch']);
      assert.notEqual(result.status, 0, result.stderr || result.stdout);
      assert.match(`${result.stderr}\n${result.stdout}`, /mission-dir|Usage:\s*omg autoresearch <mission-dir>/i);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
cat >"$OMX_TEST_REPO_ROOT/.omg/specs/autoresearch-test-launch/result.json" <<'EOF'
{
  ...
  "draftArtifactPath": "${repo}/.omg/specs/deep-interview-autoresearch-test-launch.md",
  "missionArtifactPath": "${repo}/.omg/specs/autoresearch-test-launch/mission.md",
  "sandboxArtifactPath": "${repo}/.omg/specs/autoresearch-test-launch/sandbox.md",
  "launchReady": true,
  "blockedReasons": []
}
EOF
...
`,
      ...
      execFileSync('chmod', ['+x', fakeGeminiPath], { stdio: 'ignore' });
...
`,
  ...
  it('uses split-window launch for explicit run inside tmux while preserving the interview pane', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omg-autoresearch-run-split-bin-'));
    try {
...
`,
  ...
  it('falls back to foreground execution when tmux split-window fails', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omg-autoresearch-run-fallback-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const logsRoot = join(repo, '.omg', 'logs', 'autoresearch');
  ...
  it('rejects mission directories outside a git repo', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-autoresearch-outside-git-'));
    try {
  ...
  it('rejects autoresearch launch when root ralph mode is already active', async () => {
    ...
    try {
      ...
      await writeFile(
        join(repo, '.omg', 'state', 'ralph-state.json'),
        JSON.stringify({
          active: true,
          mode: 'ralph',
          iteration: 0,
      ...
      const worktreesRoot = join(repo, '.omg', 'worktrees');
...
`,
  ...
  it('stops after repeated noop turns', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omg-autoresearch-noop-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const state = JSON.parse(await readFile(join(repo, '.omg', 'state', 'autoresearch-state.json'), 'utf-8')) as {
        active: boolean;
      };

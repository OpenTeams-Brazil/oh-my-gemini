...
describe('parseTeamStartArgs', () => {
  ...
  it('rejects deprecated omg team ralph syntax', () => {
    assert.throws(
      () => parseTeamStartArgs(['ralph', '--worktree=feature/demo', '4:executor', 'ship', 'it']),
      /Deprecated usage: `omg team ralph \.\.\.` has been removed/,
    );
  });
  ...
  it('reuses the approved team launch hint for a short English follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-followup-en-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omg', 'plans'), { recursive: true });
      await writeFile(
        join(wd, '.omg', 'plans', 'prd-issue-831.md'),
        '# Approved plan\n\nLaunch via omg team 3:executor "Execute approved issue 831 plan"\n',
      );
      await writeFile(join(wd, '.omg', 'plans', 'test-spec-issue-831.md'), '# Test spec\n');
  ...
  it('reuses the approved team launch hint for a short Korean follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-followup-ko-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omg', 'plans'), { recursive: true });
  ...
  it('preserves explicit team staffing overrides while reusing the approved plan task', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-followup-override-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omg', 'plans'), { recursive: true });
...
describe('teamCommand shutdown --force parsing', () => {
  ...
  it('persists cancelled team mode state on shutdown even when no team mode state existed beforehand', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-mode-state-'));
    const previousCwd = process.cwd();
  ...
  it('persists cancelled session-scoped team mode state on shutdown', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-session-mode-state-'));
    const previousCwd = process.cwd();
  ...
  it('does not create session-scoped team mode state on shutdown when only root team mode state exists', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-root-mode-only-'));
    const previousCwd = process.cwd();
  ...
  it('does not create session-scoped team mode state on shutdown when a stale session.json remains', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-stale-session-json-'));
    const previousCwd = process.cwd();
  ...
  it('keeps the shutdown CLI alive while tearing down a shared leader tmux session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-shared-cli-'));
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, '.omg', 'state', 'team', 'shared-shutdown-cli')), false);
  ...
  it('keeps the shutdown command alive when executed inside the leader pane PTY', { concurrency: false }, async (t) => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-shutdown-shared-in-pane-'));
    try {
...
describe('teamCommand api', () => {
  it('builds leader monitoring hints that keep team status visible while ON', () => {
    ...
    assert.equal(hints[0], 'leader_check: omg team status my-team');
    assert.match(hints[1] ?? '', /while ON, keep checking state/);
    assert.match(hints[1] ?? '', /sleep 30 && omg team status my-team/);
  });

  it('prints team-specific help for omg team --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.match(logs[0] ?? '', /Usage: omg team \[N:agent-type\]/);
      assert.match(logs[0] ?? '', /omg team api <operation>/);
      assert.match(logs[0] ?? '', /dedicated worktrees automatically by default/);
      assert.match(logs[0] ?? '', /--worktree is deprecated/);
      assert.match(logs[0] ?? '', /native Gemini subagents for small in-session fanout/);
    } finally {
      console.log = originalLog;
    }
  });

  it('prints team-specific help for omg team help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints team-api-specific help for omg team api --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omg team api <operation>/);
  ...
  it('prints team-api-specific help for omg team api help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints operation-specific help for omg team api <operation> --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omg team api send-message --input <json> \[--json\]/);
  ...
  it('prints operation-specific help for omg team api <operation> help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omg team api claim-task --input <json> \[--json\]/);
  ...
  it('prints event query help for omg team api read-events help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omg team api read-events --input <json> \[--json\]/);
  ...
  it('executes read-events via CLI api with canonical JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-api-read-events-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omg team api read-events');
  ...
  it('executes read-idle-state via CLI api with structured JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-api-read-idle-state-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omg team api read-idle-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes read-stall-state via CLI api with structured JSON results', async () => {
    ...
    try {
      ...
      await mkdir(join(wd, '.omg', 'state', 'team', 'api-read-stall'), { recursive: true });
      await writeFile(join(wd, '.omg', 'state', 'team', 'api-read-stall', 'leader-attention.json'), JSON.stringify({
      ...
      assert.equal(envelope.command, 'omg team api read-stall-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes CLI interop operation with stable JSON envelope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-api-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omg team api send-message');
      assert.equal(envelope.ok, true);
  ...
  it('returns deterministic JSON errors for invalid api usage with --json', async () => {
    ...
    try {
      ...
      assert.equal(envelope.schema_version, '1.0');
      assert.equal(typeof envelope.timestamp, 'string');
      assert.equal(envelope.command, 'omg team api');
      assert.equal(envelope.ok, false);
      assert.equal(envelope.operation, 'unknown');
      assert.equal(envelope.error?.code, 'invalid_input');
      assert.match(envelope.error?.message ?? '', /Usage: omg team api/);
  ...
  it('supports claim-safe lifecycle via CLI api (create -> claim -> transition)', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-api-lifecycle-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('accepts new canonical event types via CLI api append-event', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-api-event-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
...
describe('teamCommand status', () => {
  it('prints pane ids and sparkshell hint when tmux panes are recorded', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-panes-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-team', 'tasks', 'task-1.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omg', 'state', 'team', 'pane-team', 'tasks', 'task-1.json'), 'utf-8')) as Record<string, unknown>,
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-team', 'tasks', 'task-2.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omg', 'state', 'team', 'pane-team', 'tasks', 'task-2.json'), 'utf-8')) as Record<string, unknown>,
      ...
      config.workers[0]!.team_state_root = '/tmp/pane-team/.omg/state';
      config.workers[1]!.team_state_root = '/tmp/pane-team/.omg/state';
      ...
      const manifestPath = join(wd, '.omg', 'state', 'team', 'pane-team', 'manifest.v2.json');
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-team', 'config.json'),
      ...
      assert.match(output, /sparkshell_hint: omg sparkshell --tmux-pane <pane-id> --tail-lines 400/);
      assert.match(output, /inspect_leader: omg sparkshell --tmux-pane %10 --tail-lines 400/);
      assert.match(output, /inspect_hud: omg sparkshell --tmux-pane %11 --tail-lines 400/);
      assert.match(output, /inspect_worker-1: omg sparkshell --tmux-pane %21 --tail-lines 400/);
      assert.match(output, /inspect_worker-2: omg sparkshell --tmux-pane %22 --tail-lines 400/);
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
  ...
  it('returns pane ids and sparkshell hint in JSON mode', async () => {
    ...
    try {
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-json-team', 'tasks', 'task-1.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omg', 'state', 'team', 'pane-json-team', 'tasks', 'task-1.json'), 'utf-8')) as Record<string, unknown>,
      ...
      config.workers[0]!.team_state_root = '/tmp/pane-json-team/.omg/state';
      ...
      const manifestPath = join(wd, '.omg', 'state', 'team', 'pane-json-team', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as {
        leader_pane_id?: string | null;
        hud_pane_id?: string | null;
        workers?: Array<{ pane_id?: string }>;
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-json-team', 'config.json'),
        `${JSON.stringify(config, null, 2)}\n`,
      );
      await writeFile(
        manifestPath,
      ...
      assert.equal(payload.command, 'omg team status');
      ...
      assert.deepEqual(payload.panes?.recommended_inspect_team_state_roots, { 'worker-1': '/tmp/pane-json-team/.omg/state' });
      ...
      assert.deepEqual(payload.panes?.recommended_inspect_task_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/tasks/task-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_approval_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/approvals/task-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_state_dirs, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_status_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/status.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_heartbeat_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/heartbeat.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_identity_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/identity.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_inbox_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/inbox.md` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_mailbox_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/mailbox/worker-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_request_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/shutdown-request.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_ack_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/workers/worker-1/shutdown-ack.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dir_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_config_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/config.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_manifest_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/manifest.v2.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_events_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/events/events.ndjson` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dispatch_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/dispatch/requests.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_phase_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/phase.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_monitor_snapshot_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/monitor-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_summary_snapshot_paths, { 'worker-1': `${wd}/.omg/state/team/pane-json-team/summary-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_panes, { 'worker-1': '%41' });
      assert.equal(payload.panes?.recommended_inspect_command, 'omg sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_commands, ['omg sparkshell --tmux-pane %41 --tail-lines 400']);
      assert.equal(payload.panes?.recommended_inspect_summary, 'target=worker-1 pane=%41 cli=claude role=executor alive=false turn_count=5 turns_without_progress=0 reason=dead_worker state=working task=1 subject=Recover worker-1 progress command=omg sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_items, [{
        ...
        team_state_root: '/tmp/pane-json-team/.omg/state',
        ...
        task_claim_lock_path: `${wd}/.omg/state/team/pane-json-team/claims/task-1.lock`,
        ...
        task_path: `${wd}/.omg/state/team/pane-json-team/tasks/task-1.json`,
        approval_path: `${wd}/.omg/state/team/pane-json-team/approvals/task-1.json`,
        worker_state_dir: `${wd}/.omg/state/team/pane-json-team/workers/worker-1`,
        worker_status_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/status.json`,
        worker_heartbeat_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/heartbeat.json`,
        worker_identity_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/identity.json`,
        worker_inbox_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/inbox.md`,
        worker_mailbox_path: `${wd}/.omg/state/team/pane-json-team/mailbox/worker-1.json`,
        worker_shutdown_request_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/shutdown-request.json`,
        worker_shutdown_ack_path: `${wd}/.omg/state/team/pane-json-team/workers/worker-1/shutdown-ack.json`,
        team_dir_path: `${wd}/.omg/state/team/pane-json-team`,
        team_config_path: `${wd}/.omg/state/team/pane-json-team/config.json`,
        team_manifest_path: `${wd}/.omg/state/team/pane-json-team/manifest.v2.json`,
        team_events_path: `${wd}/.omg/state/team/pane-json-team/events/events.ndjson`,
        team_dispatch_path: `${wd}/.omg/state/team/pane-json-team/dispatch/requests.json`,
        team_phase_path: `${wd}/.omg/state/team/pane-json-team/phase.json`,
        team_monitor_snapshot_path: `${wd}/.omg/state/team/pane-json-team/monitor-snapshot.json`,
        team_summary_snapshot_path: `${wd}/.omg/state/team/pane-json-team/summary-snapshot.json`,
        command: 'omg sparkshell --tmux-pane %41 --tail-lines 400',
      }]);
      ...
      assert.equal(payload.panes?.sparkshell_hint, 'omg sparkshell --tmux-pane <pane-id> --tail-lines 400');
      assert.deepEqual(payload.panes?.sparkshell_commands, {
        leader: 'omg sparkshell --tmux-pane %30 --tail-lines 400',
        hud: 'omg sparkshell --tmux-pane %31 --tail-lines 400',
  ...
  it('prints workspace_mode in text status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const teamDir = join(wd, '.omg', 'state', 'team', 'workspace-mode-team');
  ...
  it('returns workspace_mode in JSON status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-json-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      config.workspace_mode = 'worktree';
      const teamDir = join(wd, '.omg', 'state', 'team', 'workspace-mode-json-team');
      const configPath = join(teamDir, 'config.json');
      const manifestPath = join(teamDir, 'manifest.v2.json');
      await mkdir(teamDir, { recursive: true });
      await writeFile(configPath, JSON.stringify(config, null, 2));
  ...
  it('returns a missing envelope in JSON mode when team state is absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-missing-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('records leader runtime activity when team status is read', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-activity-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const activity = JSON.parse(await readFile(join(wd, '.omg', 'state', 'leader-runtime-activity.json'), 'utf-8')) as {
  ...
  it('supports custom tail lines for generated sparkshell commands', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-status-tail-lines-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const manifestPath = join(wd, '.omg', 'state', 'team', 'pane-tail-team', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as {
        workers?: Array<{ pane_id?: string }>;
      };
      manifest.workers = config.workers.map((worker) => ({
      ...
      await writeFile(
        join(wd, '.omg', 'state', 'team', 'pane-tail-team', 'config.json'),
        `${JSON.stringify(config, null, 2)}\n`,
      );
      await writeFile(
        manifestPath,
      ...
      assert.match(logs.join('\n'), /inspect_worker-1: omg sparkshell --tmux-pane %51 --tail-lines 600/);
      ...
      assert.equal(payload.panes?.sparkshell_commands?.['worker-1'], 'omg sparkshell --tmux-pane %51 --tail-lines 550');
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
...
describe('teamCommand await', () => {
  it('returns next canonical event for a team in JSON mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-team-await-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
...
`,
    ...
    try {
      ...
      await rm(join(wd, '.omg', 'state', 'team-state.json'), { force: true });
...
`,
    ...
    try {
      ...
      await withMockPromptModeGeminiAllowed(() =>
        withoutTeamTestWorkerEnv(() => teamCommand(['1:executor', teamTask])));
      await writeFile(
        join(wd, '.omg', 'state', 'team', teamName, 'phase.json'),
        JSON.stringify({
  ...
  it('rejects legacy omg team ralph launches at command entry', async () => {

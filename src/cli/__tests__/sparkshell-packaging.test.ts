...
describe('sparkshell packaging scaffold', () => {
  it('registers native helper scripts but keeps staged native artifacts out of npm releases', () => {
    ...
    const stagedRoot = mkdtempSync(join(tmpdir(), 'omg-sparkshell-stage-'));
    ...
    assert.match(testScriptSource, /'crates', 'omg-sparkshell', 'Cargo\.toml'/);
    assert.doesNotMatch(testScriptSource, /'native', 'omg-sparkshell', 'Cargo\.toml'/);
    ...
    try {
      ...
      const buildResult = spawnSync(process.execPath, [buildScriptPath], {
        ...
        env: {
          ...process.env,
          OMG_SPARKSHELL_MANIFEST: join(process.cwd(), 'crates', 'omg-sparkshell', 'Cargo.toml'),
          OMG_SPARKSHELL_STAGE_DIR: stagedRoot,
        },
      });

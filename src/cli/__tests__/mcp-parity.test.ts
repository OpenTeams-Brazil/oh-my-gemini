...
describe("mcpParityCommand", () => {
  it("supports state write/read parity via CLI", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-mcp-parity-state-"));
  ...
  it("supports notepad and project-memory parity via CLI", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-mcp-parity-memory-"));
    const logs = captureLogs();
  ...
  it("supports trace summary parity via CLI", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-mcp-parity-trace-"));
    const logs = captureLogs();
  ...
  it("supports wiki parity via CLI", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omg-mcp-parity-wiki-"));
    const logs = captureLogs();

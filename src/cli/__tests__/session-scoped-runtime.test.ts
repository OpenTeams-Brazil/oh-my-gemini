...
function runOmx(cwd: string, ...args: string[]) {
  return spawnSync(process.execPath, [omgBin, ...args], {
    cwd,
    encoding: 'utf-8',
  });
}
...
describe('CLI session-scoped state parity', () => {
  it('status and cancel include session-scoped states', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-cli-session-scope-'));
    try {
      await mkdir(join(wd, '.omg', 'state'), { recursive: true });
      await writeFile(join(wd, '.omg', 'state', 'session.json'), JSON.stringify({ session_id: 'sess1' }));
      const scopedDir = join(wd, '.omg', 'state', 'sessions', 'sess1');
  ...
  it('cancels linked ultrawork when Ralph is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-cli-ralph-link-'));
    try {
  ...
  it('does not mutate unrelated sessions when cancelling current session mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-cli-cross-session-'));
    try {
      const stateDir = join(wd, '.omg', 'state');

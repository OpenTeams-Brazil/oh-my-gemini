...
describe('omg session help', () => {
  it('documents the session search command in help output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omg-session-help-'));
    try {
      const mainHelp = runOmx(cwd, ['--help']);
      assert.equal(mainHelp.status, 0, mainHelp.stderr || mainHelp.stdout);
      assert.match(mainHelp.stdout, /omg resume\s+Resume a previous interactive Gemini session/i);
      assert.match(mainHelp.stdout, /omg autoresearch\s+Launch thin-supervisor autoresearch with keep\/discard\/reset parity/i);
      assert.match(mainHelp.stdout, /omg session\s+Search prior local session transcripts/i);

      const sessionHelp = runOmx(cwd, ['session', '--help']);
      assert.equal(sessionHelp.status, 0, sessionHelp.stderr || sessionHelp.stdout);
      assert.match(sessionHelp.stdout, /omg session search <query>/i);

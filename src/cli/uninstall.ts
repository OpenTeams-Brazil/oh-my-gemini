/**
 * omg uninstall - Remove oh-my-gemini configuration and installed artifacts
 */
...
function detectOmxConfigArtifacts(config: string): {
  ...
  const hasTuiSection =
    /^\[tui\]/m.test(config) &&
    config.includes("oh-my-gemini (OMX) Configuration");

  const hasTopLevelKeys =
    /^\s*notify\s*=.*node/m.test(config) ||
    /^\s*model_reasoning_effort\s*=/m.test(config) ||
    /^\s*developer_instructions\s*=.*oh-my-gemini/m.test(config);
...
async function removeAgentsMd(
  ...
  if (options.verbose)
    console.log(`  ${options.dryRun ? "Would remove" : "Removed"} GEMINI.md`);
  return true;
}
...
async function removeCacheDirectory(
  ...
  const omgDir = join(projectRoot, ".omg");
  if (!existsSync(omgDir)) return false;

  if (!options.dryRun) {
    await rm(omgDir, { recursive: true, force: true });
  }
  if (options.verbose)
    console.log(`  ${options.dryRun ? "Would remove" : "Removed"} ${omgDir}`);
  return true;
}
...
async function detectLegacySkillRootWarning(
  ...
  if (overlap.overlappingSkillNames.length === 0) {
    return (
      ...
      "omg uninstall does not remove that historical root automatically; " +
      "archive or remove ~/.agents/skills if Gemini still shows stale or duplicate skills"
    );
  }
  ...
  return (
    ...
    "omg uninstall only removes the active canonical skill root; " +
    "archive or remove ~/.agents/skills if Gemini still shows duplicates"
  );
}
...
function printSummary(summary: UninstallSummary, dryRun: boolean): void {
  ...
  if (summary.hooksFileRemoved) {
    console.log(`  ${prefix} OMX-managed entries in .gemini/hooks.json`);
  }
  ...
  if (summary.agentsMdRemoved) {
    console.log(`  ${prefix} GEMINI.md`);
  }
  if (summary.cacheDirectoryRemoved) {
    console.log(`  ${prefix} .omg/ cache directory`);
  }
  ...
  if (totalActions === 0) {
    console.log(
      "  Nothing to remove. oh-my-gemini does not appear to be installed.",
    );
  }
}
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  console.log("oh-my-gemini uninstall");
  console.log("=====================\n");
  if (dryRun) {
  ...
  if (keepConfig) {
    ...
    const configResult = await cleanConfig(scopeDirs.geminiConfigFile, {
      dryRun,
      verbose,
    });
  ...
  summary.hooksFileRemoved = await removeHooksFile(scopeDirs.geminiHooksFile, {
    dryRun,
    verbose,
  });
  console.log(
  ...
  // Step 6: Remove GEMINI.md and optionally .omg/ cache directory
  ...
  if (purge) {
    ...
    const scopeFile = join(projectRoot, ".omg", "setup-scope.json");
    const hudConfig = join(projectRoot, ".omg", "hud-config.json");
  ...
  if (!dryRun) {
    console.log(
      '\noh-my-gemini has been uninstalled. Run "omg setup" to reinstall.',
    );
  } else {

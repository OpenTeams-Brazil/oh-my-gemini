...
export const AUTORESEARCH_HELP = `omg autoresearch - Launch OMX autoresearch with thin-supervisor parity semantics
...
Arguments:
  (no args)        Launch an interactive Gemini session that activates deep-interview --autoresearch,
                   writes .omg/specs artifacts, then launches only after explicit confirmation.
...
Behavior:
  - deep-interview intake writes canonical artifacts under .omg/specs before launch
...
function buildAutoresearchDeepInterviewAppendix(): string {
  return [
    ...
    'Do not start tmux, do not launch `omg autoresearch`, and do not bypass the user confirmation boundary.',
    'When the user confirms launch and the evaluator is concrete, persist canonical artifacts under `.omg/specs/` using the contracts in `src/cli/autoresearch-intake.ts`.',
...
async function writeAutoresearchDeepInterviewAppendixFile(repoRoot: string): Promise<string> {
  ...
  const dir = join(repoRoot, '.omg', 'autoresearch');
...
async function runGuidedAutoresearchDeepInterview(
  ...
  if (!result) {
    throw new Error('autoresearch deep-interview did not produce .omg/specs launch artifacts.');
  }
...
function listHudWatchPaneIdsInCurrentWindow(currentPaneId?: string): string[] {
  ...
  return (result.stdout || '')
    ...
    .filter((pane) => /\bomg\b.*\bhud\b.*--watch/i.test(pane.startCommand || ''))
...
function launchAutoresearchInSplitPane(args: {
  ...
  const omgPath = resolveOmxEntryPath();

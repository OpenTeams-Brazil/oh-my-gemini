...
import { resolveGeminiPane } from '../scripts/tmux-hook-engine.js';
...
const DEFAULT_CONFIG: TmuxHookConfig = {
  ...
  prompt_template: 'Continue from current mode state. [OMG_TMUX_INJECT]',
...
const HELP = `
Usage:
  omg tmux-hook init       Create .omg/tmux-hook.json
  omg tmux-hook status     Show config + runtime state summary
  omg tmux-hook validate   Validate config and tmux target reachability
  omg tmux-hook test       Run a synthetic notify-hook turn (end-to-end)
`;
...
function omgDir(cwd = process.cwd()): string {
  return join(cwd, '.omg');
}

function tmuxHookConfigPath(cwd = process.cwd()): string {
  return join(omgDir(cwd), 'tmux-hook.json');
}

function tmuxHookStatePath(cwd = process.cwd()): string {
  return join(omgDir(cwd), 'state', 'tmux-hook-state.json');
}

function tmuxHookLogPath(cwd = process.cwd()): string {
  return join(omgDir(cwd), 'logs', `tmux-hook-${new Date().toISOString().split('T')[0]}.jsonl`);
}
...
async function readValidatedConfig(cwd = process.cwd()): Promise<TmuxHookConfig> {
  ...
  if (!existsSync(configPath)) {
    throw new Error('tmux-hook config missing. Run: omg tmux-hook init');
  }
  ...
  if (!existsSync(configPath)) {
    ...
    if (initResult.created) {
      ...
      if (initResult.usedPlaceholderTarget) {
        console.log('Could not auto-detect a tmux target. Edit `.omg/tmux-hook.json` when ready.');
...
function detectInitialTarget(): InitialTargetDetection | null {
  const canonicalPane = resolveGeminiPane();
  if (canonicalPane) {
...
async function initTmuxHookConfig(opts?: { silent?: boolean; cwd?: string }): Promise<InitConfigResult> {
  ...
  const configPath = tmuxHookConfigPath(cwd);
  await mkdir(omgDir(cwd), { recursive: true });
...
async function testTmuxHook(args: string[]): Promise<void> {
  ...
  const payload = {
    ...
    'thread-id': threadId,
    'turn-id': turnId,
    'input-messages': ['omg tmux-hook test'],
  ...
  console.log('Check: .omg/logs/tmux-hook-YYYY-MM-DD.jsonl for skip/reason codes.');
}

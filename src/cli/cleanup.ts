...
const HELP = [
  'Usage: omg cleanup [--dry-run]',
  '',
  'Kill orphaned OMX MCP server processes and remove stale OMX /tmp directories left behind by previous Gemini App sessions.',
  '',
  'Options:',
...
function resolveProtectedRootPid(
  ...
  while (typeof pid === 'number' && pid > 1) {
    ...
    if (command && isGeminiSessionProcess(command)) return pid;
...
export function findLaunchSafeCleanupCandidates(
  ...
  return findCleanupCandidates(processes, currentPid).filter((candidate) => {
    ...
    return (
      !hasAncestorMatching(processByPid, candidate.pid, isGeminiSessionProcess) &&

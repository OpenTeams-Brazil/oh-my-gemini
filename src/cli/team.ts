...
function readPersistedTeamFollowupState(cwd: string): {
  ...
  const path = join(cwd, '.omg', 'state', 'team-state.json');
...
const TEAM_HELP = `
Usage: omg team [N:agent-type] "<task description>"
       omg team status <team-name> [--json] [--tail-lines <100-1000>]
       omg team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]
       omg team resume <team-name>
       omg team shutdown <team-name> [--force] [--confirm-issues]
       omg team api <operation> [--input <json>] [--json]
       omg team api --help

Notes:
  team workers use dedicated worktrees automatically by default.
  --worktree is deprecated for omg team and is now only a backward-compatible no-op override.
  use native Gemini subagents for small in-session fanout; use omg team for durable tmux/state/worktree coordination.

Examples:
  omg team 3:executor "fix failing tests"
  omg team status my-team
  omg team status my-team --json
  omg team status my-team --tail-lines 600
...
const TEAM_API_OPERATION_NOTES: Partial<Record<TeamApiOperation, string>> = {
  ...
  'read-events': 'Events are returned in canonical form; worker_idle log entries normalize to type worker_state_changed with source_type worker_idle. wakeable_only defaults to false; set wakeable_only=true to mirror omg team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
  'await-event': 'Waits for the next matching event and returns status=timeout when no matching event arrives before timeout_ms. wakeable_only defaults to false; set wakeable_only=true to mirror omg team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
  ...
  switch (field) {
    ...
    case 'metadata':
      return {
        ...
        diff_path: '/tmp/team/worktrees/worker-1/.omg/diff.md',
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (token === '--tail-lines') {
      ...
      if (!Number.isFinite(parsed) || parsed < MIN_SPARKSHELL_TAIL_LINES || parsed > MAX_SPARKSHELL_TAIL_LINES) {
        throw new Error(`Usage: omg team status <team-name> [--json] [--tail-lines <${MIN_SPARKSHELL_TAIL_LINES}-${MAX_SPARKSHELL_TAIL_LINES}>]`);
      }
      return parsed;
    }
...
function renderTeamPaneStatus(
  ...
  if (paneStatus.sparkshell_hint) {
    console.log('sparkshell_hint: omg sparkshell --tmux-pane <pane-id> --tail-lines 400');
  }
...
function parseTeamArgs(args: string[], cwd: string = process.cwd()): ParsedTeamArgs {
  ...
  if (tokens[0]?.toLowerCase() === 'ralph') {
    throw new Error('Deprecated usage: `omg team ralph ...` has been removed. Use `omg team ...` or run `omg ralph ...` separately.');
  }
...
async function persistTeamShutdownModeState(
  ...
  const sessionStatePath = join(cwd, '.omg', 'state', 'session.json');
...
export function buildLeaderMonitoringHints(teamName: string): string[] {
  ...
  return [
    `leader_check: omg team status ${sanitized}`,
    `leader_loop_hint: while ON, keep checking state (example: sleep 30 && omg team status ${sanitized})`,
  ];
}
...
export async function teamCommand(args: string[], _options: TeamCliOptions = {}): Promise<void> {
  ...
  if (subcommand === 'status') {
    ...
    const wantsJson = teamArgs.includes('--json');
    if (!name) throw new Error('Usage: omg team status <team-name> [--json]');
  ...
  if (subcommand === 'await') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omg team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]');
    const wantsJson = teamArgs.includes('--json');
  ...
  if (subcommand === 'resume') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omg team resume <team-name>');
  ...
  if (subcommand === 'shutdown') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omg team shutdown <team-name> [--force] [--confirm-issues]');
    ...
    await persistTeamShutdownModeState(
      ...
      console.warn('[omg] warning: failed to persist team mode shutdown state', {
        team: name,

...
const PACKAGE_NAME = 'oh-my-gemini';
...
function updateStatePath(cwd: string): string {
  return join(cwd, '.omg', 'state', 'update-check.json');
}
...
export async function maybeCheckAndPromptUpdate(
  ...
  const approved = await updateDependencies.askYesNo(
    `[omg] Update available: v${current} → v${latest}. Update now? [Y/n] `,
  );
  if (!approved) return;

  console.log(`[omg] Running: npm install -g ${PACKAGE_NAME}@latest`);
  const result = updateDependencies.runGlobalUpdate();

  if (result.ok) {
    await updateDependencies.setup({ force: true });
    console.log(`[omg] Updated to v${latest}. Restart to use new code.`);
  } else {
    console.log('[omg] Update failed. Run manually: npm install -g oh-my-gemini@latest');
  }
}

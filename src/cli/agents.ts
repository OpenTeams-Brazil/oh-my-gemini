...
import { geminiAgentsDir, projectGeminiAgentsDir } from '../utils/paths.js';

export const RESERVED_NATIVE_AGENT_NAMES = new Set(['default', 'worker', 'explorer']);
const DEFAULT_AGENT_MODEL = 'gpt-5.4';
const AGENTS_USAGE = [
  'Usage:',
  '  omg agents list [--scope user|project]',
  '  omg agents add <name> [--scope user|project] [--force]',
  '  omg agents edit <name> [--scope user|project]',
  '  omg agents remove <name> [--scope user|project] [--force]',
  '',
  'Manage Gemini native agent TOML files under ~/.gemini/agents/ or ./.gemini/agents/.',
  '',
  'Notes:',
...
function normalizeAgentName(name: string): string {
  ...
  if (isReservedNativeAgentName(trimmed)) {
    throw new Error(`"${trimmed}" is reserved by Gemini built-in agents`);
  }
  return trimmed;
}

function resolveAgentsDir(scope: AgentScope, cwd = process.cwd()): string {
  return scope === 'project' ? projectGeminiAgentsDir(cwd) : geminiAgentsDir();
}
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.omg', 'setup-scope.json');
  ...
  return existsSync(join(cwd, '.gemini')) ? 'project' : 'user';
}
...
function scaffoldAgentToml(name: string): string {
  ...
  return [
    `# Gemini native agent: ${normalized}`,
    `name = "${normalized}"`,
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: omg agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: omg agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: omg agents remove <name>');

...
const HELP = [
	"Usage: omg adapt <target> <probe|status|init|envelope|doctor> [--json] [--write]",
	"",
	"Targets:",
	...
	"  init      Preview or write OMX-owned adapter artifacts under .omg/adapters/<target>/...",
	...
	"  --write   Only valid with init; write adapter artifacts under .omg/adapters/<target>/...",
	"",
	"Examples:",
...
function targetHelp(target: AdaptTarget): string {
	...
	return [
		`Usage: omg adapt ${target} <${ADAPT_SUBCOMMANDS.join("|")}> [--json] [--write]`,
		"",
		descriptor.summary,
		"",
		"Notes:",
...
export async function adaptCommand(
	...
	if (write && subcommand !== "init") {
		throw new Error("--write is only supported with omg adapt <target> init");
	}

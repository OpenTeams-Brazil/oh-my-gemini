...
export function buildAutoresearchDeepInterviewPrompt(
  ...
  return [
    ...
    'Run the deep-interview skill in autoresearch mode for `omg autoresearch`.',
    'Guide the user through research topic definition, evaluator readiness, keep policy, and slug/session naming.',
    'Do not launch tmux or run `omg autoresearch` yourself.',
    'When the user confirms launch and the evaluator is concrete, write/update these canonical artifacts under `.omg/specs/`:',
...
export function spawnAutoresearchTmux(missionDir: string, slug: string): void {
  ...
  const sessionName = `omg-autoresearch-${slug}`;
  ...
  const omgPath = resolveOmxEntryPath() ?? resolve(join(__dirname, 'omg.js'));
  const cmd = `${shellQuote(process.execPath)} ${shellQuote(omgPath)} autoresearch ${shellQuote(missionDir)}`;

...
export const RALPH_HELP = `omg ralph - Launch Gemini with ralph persistence mode active
...
PRD mode:
  Ralph initializes persistence artifacts in .omg/ so PRD and progress
  state can survive across Gemini sessions. Provide task text either as
...
Common patterns:
  omg ralph "Fix flaky notify-hook tests"
  omg ralph --prd "Ship release checklist automation"
  omg ralph --model gpt-5 "Refactor state hydration"
  omg ralph -- --task-with-leading-dash
`;
...
export function assertRequiredRalphPrdJson(cwd: string, args: readonly string[]): void {
  ...
  if (!existsSync(requiredPath)) {
    throw new Error(`[ralph] Missing required PRD.json at ${REQUIRED_RALPH_PRD_JSON_PATH}. Create the file before running \`omg ralph --prd ...\`.`);
  }
...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- Prefer Gemini native subagents for independent parallel subtasks.',
    '- Treat `.omg/state/subagent-tracking.json` as the native subagent activity ledger for this session.',
    ...
    options.noDeslop
      ...
      : `- Step 7.5 must run oh-my-gemini:ai-slop-cleaner in standard mode on changed files only, using the repo-relative paths listed in \`${options.changedFilesPath}\`.`,
    options.noDeslop
...
async function writeRalphSessionFiles(
  ...
  await writeFile(
    instructionsPath,
    `${buildRalphAppendInstructions(task, { changedFilesPath: '.omg/ralph/changed-files.txt', noDeslop: options.noDeslop, approvedHint: options.approvedHint ?? null })}\n`,
  );
  return { instructionsPath, changedFilesPath: '.omg/ralph/changed-files.txt' };
}
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_tracking_path: '.omg/state/subagent-tracking.json',
    native_subagent_policy: 'Parallel Gemini subagents are allowed for independent work, but phase completion must wait for active native subagent threads to finish.',
  ...
  console.log('[ralph] Ralph persistence mode active. Launching Gemini...');

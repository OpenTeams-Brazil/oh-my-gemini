...
export const ASK_USAGE = [
  'Usage: omg ask <claude|gemini> <question or task>',
  '   or: omg ask <claude|gemini> -p "<prompt>"',
  '   or: omg ask claude --print "<prompt>"',
  '   or: omg ask gemini --prompt "<prompt>"',
  '   or: omg ask <claude|gemini> --agent-prompt <role> "<prompt>"',
  '   or: omg ask <claude|gemini> --agent-prompt=<role> --prompt "<prompt>"',
].join('\n');
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "omg setup" to install prompts.`);
  }

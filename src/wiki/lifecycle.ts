...
import { geminiHome, omgProjectMemoryPath } from '../utils/paths.js';
import {
  appendLogUnsafe,
  getWikiDir,
  listPages,
...
function loadWikiConfig(root: string): WikiConfig {
  const candidates = [join(root, '.omg-config.json'), join(geminiHome(), '.omg-config.json')];
...
export function onSessionStart(data: { cwd?: string }): { additionalContext?: string } {
  try {
    ...
    const summary = [
      `[OMX Wiki: ${pages.length} pages at .omg/wiki/]`,
      '',
...
function feedProjectMemory(root: string): void {
  try {
    const projectMemoryPath = omgProjectMemoryPath(root);

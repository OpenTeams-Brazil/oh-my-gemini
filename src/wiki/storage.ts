...
import { omgWikiDir } from '../utils/paths.js';
import {
  type WikiLogEntry,
  type WikiPage,
...
export function getWikiDir(root: string): string {
  return omgWikiDir(root);
}

export function ensureWikiDir(root: string): string {
  const wikiDir = getWikiDir(root);
  mkdirSync(wikiDir, { recursive: true });
  const omgRoot = join(root, '.omg');
  mkdirSync(omgRoot, { recursive: true });
  const gitignorePath = join(omgRoot, '.gitignore');

...
describe('session-idle tmux tail dedupe integration', () => {
  ...
  beforeEach(async () => {
    clearEnv();
    const root = await mkdtemp(join(tmpdir(), 'omg-session-idle-tail-'));
    tempGeminiHome = join(root, '.gemini');
    projectPath = join(root, 'project');
    await mkdir(tempGeminiHome, { recursive: true });
    await mkdir(projectPath, { recursive: true });
    await writeFile(join(tempGeminiHome, '.omg-config.json'), JSON.stringify({
      notifications: {
        enabled: true,
        events: {
          'session-idle': {
  ...
  afterEach(async () => {
    ...
    const root = tempGeminiHome ? join(tempGeminiHome, '..') : '';
    clearEnv();

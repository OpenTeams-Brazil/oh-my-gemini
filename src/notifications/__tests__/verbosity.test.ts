...
describe('getVerbosity', () => {
  const origEnv = process.env.OMG_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.OMG_NOTIFY_VERBOSITY;
    } else {
      process.env.OMG_NOTIFY_VERBOSITY = origEnv;
    }
  });

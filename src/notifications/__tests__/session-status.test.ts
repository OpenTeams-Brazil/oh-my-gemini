...
describe('session-status helper', () => {
  ...
  it('renders a bounded running summary with active subagent details', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-session-status-'));
    try {
      await writeSessionStart(wd, 'sess-1', { nativeSessionId: 'native-1' });
      await writeJson(join(wd, '.omg', 'state', 'sessions', 'sess-1', 'skill-active-state.json'), {
        active: true,
        skill: 'ralph',
        phase: 'executing',
      ...
      assert.match(status, /Tmux: omg-session \/ %9/);
  ...
  it('renders stale partial summaries with explicit unknown fields', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-session-status-stale-'));
    try {
      await mkdir(join(wd, '.omg', 'logs'), { recursive: true });
      await writeFile(
        join(wd, '.omg', 'logs', 'session-history.jsonl'),
        `${JSON.stringify({
          session_id: 'sess-old',
  ...
  it('does not report a stale tracked session as running when only raw session.json remains', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-session-status-orphaned-'));
    try {
      await writeJson(join(wd, '.omg', 'state', 'session.json'), {
  ...
  it('returns a bounded failure message when correlated state cannot be resolved', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omg-session-status-empty-'));
    try {
      const status = await buildDiscordSessionStatusReply(createMapping(wd));

...
describe('analyzePaneContent', () => {
  ...
  it('detects "omg" keyword', () => {
    const result = analyzePaneContent('omg session started');
    assert.equal(result.hasGemini, true);
  });

  it('detects "oh-my-gemini" keyword', () => {
    const result = analyzePaneContent('oh-my-gemini v1.0');
    assert.equal(result.hasGemini, true);
  });
  ...
  it('caps confidence at 1.0', () => {
    const result = analyzePaneContent('Gemini $ > agent task running omg');

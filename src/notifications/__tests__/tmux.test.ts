...
describe('getTeamTmuxSessions - session matching', () => {
  ...
  function makeFakeTmux(sessions: string[]): void {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omg-tmux-team-test-'));
  ...
  it('returns canonical session name (omg-team-alpha) for team "alpha"', () => {
    makeFakeTmux(['omg-team-alpha', 'other-session']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omg-team-alpha']);
  });

  it('returns prefixed worker sessions (omg-team-alpha-worker1)', () => {
    makeFakeTmux(['omg-team-alpha-worker1', 'omg-team-alpha-worker2']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omg-team-alpha-worker1', 'omg-team-alpha-worker2']);
  });

  it('does NOT return sessions for a different team', () => {
    makeFakeTmux(['omg-team-beta', 'omg-team-beta-worker1']);
...
describe('captureTmuxPane', () => {
  ...
  it('captures output for valid pane id and sanitizes/clamps lines', () => {
    ...
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omg-tmux-test-'));
    tmpDirs.push(fakeBinDir);
    const tmuxPath = join(fakeBinDir, 'tmux');
    writeFileSync(
      tmuxPath,
  ...
  it('suppresses capture when the target pane is already dead', () => {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omg-tmux-dead-pane-test-'));
    tmpDirs.push(fakeBinDir);
    const tmuxPath = join(fakeBinDir, 'tmux');
    writeFileSync(
      tmuxPath,

...
describe('loadNotificationConfig', () => {
  it('returns null when config file does not exist', async () => {
    const fakePath = join(tmpdir(), `omg-test-${randomUUID()}`);
    const config = await loadNotificationConfig(fakePath);
    assert.equal(config, null);
  });

  it('returns parsed config when file exists', async () => {
    const tmpDir = join(tmpdir(), `omg-test-${randomUUID()}`);
    const omgDir = join(tmpDir, '.omg');
    ...
    writeFileSync(join(omgDir, 'notifications.json'), JSON.stringify(configData));
  ...
  it('returns null for invalid JSON', async () => {
    ...
    const omgDir = join(tmpDir, '.omg');
    mkdirSync(omgDir, { recursive: true });
    writeFileSync(join(omgDir, 'notifications.json'), 'not-json');

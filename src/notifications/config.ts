...
import { geminiHome } from "../utils/paths.js";
import type {
...
const CONFIG_FILE = join(geminiHome(), ".omg-config.json");
...
export function buildConfigFromEnv(): FullNotificationConfig | null {
  ...
  const discordMention = validateMention(process.env.OMG_DISCORD_MENTION);

  const discordBotToken = process.env.OMG_DISCORD_NOTIFIER_BOT_TOKEN;
  const discordChannel = process.env.OMG_DISCORD_NOTIFIER_CHANNEL;
  ...
  const discordWebhook = process.env.OMG_DISCORD_WEBHOOK_URL;
  if (discordWebhook) {
    config.discord = {
      enabled: true,
  ...
  const telegramToken =
    process.env.OMG_TELEGRAM_BOT_TOKEN ||
    process.env.OMG_TELEGRAM_NOTIFIER_BOT_TOKEN;
  const telegramChatId =
    process.env.OMG_TELEGRAM_CHAT_ID ||
    process.env.OMG_TELEGRAM_NOTIFIER_CHAT_ID ||
    process.env.OMG_TELEGRAM_NOTIFIER_UID;
  ...
  const slackWebhook = process.env.OMG_SLACK_WEBHOOK_URL;
  if (slackWebhook) {
    const slackMention = validateSlackMention(process.env.OMG_SLACK_MENTION);
    config.slack = {
      enabled: true,
 ...
 *   2. OMG_NOTIFY_PROFILE environment variable
...
export function resolveProfileConfig(
  ...
  const name =
    profileName ||
    process.env.OMG_NOTIFY_PROFILE ||
...
export function getActiveProfileName(): string | null {
  if (process.env.OMG_NOTIFY_PROFILE) {
    return process.env.OMG_NOTIFY_PROFILE;
  }
  const raw = readRawConfig();
  if (!raw) return null;
  const notifications = raw.notifications as NotificationsBlock | undefined;
...
export function getNotificationConfig(
  ...
  if (raw) {
    ...
    if (notifications) {
      ...
      const envMention = validateMention(process.env.OMG_DISCORD_MENTION);
      if (envMention) {
...
export function getVerbosity(config: FullNotificationConfig | null): VerbosityLevel {
  const envVal = process.env.OMG_NOTIFY_VERBOSITY as string | undefined;
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
  ...
  const enabled = process.env.OMG_REPLY_ENABLED === "true" || replyRaw?.enabled === true;
  if (!enabled) return null;

  const authorizedDiscordUserIds = parseDiscordUserIds(
    process.env.OMG_REPLY_DISCORD_USER_IDS,
  ...
  if (hasDiscordBot && authorizedDiscordUserIds.length === 0) {
    console.warn(
      ...
      "Set OMG_REPLY_DISCORD_USER_IDS or add to .omg-config.json notifications.reply.authorizedDiscordUserIds"
    );
  }

  const pollIntervalMs = normalizeInteger(
    parseIntegerInput(process.env.OMG_REPLY_POLL_INTERVAL_MS)
  ...
  const rateLimitPerMinute = normalizeInteger(
    parseIntegerInput(process.env.OMG_REPLY_RATE_LIMIT)
  ...
  return {
    ...
    maxMessageLength,
    rateLimitPerMinute,
    includePrefix: process.env.OMG_REPLY_INCLUDE_PREFIX !== "false" && (replyRaw?.includePrefix !== false),

...
const REPLY_ACK_PREFIX = 'Injected into Gemini CLI session.';
...
function injectReply(
  ...
  if (analysis.confidence < 0.4) {
    log(`WARN: Pane ${paneId} does not appear to be running Gemini CLI (confidence: ${analysis.confidence}). Skipping injection, removing stale mapping.`);

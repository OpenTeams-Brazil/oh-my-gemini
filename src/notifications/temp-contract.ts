export const OMG_NOTIFY_TEMP_ENV = 'OMG_NOTIFY_TEMP';
export const OMG_NOTIFY_TEMP_CONTRACT_ENV = 'OMG_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[OMG_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[OMG_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[OMG_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {

 ...
 * Gemini CLI, and inject text into panes. Used by the reply-listener daemon.
 */
...
export function analyzePaneContent(content: string): PaneAnalysis {
  ...
  return { hasGemini, hasRateLimitMessage, isBlocked, confidence: Math.min(confidence, 1) };
}
...
export function buildSendPaneArgvs(
  ...
  if (pressEnter) {
    // Gemini CLI uses raw input mode; send C-m (carriage return) twice
 ...
 * Source of truth: Rust runtime's submit_presses_for_worker_cli (Claude=1, Gemini/Other=2).

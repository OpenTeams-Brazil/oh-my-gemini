...
export const OMG_MODELS_START_MARKER = '<!-- OMX:MODELS:START -->';
export const OMG_MODELS_END_MARKER = '<!-- OMX:MODELS:END -->';
...
export function resolveAgentsModelTableContext(
  ...
  const { geminiHomeOverride, env = process.env } = options;
  const frontierModel =
    getRootModelName(configTomlContent) ??
    getEnvConfiguredMainDefaultModel(env, geminiHomeOverride) ??
    DEFAULT_FRONTIER_MODEL;
  const sparkModel =
    getEnvConfiguredSparkDefaultModel(env, geminiHomeOverride) ??
    getSparkDefaultModel(geminiHomeOverride) ??
    DEFAULT_SPARK_MODEL;
  const subagentDefaultModel =
    getEnvConfiguredStandardDefaultModel(env, geminiHomeOverride) ??
    getStandardDefaultModel(geminiHomeOverride) ??
    DEFAULT_STANDARD_MODEL;
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(OMG_MODELS_START_MARKER);
  const endIndex = content.indexOf(OMG_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + OMG_MODELS_END_MARKER.length;

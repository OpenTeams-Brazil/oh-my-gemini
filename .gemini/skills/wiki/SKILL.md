---
name: wiki
description: Persistent markdown project wiki stored under .omg/wiki with keyword search and lifecycle capture
...
## Auto-Capture
At session end, discoveries can be captured as `session-log-*` pages. Configure via `wiki.autoCapture` in `.omg-config.json`.

## Hard Constraints
- No vector embeddings — query uses keyword + tag matching only
- Wiki files remain local project state under `.omg/wiki/`

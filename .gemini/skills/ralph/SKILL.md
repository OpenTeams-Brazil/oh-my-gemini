...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.omg/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_OMG_EXPLORE_CMD`, prefer `omg explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.omg/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `oh-my-gemini:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
- Use `ask_gemini` with `agent_role: "architect"` for verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Skip Gemini consultation for simple feature additions, well-tested changes, or time-critical verification
- If ToolSearch finds no MCP tools or Gemini is unavailable, proceed with architect agent verification alone -- never block on external tools
...
Use the `omg_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
Prompt-side `$ralph` workflow activation is lighter-weight than `omg ralph --prd ...`.
It seeds Ralph workflow state and guidance, but it does not implicitly launch the
CLI entrypoint or apply the PRD startup gate. Treat `omg ralph --prd ...` as the
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.omg/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.omg/plans/prd-{slug}.md`
   - Progress ledger: `.omg/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.omg/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.omg/plans/prd-{slug}.md`, create `.omg/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from `.omg/prd.json`.
- `.omg/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup validation source.
- If `.omg/prd.json` exists and canonical PRD is absent, migrate one-way into `.omg/plans/prd-{slug}.md`.
- If `.omg/progress.txt` exists and canonical progress ledger is absent, import one-way into `.omg/state/{scope}/ralph-progress.json`.

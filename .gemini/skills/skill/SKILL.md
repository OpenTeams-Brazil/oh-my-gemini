...
Meta-skill for managing oh-my-gemini skills via CLI-like commands.
...
**Behavior:**
1. Scan user skills at `~/.gemini/skills/`
2. Scan project skills at `.gemini/skills/`
...
✓ Created skill at ~/.gemini/skills/custom-logger/SKILL.md
...
3. **If confirmed:**
   - Delete entire skill directory (e.g., `~/.gemini/skills/<name>/`)
...
```
Skill: custom-logger
Scope: user (~/.gemini/skills/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.gemini/skills/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.gemini/skills/custom-logger/SKILL.md
...
User: yes
Assistant: ✓ Copied 'error-handler' to .gemini/skills/
...
USER_SKILLS_DIR="$HOME/.gemini/skills"
...
echo "=== USER-LEVEL SKILLS (~/.gemini/skills/) ==="
if [ -d "$HOME/.gemini/skills" ]; then
  USER_COUNT=$(find "$HOME/.gemini/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "$HOME/.gemini/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.gemini/skills/) ==="
if [ -d ".gemini/skills" ]; then
  PROJECT_COUNT=$(find ".gemini/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".gemini/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.gemini/skills/) - Available across all projects
- **Project-level** (.gemini/skills/) - Only for this project
...
**Automatic Application**: Gemini detects triggers and applies skills automatically - no need to remember or search for solutions.

**Version Control**: Project-level skills (.gemini/skills/) are committed with your code, so the whole team benefits.

**Evolving Knowledge**: Skills improve over time as you discover better approaches and refine triggers.

**Reduced Token Usage**: Instead of re-solving the same problems, Gemini applies known patterns efficiently.
...
✓ User skills directory exists: ~/.gemini/skills/
✓ Project skills directory exists: .gemini/skills/

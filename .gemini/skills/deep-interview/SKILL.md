...
- **Autoresearch (`--autoresearch`)**: same interview rigor as Standard, but specialized for `omg autoresearch` launch readiness and `.omg/specs/` mission/sandbox artifact handoff

If no flag is provided, use **Standard**.

<Mode_Flags>
- **`--autoresearch`**: switch the interview into autoresearch-intake mode for `omg autoresearch` handoff. In this mode, the interview should converge on a launch-ready research mission, write canonical artifacts under `.omg/specs/`, and preserve the explicit `refine further` vs `launch` boundary for downstream CLI intake.
</Mode_Flags>
</Depth_Profiles>
...
- In Gemini CLI, prefer `request_user_input` when available; if unavailable, fall back to concise plain-text one-question turns
...
2. Attempt to load the latest relevant context snapshot from `.omg/context/{slug}-*.md`.
...
4. Save snapshot to `.omg/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".omg/context/<slug>-<timestamp>.md"
  }
}
```
...
1. Write interview transcript summary to:
   - `.omg/interviews/{slug}-{timestamp}.md`  
     (kept for ralph PRD compatibility)
2. Write execution-ready spec to:
   - `.omg/specs/deep-interview-{slug}.md`
...
When the clarified task is specifically about `omg autoresearch`, or the skill is invoked with `--autoresearch`, keep the interview domain-specific and emit launch-consumable artifacts without skipping clarification.
...
- **Canonical artifact path:** `.omg/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.omg/specs/autoresearch-{slug}/mission.md`, `.omg/specs/autoresearch-{slug}/sandbox.md`, and `.omg/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.omg/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.omg/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- **Structured result contract:** `result.json` should point to the draft + mission/sandbox artifacts and carry the finalized `topic`, `evaluatorCommand`, `keepPolicy`, `slug`, `launchReady`, and `blockedReasons` fields so `omg autoresearch` can consume it directly
...
- **Input Artifact:** `.omg/specs/deep-interview-{slug}.md` (optionally accompanied by the transcript/context snapshot for traceability)
...
- **Expected Output:** Canonical planning artifacts under `.omg/plans/`, especially `prd-*.md` and `test-spec-*.md`
...
### 2. **`$autopilot`**
- **Input Artifact:** `.omg/specs/deep-interview-{slug}.md`
...
- Read/write context snapshots under `.omg/context/`
- Save transcript/spec artifacts under `.omg/interviews/` and `.omg/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.omg/context/{slug}-{timestamp}.md`
...
- [ ] Transcript written to `.omg/interviews/{slug}-{timestamp}.md`

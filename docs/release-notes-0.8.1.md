...
If you use `$team`/`omg team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- For notification setup, prefer:
  - `omg configure-notifications` (or skill equivalent)
...
Thanks for using **oh-my-gemini**. If anything regresses, please open an issue with repro steps and logs.

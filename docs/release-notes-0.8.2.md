...
- `$team` / team runtime can now launch **Gemini CLI workers** alongside Gemini and Claude (`#576`, `#579`, related issue `#573`).
...
- `omg setup` now skips the deprecated **`[tui]`** config section for Gemini CLI `>= 0.107.0` (`#572`, fixes `#564`).
...
OMX team mode now supports **Gemini** as a worker CLI provider in addition to Gemini and Claude.

Included in this update:
- Gemini worker launch support in runtime/session resolution
- mixed CLI maps with Gemini / Claude / Gemini workers
...
Setup now respects the catalog manifest and current Gemini compatibility more strictly:
...
- skips writing the deprecated `[tui]` section when Gemini CLI is `>= 0.107.0`

**Why this matters:**
- cleaner installs and upgrades
- fewer stale shipped assets after upgrades
- fewer setup/config issues on newer Gemini CLI versions
...
- #572 — fix(setup): skip `[tui]` section for Gemini >= `0.107.0`
- #571 — docs: improve OpenClaw gateway configuration examples

### Related issues tagged in this release
- #564 — setup/config breakage caused by deprecated `[tui]` generation on newer Gemini CLI versions

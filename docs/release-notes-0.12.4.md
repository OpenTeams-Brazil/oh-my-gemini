...
- **MCP-CLI parity** — `omg state`, `omg notepad`, `omg project-memory`, `omg trace`, and `omg code-intel` expose MCP server tools through the CLI, making every MCP operation scriptable without transport overhead.
...
### Added
- MCP-CLI parity surface: `omg state`, `omg notepad`, `omg project-memory`, `omg trace`, `omg code-intel`
...
- User-owned Gemini hooks preserved during `omg setup` refresh
...
### Changed
- State CLI routed consistently through `omg state`

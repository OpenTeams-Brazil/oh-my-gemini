...
multi-state compatibility rollout from `.omg/plans/prd-multi-state-compat.md`.

## Canonical sources of truth

The runtime must treat workflow state as a combination of:

- mode state files under `.omg/state/{scope}/<mode>-state.json`
- canonical workflow enumeration under `.omg/state/{scope}/skill-active-state.json`
...
> incompatible state first via `omg state ...` or the `omg_state.*` MCP tools,
...
- `omg state clear --input '{"mode":"team"}' --json`
- `omg state clear --input '{"mode":"ralph","all_sessions":true}' --json`

MCP parity surface:

- `omg_state.state_clear({ mode: "team" })`
- `omg_state.state_clear({ mode: "ralph", all_sessions: true })`
...
5. denial messages mention both `omg state` and `omg_state.*`

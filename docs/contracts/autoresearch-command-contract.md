# `omg autoresearch` parity contract

`omg autoresearch` is a thin supervisor that drives one Gemini experiment session per iteration while OMX owns the durable keep/discard/reset loop.
...
- `--resume <run-id>` loads `.omg/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.omg/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.omg-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.omg/logs/autoresearch/<run-id>/`

Repo-root state responsibilities:
- `.omg/state/autoresearch-state.json` = active-run pointer/lock only
- `.omg/logs/autoresearch/<run-id>/manifest.json` = authoritative per-run state
- `.omg/logs/autoresearch/<run-id>/candidate.json` = candidate handoff from the just-finished Gemini session
- `.omg/logs/autoresearch/<run-id>/iteration-ledger.json` = durable iteration history
- `.omg/logs/autoresearch/<run-id>/latest-evaluator-result.json` = latest evaluator output

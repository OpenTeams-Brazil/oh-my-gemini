# `omg autoresearch` Full-Parity Review Notes
...
- `.omg/plans/prd-autoresearch-full-parity.md`
- `.omg/plans/test-spec-autoresearch-full-parity.md`
...
- `omg autoresearch` exposes both fresh launch and `--resume <run-id>` flows.
...
**Pass.**
- `buildAutoresearchInstructions()` tells the launched Gemini session to perform exactly one experiment cycle and write the candidate artifact before exit.
- `runAutoresearchLoop()` relaunches Gemini after `processAutoresearchCandidate()` unless the run aborts/errors.

### 2. Repo-root state authority
**Pass.**
- repo-root active-run lock lives at `.omg/state/autoresearch-state.json`.
- per-run manifest, candidate, ledger, and latest evaluator files live under `.omg/logs/autoresearch/<run-id>/`.

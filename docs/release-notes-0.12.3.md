...
`0.12.3` is a tight follow-up patch to `0.12.2`. It ships PR [#1364](https://github.com/Yeachan-Heo/oh-my-gemini/pull/1364), which was intended to land in `0.12.2` but finished its conflict resolution after the `0.12.2` cut — `$team` prompt-routing correctness and duplicate team launch teardown — plus synchronized `0.12.3` release collateral.

## Included fixes and changes

- `$team` detected in `UserPromptSubmit` now seeds root `team-state.json` and nudges operators toward `omg team ...` / `omg team --help` instead of silently misrouting the prompt (#1364)

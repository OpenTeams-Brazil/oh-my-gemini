# Heavy / Manual Stress Lane: `omg-explore` and `omg-sparkshell`
...
| Command | Result |
|---|---|
| `node bin/omg.js explore --help` | PASS |
| `node bin/omg.js sparkshell --help` | PASS |
| `node bin/omg.js sparkshell git --version` | PASS |
| `node bin/omg.js version` | PASS |
| `for i in $(seq 1 20); do node bin/omg.js sparkshell git --version; done` | PASS (bounded sample verified locally) |
| `seq 1 5 | xargs -I{} -P5 node bin/omg.js sparkshell git --version` | PASS (bounded sample verified locally) |
...
**Goal**
- Confirm `omg-sparkshell --tmux-pane` preserves operator-critical facts when pane tails are large and noisy.
...
```bash
node bin/omg.js sparkshell git --version
```

Suggested repeated-run shape:

```bash
for i in $(seq 1 20); do node bin/omg.js sparkshell git --version; done
```

Suggested concurrent-run shape:

```bash
seq 1 5 | xargs -I{} -P5 node bin/omg.js sparkshell git --version
```
...
**Goal**
- Stress `omg-explore` and `omg-sparkshell` summarization with outputs that bury signal under distractors.
...
```bash
node bin/omg.js sparkshell --help
node bin/omg.js explore --help
```

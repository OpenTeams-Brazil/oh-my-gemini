...
- quality-first guidance + agent contract refresh (`GEMINI.md`, `templates/GEMINI.md`, `prompts/*.md`, `docs/prompt-guidance-*`, `skills/team/SKILL.md`)
...
1. **Native Gemini hook ownership moved into the repo/runtime contract.**
   - Call out that non-team OMX sessions now own native Gemini hook setup locally, preserve session-start + stop-state continuity, and expose documented setup/uninstall behavior.
...
|---|---|---|---|
| Native hook ownership + lifecycle | `src/scripts/gemini-native-hook.ts`, `src/config/gemini-hooks.ts`, `src/cli/setup.ts`, `src/cli/uninstall.ts`, `docs/gemini-native-hooks.md` | repo-local native hook ownership; session-start/stop continuity | `npm run build`; `node --test dist/scripts/__tests__/gemini-native-hook.test.js dist/config/__tests__/generator-idempotent.test.js dist/cli/__tests__/setup-scope.test.js dist/cli/__tests__/uninstall.test.js` |
| Native pre/post tool hooks | `src/scripts/gemini-native-pre-post.ts`, `src/scripts/__tests__/gemini-native-hook.test.ts`, `docs/gemini-native-hooks.md` | first-party `PreToolUse` / `PostToolUse` Bash guidance | `node --test dist/scripts/__tests__/gemini-native-hook.test.js` |
...
| CLI version smoke | `node dist/cli/omg.js version` | PASS (`oh-my-gemini v0.12.0`) |
| Build | `npm run build` | PASS |
| Lint | `npm run lint` | PASS |
| Full Node suite | `npm test` | PASS (`2949` pass / `0` fail) |
| Rust runtime core | `cargo test -p omg-runtime-core` | PASS (`54` pass / `0` fail) |

# PR Draft: Deprecate `omg team ralph` and keep team standalone

## Target branch
`dev`

## Title
Deprecate `omg team ralph` and remove linked team↔Ralph lifecycle machinery

## Summary
This PR removes the built-in linked `omg team ralph` workflow and restores a clean separation between `team` and `ralph`.

After this change:
- `omg team ...` / `$team ...` is the only supported team launch path
- `omg ralph ...` / `$ralph ...` remains available as a separate, explicit follow-up
- team no longer creates, syncs, or depends on linked Ralph state
- legacy `omg team ralph ...` usage now fails with a clear deprecation error instead of being tolerated silently
...
- advertised/generated `omg team ralph ...` launch hints
- linked-Ralph-specific tests and compatibility paths inside team runtime

### Updated
- team help/usage text now documents only `omg team [N:agent-type] "<task>"`
...
- follow-up planner and pipeline launch hints now generate plain `omg team ...`
- shutdown/resume/state flows now operate on standalone team semantics only

### Behavior change
- `omg team ralph ...` is now rejected with an explicit deprecation error:
  - use `omg team ...` for coordinated team execution
  - use `omg ralph ...` separately if a later persistent follow-up loop is still needed
...
## Impact
### Before
- `omg team ralph ...` had special runtime behavior
...
### After
- `omg team ...` runs coordinated team execution only
- team owns its own verification lanes and shutdown evidence
- `omg ralph ...` is separate and explicit
...
## Risks / notes
- This is an intentional CLI behavior break for users still invoking `omg team ralph ...`

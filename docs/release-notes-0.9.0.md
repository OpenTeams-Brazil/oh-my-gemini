...
### Spark Initiative: `omg explore` and `omg sparkshell`

OMX now has a stronger native fast path for repository discovery and shell-native inspection.

This release:
- introduces `omg explore` as the default read-only exploration entrypoint
- adds the Rust-backed explore harness plus packaging and source-fallback flow
- introduces `omg sparkshell <command> [args...]` as an explicit operator-facing native sidecar
- allows qualifying read-only shell-native `omg explore` tasks to route through `omg sparkshell`
- keeps the explore path intentionally constrained: shell-only, read-only, and allowlisted

Representative changes:
- `fb07c3c` — feat: add omg explore harness and packaging flow
- `71858c3` — feat: add omg sparkshell and team inspection metadata
...
This release:
- unifies cross-platform native publishing for `omg-explore-harness` and `omg-sparkshell`
...
This release:
- exposes `omg sparkshell --tmux-pane <pane-id> --tail-lines <100-1000>` for explicit pane summarization
...
- Windows Gemini command shim probing fix (`8fc859c`, PR `#793`)
...
- Expect `omg explore` and `omg sparkshell` packaged installs to rely on release-asset hydration when no explicit binary override or repo-local artifact is present.
...
- [#781](https://github.com/Yeachan-Heo/oh-my-gemini/pull/781) — force low reasoning for sparkshell summaries
- [#744](https://github.com/Yeachan-Heo/oh-my-gemini/issues/744) — persist lifecycle profiles for linked Ralph vs default team runs
- [#745](https://github.com/Yeachan-Heo/oh-my-gemini/issues/745) — team cleanup policy hardening
- [#746](https://github.com/Yeachan-Heo/oh-my-gemini/issues/746) — team policy/governance split follow-through
- [#741](https://github.com/Yeachan-Heo/oh-my-gemini/issues/741) — release-readiness / linked team-Ralph runtime follow-up
- [#732](https://github.com/Yeachan-Heo/oh-my-gemini/issues/732) — related team stall/lifecycle cleanup follow-up

...
This is a **setup-flow patch release** focused on making `omg setup` refresh behavior safer, more predictable, and easier to rerun.

---

## TL;DR

- `omg setup` now refreshes managed OMX artifacts by default instead of leaving stale generated content behind.
- Managed refresh paths now preserve backups where applicable before overwriting files.
- Setup prompts before upgrading managed Gemini model references from `gpt-5.3-gemini` to `gpt-5.4`.
...
Setup now treats managed OMX artifacts as refreshable outputs rather than one-time drops. Re-running `omg setup` updates shipped artifacts more consistently, helping existing installations stay aligned with current templates and generated assets.
...
When managed configuration refreshes would upgrade Gemini model references from `gpt-5.3-gemini` to `gpt-5.4`, setup now asks before making that change.
...
- `6aa577d` — feat(setup): prompt before upgrading gpt-5.3-gemini to gpt-5.4
...
Thanks for using **oh-my-gemini**. If anything regresses, please open an issue with reproduction steps, logs, and your CLI/runtime details.

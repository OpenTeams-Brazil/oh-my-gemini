...
- Setup prompts before upgrading managed Gemini model references from `gpt-5.3-codex` to `gpt-5.4`.
...
When managed configuration refreshes would upgrade Gemini model references from `gpt-5.3-codex` to `gpt-5.4`, setup now asks before making that change.

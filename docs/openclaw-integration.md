  ...
  "notifications": {
    ...
    "openclaw": {
      ...
      "gateways": {
        "local": {
          "type": "command",
          "command": "(clawdbot agent --session-id omg-hooks --message {{instruction}} --thinking minimal --deliver --reply-channel discord --reply-to 'channel:1468539002985644084' --timeout 120 --json >>/tmp/omg-openclaw-agent.jsonl 2>&1 || true)",
          "timeout": 120000
        }
      },
      "hooks": {
...
clawdbot agent --session-id omg-hooks \
...
```bash
curl -sS -o /tmp/omg-openclaw-agent-check.json -w "HTTP %{http_code}\n" \

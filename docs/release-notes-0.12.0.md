...
`0.12.0` is a minor release after `0.11.13` because the shipped delta is broad and user-visible across native Gemini hook behavior, team/runtime delivery, Windows/tmux supervision, and workflow documentation.

## Included fixes and changes

- native Gemini hook ownership now lives in the repo/runtime contract for non-team OMX sessions
...
- `npm ci` ✅
- `npm run build` ✅
- `node dist/cli/omg.js version` ✅ (`oh-my-gemini v0.12.0`)

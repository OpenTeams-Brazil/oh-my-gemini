...
- use `omg setup` and need predictable install scope behavior,
- run `omg team`/tmux workflows and want the latest reliability fixes,
...
- `omg setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are auto-migrated.
...
| `$deepinit` | `omg agents-init [path]` | Lightweight CLI successor for GEMINI.md bootstrap only; immediate child directories only, unmanaged files preserved unless `--force`. |
| `$learn-about-omg` / `$learner` / `$writer-memory` | No in-repo replacement | Remove stale references from workflows/docs. |
...
- [ ] Confirm removed references are gone from local notes/scripts:
  ```bash
  rg -n "deep-executor|scientist|pipeline|project-session-manager|\bpsm\b|ultrapilot|learn-about-omg|writer-memory|learner|deepinit|\brelease\b" README.md docs scripts .omg -S
  ```
...
- [ ] If using spark worker routing, verify flags are available:
  ```bash
  omg --help | rg "spark|madmax-spark"
  ```

...
Launching `omg --madmax --xhigh` on an Apple Silicon Mac caused `syspolicyd`
...
- `omg --madmax --xhigh` in `GhostVMPriv`
...
- Stale `.omg/state` can still keep `leader_nudge` polling active; this patch

...
<p align="center">
  ...
  <br>
  <em>Починайте з Gemini впевненіше — нехай OMX додасть кращі промпти, робочі процеси та підтримку під час виконання, коли обсяг роботи зростає.</em>
</p>
...
**Вебсайт:** https://yeachan-heo.github.io/oh-my-gemini-website/
...
- зберігати керівні принципи проєкту, плани, журнали та стан у `.omg/`
...
Далі працюйте звично всередині Gemini:
...
Використовуйте OMX, якщо вам уже подобається Gemini і ви хочете зручнішу щоденну роботу з ним:
...
- керівні принципи проєкту через локальний `GEMINI.md`
- постійний стан у `.omg/` для планів, журналів, пам'яті та відстеження режимів

Якщо ви хочете простий Gemini без додаткового шару робочих процесів, OMX вам, мабуть, не потрібен.
...
1. Запустіть `omg setup`
2. Запустіть з `omg --madmax --high`
...
- `omg setup` встановлює промпти, навички, конфіг і структуру AGENTS
- `omg doctor` перевіряє встановлення, коли щось здається не так
- `omg hud --watch` — це інструмент моніторингу стану, а не основний робочий процес користувача

### Explore і sparkshell

- `omg explore --prompt "..."` призначений для пошуку в репозиторії лише для читання
- `omg sparkshell <command>` призначений для перевірки в терміналі та обмеженої валідації
...
- `omg setup` має увімкнути нативні хуки Gemini (`[features].gemini_hooks = true`) у
  підтримуваних областях.
- локальні хуки Gemini на рівні репозиторію є канонічною поверхнею автоматизації для не-командних сесій
- `omg tmux-hook` зарезервовано для поведінки командного рушія та усунення проблем із застарілим tmux
...
`omg team` потребує інструмента, сумісного з tmux:

...
<p align="center">
  ...
  <br>
  <em>Ξεκινήστε το Gemini πιο δυναμικά και αφήστε το OMX να προσθέσει καλύτερα prompts, workflows και runtime υποστήριξη όταν η δουλειά μεγαλώνει.</em>
</p>
...
- να κρατάτε οδηγίες έργου, σχέδια, αρχεία καταγραφής και κατάσταση στον φάκελο `.omg/`
...
Στη συνέχεια εργαστείτε κανονικά μέσα στο Gemini:
...
Χρησιμοποιήστε το OMX αν σας αρέσει ήδη το Gemini και θέλετε ένα καλύτερο καθημερινό περιβάλλον εργασίας γύρω του:
...
- καθοδήγηση έργου μέσω scoped `GEMINI.md`
- επίμονη κατάσταση στον `.omg/` για σχέδια, αρχεία καταγραφής, μνήμη και παρακολούθηση λειτουργίας

Αν θέλετε απλό Gemini χωρίς επιπλέον επίπεδο ροής εργασίας, πιθανότατα δεν χρειάζεστε το OMX.
...
Το OMX **δεν** αντικαθιστά το Gemini.

Προσθέτει ένα καλύτερο workflow layer γύρω του:
- **Gemini** κάνει την πραγματική δουλειά του agent
...
- **`.omg/`** αποθηκεύει σχέδια, αρχεία καταγραφής, μνήμη και κατάσταση εκτέλεσης
...
1. Εκτελέστε `omg setup`
2. Εκκινήστε με `omg --madmax --high`
...
- `omg setup` εγκαθιστά prompts, skills, ρυθμίσεις και scaffolding AGENTS
- `omg doctor` επαληθεύει την εγκατάσταση όταν κάτι φαίνεται λάθος
- `omg hud --watch` είναι λειτουργία παρακολούθησης/κατάστασης, όχι η κύρια ροή εργασίας του χρήστη

### Explore και sparkshell

- `omg explore --prompt "..."` είναι για αναζήτηση μόνο ανάγνωσης στο repository
- `omg sparkshell <command>` είναι για επιθεώρηση απευθείας από το shell και στοχευμένη επαλήθευση
...
Η `omg team` χρειάζεται ένα tmux-συμβατό backend:

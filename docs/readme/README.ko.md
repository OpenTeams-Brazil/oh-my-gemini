...
[OpenAI Gemini CLI](https://github.com/openai/codex)를 위한 멀티 에이전트 오케스트레이션 레이어.
...
이것은 `CODEX_HOME`의 `GEMINI.md`와 프로젝트 `GEMINI.md`(있는 경우)를 병합한 뒤 런타임 오버레이를 추가합니다.
Gemini 동작을 확장하지만, Gemini 핵심 시스템 정책을 대체/우회하지 않습니다.

제어:

```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omg     # GEMINI.md 주입 비활성화
...
OMX_TEAM_WORKER_CLI=codex   # Gemini CLI 워커 강제
...
- 시작 동작: 저장된 범위가 `project`이면, `omg` 시작 시 자동으로 `CODEX_HOME=./.gemini`를 사용합니다 (`CODEX_HOME`이 이미 설정되지 않은 경우).
- 시작 지침은 `~/.gemini/GEMINI.md`(또는 `CODEX_HOME/GEMINI.md`)와 프로젝트 `./GEMINI.md`를 병합한 뒤 런타임 오버레이를 추가해 사용합니다.

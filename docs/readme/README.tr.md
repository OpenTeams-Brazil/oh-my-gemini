...
[OpenAI Gemini CLI](https://github.com/openai/gemini) için çok ajanlı orkestrasyon katmanı.
...
- Eklentiler varsayılan olarak kapalıdır; `OMG_HOOK_PLUGINS=1` ile etkinleştirin.
...
Bu, `GEMINI_HOME` içindeki `GEMINI.md` ile proje `GEMINI.md` dosyasını (varsa) birleştirir ve ardından çalışma zamanı kaplamasını ekler.
Gemini davranışını genişletir, ancak Gemini çekirdek sistem politikalarını değiştirmez/atlamaz.

Kontroller:

```bash
OMG_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omg     # GEMINI.md enjeksiyonunu devre dışı bırak
...
```bash
OMG_TEAM_WORKER_CLI=auto    # varsayılan; worker --model "claude" içeriyorsa claude kullanır
OMG_TEAM_WORKER_CLI=gemini   # Gemini CLI çalışanlarını zorla
OMG_TEAM_WORKER_CLI=claude  # Claude CLI çalışanlarını zorla
OMG_TEAM_WORKER_CLI_MAP=gemini,gemini,claude,claude  # çalışan başına CLI karışımı (uzunluk=1 veya çalışan sayısı)
OMG_TEAM_AUTO_INTERRUPT_RETRY=0  # isteğe bağlı: adaptif queue->resend geri dönüşünü devre dışı bırak
```

Notlar:
- Worker başlatma argümanları hâlâ `OMG_TEAM_WORKER_LAUNCH_ARGS` aracılığıyla paylaşılır.
- `OMG_TEAM_WORKER_CLI_MAP`, çalışan başına seçim için `OMG_TEAM_WORKER_CLI`'yi geçersiz kılar.
...
- Başlatma davranışı: kalıcı kapsam `project` ise, `omg` başlatma otomatik olarak `GEMINI_HOME=./.gemini` kullanır (`GEMINI_HOME` zaten ayarlanmadıysa).
- Başlatma talimatları `~/.gemini/GEMINI.md` (veya geçersiz kılındıysa `GEMINI_HOME/GEMINI.md`) ile proje `./GEMINI.md` dosyasını birleştirir ve ardından çalışma zamanı kaplamasını ekler.

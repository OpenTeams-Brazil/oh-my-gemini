...
- Register the server as `omg_wiki`.
- Keep wiki storage under `.omg/wiki/`.
- Do **not** add vector embeddings; query stays keyword/tag based.

## Config + generator contract

`omg setup` / the config generator should own the dedicated wiki MCP block:
...
The bootstrap/config path should treat `omg_wiki` as a first-party OMX server
...
- `.omg/wiki/*.md` — content pages
- `.omg/wiki/index.md` — generated catalog
- `.omg/wiki/log.md` — append-only operation log
...
- `SessionStart` stays **native** and **bounded**.
  - It may read `.omg/wiki/` and surface brief context when the wiki already exists.
...
The dedicated `omg_wiki` server should expose the seven stabilized wiki tools:
...
These tools should operate only on `.omg/wiki/` and keep lifecycle behavior bounded.
